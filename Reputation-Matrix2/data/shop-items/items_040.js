// Shop items chunk 40 of 51
// Items 3901 to 4000 (100 items)
// Auto-generated: 2026-03-21 13:41:41

import { SHOP_CATEGORIES } from './categories.js';

export const ITEMS_040 = {
  'shadowfell_shadow_ward': {
        id: 'shadowfell_shadow_ward',
        name: "Shadow Ward of the Onyx Hand",
        description: "A heavy cloak woven from the essence of forgotten horrors. Grants the wearer immunity to illusions and minor psychic manipulation.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3000,
        icon: '🕯',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+15% resistance to illusions",
        "+20% damage reduction from psychic attacks",
        "-10% movement speed (but gains +30% stealth)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_shadowwhisper_ambush': {
        id: 'shadowfell_shadowwhisper_ambush',
        name: "Shadowwhisper Ambush",
        description: "A cursed dagger that whispers your enemy’s fears before they strike. Its blade shimmers with onyx veins, and when swung, it momentarily paralyzes foes with psychic dread. Perfect for assassins in the Shadowfell’s shifting corridors.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% critical hit chance",
        "Inflicts Fear on target for 3 turns",
        "On hit, deals 10% extra damage to undead"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_shard_of_hyrule': {
        id: 'shadowfell_shard_of_hyrule',
        name: "Shard of Hyrule",
        description: "A fractured piece of the Triforce, stolen from the Temple of Echoes and hidden in the Shadowfell’s depths. It pulses with ancient power, granting temporary protection from dark magic. But if used too often, it begins to whisper of the Triforce’s betrayal, and the caster may be drawn into the cycle of reincarnation.",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 50000,
        icon: '🏯',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Grants immunity to dark magic for 10 rounds",
        "Can be used to activate the Triforce’s echo",
        "Causes memory distortion after 3 uses",
        "Requires attunement with a sacred beast",
        "Triggers reincarnation sequence if used by a soulless being",
        "Crafted by: Asgard"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Cheep Cheep Shipping',
        levelRequirement: 15
    },

  'shadowfell_shroud_of_the_unseen': {
        id: 'shadowfell_shroud_of_the_unseen',
        name: "Shroud of the Unseen",
        description: "A cloak that blends with the shadows, granting the wearer perfect concealment. Activates when entering darkness or shadowed zones. Leaves a faint trace of the user’s presence if used in daylight.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2800,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "100% stealth in shadowed zones",
        "+10% chance to trigger ambush on enemies",
        "Reveals hidden enemy positions for 3s"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_silence_essence': {
        id: 'shadowfell_silence_essence',
        name: "Essence of the Silent Vault",
        description: "A vial containing the essence of a vampire who chose silence over vengeance. When consumed, it grants the user temporary immunity to psychic attacks and allows them to hear the whispers of the Shadowfell without being affected.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Immunity to psychic attacks for 10 minutes",
        "Allows hearing of Shadowfell whispers",
        "Causes temporary mental fog after use"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_silken_haunting': {
        id: 'shadowfell_silken_haunting',
        name: "Silken Haunting",
        description: "A cloak woven from the dreams of forgotten souls, granting the wearer invisibility in dim light and amplifying their stealth. The fabric drips with sorrow, making enemies hesitate before striking.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 10% chance to dodge attacks in low light",
        "Adds +1 to Stealth checks",
        "Causes enemies to hesitate for 1 round on first sight"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_silken_silence': {
        id: 'shadowfell_silken_silence',
        name: "Silken Silence of the Unseen",
        description: "A cloak woven from the whispers of forgotten vampires. It mutes all ambient sound and grants the wearer invisibility in shadow zones. Ideal for sneaking past patrols or hiding during feuds.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reduces sound detection radius by 30%",
        "Grants 30 seconds of invisibility in shadows",
        "Unlocks passive: +10% dodge chance in darkness"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_silken_sorrow': {
        id: 'shadowfell_silken_sorrow',
        name: "Silken Sorrow Vial",
        description: "A vial containing the essence of a slain Onyx Hand’s regret. Drinking it grants temporary empathy with the damned, but may cause the user to see visions of their own past sins. Not for the faint of heart.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1200,
        icon: '🍸',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "+5% chance to dodge enemy attacks",
        "Grants temporary ‘Soul Echo’ – user can cast a minor fear aura (30s duration)",
        "-10% movement speed if the user is above 70% HP"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 3
    },

  'shadowfell_silken_soul_cloth': {
        id: 'shadowfell_silken_soul_cloth',
        name: "Silken Soul Cloth",
        description: "Woven from the dreams of the dead, this cloth absorbs ambient negativity and transforms it into raw energy. Wearing it in battle grants a shield of whispered sorrow.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reduces incoming damage by 15%",
        "Regenerates 10% HP per second while under enemy attack",
        "Grants +5% to all stealth abilities"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_siphon_blade': {
        id: 'shadowfell_siphon_blade',
        name: "Siphon Blade of the Void",
        description: "A blade forged from the essence of a fallen Onyx Hand, it drains vitality from foes and channels it into the wielder’s own health. Its edge glows with stolen life, making it deadly in close quarters.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+50% damage to undead",
        "-10% HP regeneration per second when used in combat",
        "Absorbs 10% of enemy HP into wielder’s HP upon strike"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_soul_ash_pouch': {
        id: 'shadowfell_soul_ash_pouch',
        name: "Pouch of the Ashen Soul",
        description: "Contains the fragmented essence of a fallen vampire, allowing the wearer to absorb enemy soul damage and convert it into temporary vitality. Perfect for ambushers and assassins.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1500,
        icon: '🔮',
        stock: 5,
        rarity: 'rare',
        effects: [
            "Restores 10% of max HP from enemy soul damage taken",
        "Grants +5% critical hit chance",
        "Consumes 1 soul per use"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 4
    },

  'shadowfell_soul_cage': {
        id: 'shadowfell_soul_cage',
        name: "Soul Cage",
        description: "A cursed amulet that traps the essence of the captured. Wears it to siphon energy from nearby enemies, but at the cost of your own stamina.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1200,
        icon: '📦',
        stock: 5,
        rarity: 'rare',
        effects: [
            "Siphons 50% of target’s current HP as energy (1 turn)",
        "Reduces caster’s stamina by 30% for 3 turns",
        "If target is undead, chance to summon a shadowy echo for 2 turns"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 4
    },

  'shadowfell_soul_chime': {
        id: 'shadowfell_soul_chime',
        name: "Soul Chime",
        description: "A hollow crystal bell that resonates with the echoes of forgotten souls. When struck, it summons a temporary spectral guardian, and the chime itself drains vitality to fuel its power.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '🎵',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants 1 round of spectral guardian support (20% damage boost, 5s duration)",
        "Restores 5% max HP per strike used",
        "Inflicts 1d6 necrotic damage to enemies within 5m after summon"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_soul_coral_key': {
        id: 'shadowfell_soul_coral_key',
        name: "Soul Coral Key",
        description: "A bioluminescent key carved from coral that glows with the sorrow of lost souls. It unlocks the gates of the Coral Fleet’s hidden ports, but each use drains a fraction of the wearer’s vitality. Found in the ruins of Isle Delfino, it was once used to trade souls for passage through the Shadowfell’s deepest trenches.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 800,
        icon: '🌊',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Opens hidden ports on Coral Fleet vessels",
        "Each use reduces HP by 1d4",
        "Grants +1 to Stealth checks in dark environments",
        "Requires attunement with a soul trader",
        "Crafted by: Noki Elders"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Blooper Wetworks',
        levelRequirement: 4
    },

  'shadowfell_soul_of_the_fallen_blood': {
        id: 'shadowfell_soul_of_the_fallen_blood',
        name: "Soul of the Fallen Blood",
        description: "A vial containing the essence of a vampire elder who died in a battle of shadows. Infuses the user with dark charisma and temporary regeneration.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Regenerate 15 vitality for 10 seconds",
        "Gain +10% chance to dodge attacks",
        "Infuses Dark Charisma: Increases enemy fear by 30%"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_soul_shard': {
        id: 'shadowfell_soul_shard',
        name: "Soul Shard of the Fallen Mirror",
        description: "A broken piece of a mirror that once held a soul trapped in the Shadowfell’s deepest void. Wears it to absorb negative energy, turning it into temporary power. Use with caution—may drain your sanity.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Absorbs 500 negative energy per use",
        "Grants +30% damage to dark magic spells",
        "-10% stamina after use"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_soul_shard_of_peasley': {
        id: 'shadowfell_soul_shard_of_peasley',
        name: "Soul Shard of Peasley",
        description: "A shard of dark glass that glows with Prince Peasley’s stolen essence",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 5000,
        icon: '🌑',
        stock: 0,
        rarity: 'legendary',
        effects: [
            "Can be used to trade souls for diplomatic favors",
        "Grants 10% resistance to fear effects",
        "Causes 3d6 psychic damage to enemies who resist trade",
        "Unlocks secret chambers in Beanbean Kingdom",
        "Causes hallucinations if used without attunement",
        "Crafted by: Paratroopa Air",
        "Thrown into a mirror to summon Peasley’s ghost"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Paratroopa Air',
        levelRequirement: 10
    },

  'shadowfell_soul_shroud': {
        id: 'shadowfell_soul_shroud',
        name: "Soul Shroud",
        description: "A spectral cloak woven from the lingering essence of the damned. Grants invisibility for 10 seconds per use and siphons 10% of enemy HP into the user’s reserves.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Invisibility for 10 seconds",
        "Siphons 10% of target HP per second for 5 seconds",
        "+10% evasion chance while active"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_soul_siphon': {
        id: 'shadowfell_soul_siphon',
        name: "Siphon of the Unseen Bloodline",
        description: "A vial that allows the user to drain a soul’s essence for temporary power boosts. Each use drains 10% of the user’s HP and grants +15% damage for 10s. Use wisely — the soul’s pain echoes in your veins.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '💉',
        stock: 4,
        rarity: 'rare',
        effects: [
            "+15% damage for 10s per use",
        "-10% HP after use",
        "+5% chance to trigger enemy fear"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 4
    },

  'shadowfell_soul_siphon_cuff': {
        id: 'shadowfell_soul_siphon_cuff',
        name: "Soul Siphon Cuff",
        description: "A blackened brass cuff that drains vitality from enemies. Wears its user as a vessel of the void, allowing them to absorb energy from foes and convert it into temporary strength.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Absorbs 50% of enemy vitality on hit",
        "Restores 10% health per 10 seconds of absorption",
        "Reduces enemy regeneration by 30% while active"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 6
    },

  'shadowfell_soul_siphon_harness': {
        id: 'shadowfell_soul_siphon_harness',
        name: "Soul Siphon Harness",
        description: "A dark leather harness that drains life essence from nearby creatures",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4500,
        icon: '🩸',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Siphons 1d4 HP from enemies within 30 feet",
        "Grants +2 to stealth checks while wearing",
        "Causes 1d4 temporary damage to wearer after 30 seconds of use",
        "Can be used only once per session",
        "Requires attunement by a vampire or necromancer",
        "Crafted by: Onyx Hand"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Cheep Cheep Shipping',
        levelRequirement: 6
    },

  'shadowfell_soul_siphon_pouch': {
        id: 'shadowfell_soul_siphon_pouch',
        name: "Soul Siphon Pouch",
        description: "A pouch woven from the inner skin of a forgotten vampire, designed to collect the life essence of the slain. Use it to replenish vitality at the cost of temporary necrotic damage.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 30% of max HP when used",
        "Deals 20% damage to nearby enemies on next attack",
        "Applies Necrotic Echo — 20% chance to stagger enemy for 1 turn"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadowfell_soul_tether': {
        id: 'shadowfell_soul_tether',
        name: "Soul Tether of the Crimson Veil",
        description: "A vial of captured despair that binds the user to the emotional residue of slain foes. Each use drains 10% of the user’s vitality but grants a temporary psychic shield.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1800,
        icon: '📦',
        stock: 5,
        rarity: 'rare',
        effects: [
            "+30% damage to undead targets",
        "+15% resistance to psychic attacks",
        "-5% movement speed when active (but no longer vulnerable)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_soulbound_chalice': {
        id: 'shadowfell_soulbound_chalice',
        name: "Soulbound Chalice of the Hollow Throne",
        description: "A goblet that drinks souls to fuel its wielder’s darkest desires. It shrouds the drinker in shadow and grants control over spectral entities.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 20% bonus to stealth and shadow movement",
        "Allows temporary possession of a spectral entity for 1 minute",
        "Consumes 1 soul point from the caster’s soul pool"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_soulshard_catalyst': {
        id: 'shadowfell_soulshard_catalyst',
        name: "Soulshard Catalyst",
        description: "A pulsating shard of fractured essence, it channels the raw power of the dying to fuel dark rituals. When infused with a victim’s last breath, it amplifies spells and curses with terrifying potency.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+30% damage to curses and spells cast in shadowy zones",
        "Grants temporary invisibility for 3 seconds after casting",
        "Consumes a soul essence from the caster’s current target"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_spectral_shroud': {
        id: 'shadowfell_spectral_shroud',
        name: "Spectral Shroud of the Onyx Hand",
        description: "A shifting veil of dark energy that conceals the wearer from physical sight and sound. Perfect for stealthy ambushes or escaping vampire patrols. Glows faintly when near hostile entities.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Conceals from sight and sound within 10 meters",
        "Gains 10% damage resistance when near enemies",
        "Glows when proximity to hostile creatures"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_spectral_tattoo': {
        id: 'shadowfell_spectral_tattoo',
        name: "Ink of the Forgotten Veil",
        description: "A vial containing ink that can be applied to skin or parchment. It temporarily binds the wearer to the Shadowfell’s whispered truths, granting psychic awareness of nearby undead and hidden portals.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 750,
        icon: '🖋',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "+10% stealth bonus while wearing",
        "Reveals hidden enemy spawn points",
        "Temporary aura of dread (5s cooldown)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 3
    },

  'shadowfell_tainted_essence': {
        id: 'shadowfell_tainted_essence',
        name: "Tainted Essence of the Crimson Veil",
        description: "A pulsating vial of corrupted blood that amplifies dark magic and drains vitality from foes. Siphons life to fuel your spells, but leaves you weaker after use.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+30% spell damage",
        "-20% max HP while active",
        "10s cooldown after use"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_talisman_of_rebirth': {
        id: 'shadowfell_talisman_of_rebirth',
        name: "Talisman of Rebirth",
        description: "A cracked, glowing artifact forged in the ruins of the Oracle’s lab. It whispers echoes of reincarnation and lets the wearer relive moments of their past. But each time it is activated, it consumes a soul fragment from the caster’s own memory. The Oracle’s vessel grows stronger with each soul given.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 3000,
        icon: '🧬',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Gain 10 Coins when activated",
        "Triggers memory recall once per day",
        "Costs 1 soul fragment to activate",
        "Can be used to bind souls to a vessel",
        "Cannot be used by non-magical beings",
        "Crafted by: Asgard"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Cheep Cheep Shipping',
        levelRequirement: 7
    },

  'shadowfell_tattered_soul_satchel': {
        id: 'shadowfell_tattered_soul_satchel',
        name: "Tattered Soul Satchel",
        description: "A stitched leather pouch dripping with dark magic, it holds fragments of lost souls. When opened, it emits a low hum that soothes psychic distress and amplifies empathy. Ideal for those navigating the Fractured Atrium’s unstable corridors.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1200,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+10% chance to resist psychic traps",
        "Empathic resonance: grants temporary insight into nearby creatures’ emotions",
        "Consumed to restore 50% of maximum willpower"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_tear_of_the_void': {
        id: 'shadowfell_tear_of_the_void',
        name: "Tear of the Void",
        description: "A shard of broken dimension that whispers the forgotten names of fallen souls. Wears its user into the shadows, granting temporary invisibility and a chilling aura of dread.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Invisibility for 30 seconds",
        "Deals 10% extra damage to undead",
        "Inflicts “Shadow Hunger” on enemies — they become visibly drawn to the user’s presence"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_tongue_of_the_onyx_hand': {
        id: 'shadowfell_tongue_of_the_onyx_hand',
        name: "Tongue of the Onyx Hand",
        description: "A cursed relic of the vampire lords of the Shadowfell. When spoken to, it whispers truths only the dead remember. Sustains the user with dark energy but drains sanity with every use.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 200 HP per use",
        "Grants 10% increased damage to enemies with 0 HP",
        "Increases chance of critical hit by 15%"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_tongue_of_the_void': {
        id: 'shadowfell_tongue_of_the_void',
        name: "Tongue of the Void",
        description: "A jagged, obsidian tongue that speaks in forgotten tongues",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 750,
        icon: '🗡',
        stock: 1,
        rarity: 'uncommon',
        effects: [
            "Grants +1 to all stealth checks",
        "When used in combat reveals enemy weaknesses",
        "Consumes 100 Beanbean coins per use",
        "Causes temporary paranoia in nonvampires",
        "Can be used to whisper secrets to undead",
        "Crafted by: Paratroopa Air"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Paratroopa Air',
        levelRequirement: 3
    },

  'shadowfell_vampire_blade': {
        id: 'shadowfell_vampire_blade',
        name: "Onyx Fang of the Veil",
        description: "A wickedly sharp blade forged from the shadowed bones of fallen vampires. It drips with the essence of the dead, granting its wielder temporary necrotic resonance. Perfect for those who walk the line between life and oblivion.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% damage against undead",
        "-10% defense when under 50% HP",
        "+10% chance to inflict a curse on enemies on hit"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampire_bloodstone': {
        id: 'shadowfell_vampire_bloodstone',
        name: "Bloodstone of the Crimson Veil",
        description: "A jagged obsidian crystal infused with the essence of an ancient vampire. When held, it grants temporary strength to the bearer but drains their sanity if used beyond 3 uses.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 3000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Power Surge: +50% damage for 30 seconds",
        "Sanity Drain: -10% sanity per use (max 3 uses)",
        "Vampire Echo: Upon activation, briefly mimics the target’s last attack"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadowfell_vampire_crown': {
        id: 'shadowfell_vampire_crown',
        name: "Crown of the Hollow King",
        description: "A cursed regalia that amplifies a vampire’s dark charisma and grants them the ability to summon a spectral companion for one turn. Worn by those who dare to rule the void.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+30% charisma bonus",
        "+10% damage to enemies with negative stats",
        "Summon a spectral shadow companion (1 turn, 20% chance to trigger a fear effect)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampire_eyes': {
        id: 'shadowfell_vampire_eyes',
        name: "Vampiric Gaze of the Onyx Hand",
        description: "Glows with the essence of the Shadowfell’s oldest vampires. Grants temporary invisibility to the wearer and amplifies their ability to detect hidden enemies. Perfect for stalking the Purple Legion’s weak points.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Invisibility for 10 seconds",
        "Detect Hidden Enemies (reveals enemy location on map)",
        "+20% damage against stealth enemies"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_vampire_forged_glove': {
        id: 'shadowfell_vampire_forged_glove',
        name: "Vampire Forged Glove of the Veil",
        description: "A pair of gloves that absorb the soul of any living being touched. Worn by the Iron Legion, they enhance shadow movement and allow the wearer to phase through solid objects, but at the cost of a soul each time they do so.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Phase through solid objects (1 use per day)",
        "Enhances shadow movement by 30%",
        "Consumes a soul per phase"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_vampire_heart_pendant': {
        id: 'shadowfell_vampire_heart_pendant',
        name: "Vampire Heart Pendant",
        description: "A pendant shaped like a pulsating heart, infused with the essence of a fallen Onyx Hand. Wears it to gain a minor boost in charisma and influence the minds of those who hear your voice.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+10% persuasion chance",
        "+10% chance to disarm enemies with dialogue",
        "Grants +10% stealth while moving"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_vampire_heart_vial': {
        id: 'shadowfell_vampire_heart_vial',
        name: "Vampire Heart Vial",
        description: "A vial containing the pulsing essence of a fallen Onyx Hand. Infuses the user with vampiric vitality, granting regeneration and a chilling aura that frightens foes. Best used in ambushes or shadowed corners.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '🔮',
        stock: 5,
        rarity: 'rare',
        effects: [
            "Regenerates 20% HP per 5s",
        "+10% damage to enemies with negative traits",
        "Grants 30% chance to silence target on hit"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_vampire_horn': {
        id: 'shadowfell_vampire_horn',
        name: "Horn of the Hollow Throne",
        description: "A twisted horn carved from the bone of a fallen Onyx Hand. When used, it summons a shadowy phantom to attack enemies and restore the user’s vitality.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Summons a phantom for 10 seconds that deals 300 damage",
        "Restores 20 HP to user after phantom is destroyed",
        "Phantom deals 50% damage if target is below 30% HP"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_vampire_mantle': {
        id: 'shadowfell_vampire_mantle',
        name: "Vampire Mantle of the Black Veil",
        description: "A cloak woven from the shadowed essence of a thousand Onyx Hands. Grants the wearer the ability to phase through barriers and summon fleeting phantoms to assist in battle.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Phases through solid terrain for 5 seconds",
        "Summons 1 ghostly phantom that follows for 10 seconds",
        "Grants +20% stealth rating while wearing"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 6
    },

  'shadowfell_vampire_marrow': {
        id: 'shadowfell_vampire_marrow',
        name: "Vampire Marrow",
        description: "A vial containing the essence of a fallen vampire’s soul, granting temporary vampiric regeneration and a chilling aura that weakens enemies’ morale. Must be consumed during battle.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Regenerates 1d4 HP per turn while active",
        "Causes enemies to roll a failed morale check (DC 10)",
        "Grants +2 to Will saves against fear effects"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampire_scarf_of_wicked_sorrow': {
        id: 'shadowfell_vampire_scarf_of_wicked_sorrow',
        name: "Scarf of Wicked Sorrow",
        description: "Woven from the loom of a forgotten vampire queen’s tears, this scarf whispers the sorrows of the damned. Wearing it drains the soul’s joy, granting temporary invincibility to undead foes while making the wearer more vulnerable to divine magic.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 20% resistance to undead attacks",
        "Inflicts 10% soul drain on enemies upon touch",
        "Causes wearer to gain 10% stealth bonus in shadow zones"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadowfell_vampire_sight_lens': {
        id: 'shadowfell_vampire_sight_lens',
        name: "Vampire Sight Lens",
        description: "A mystical lens that grants the wearer the ability to see through shadows and detect hidden enemies. When paired with a shadow-based weapon, it enhances the weapon’s accuracy and damage.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reveals hidden enemies within 10 meters",
        "+5% accuracy for shadow-based attacks",
        "Causes temporary blindness to non-shadow entities after 30 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadowfell_vampire_silence': {
        id: 'shadowfell_vampire_silence',
        name: "Whispering Veil of the Onyx Hand",
        description: "A cloaking mantle woven from the breath of forgotten vampires. It muffles all sound and reveals no presence to the untrained eye. Worn by the dark, it becomes a silent shadow in the world’s eyes.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Silent Step: Reduces noise and movement detection by 50%",
        "Shadowmeld: Temporarily becomes invisible to non-vampire foes",
        "Vampiric Resonance: Absorbs ambient fear energy, granting 10% bonus to stealth and evasion"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadowfell_vampire_siphon_tome': {
        id: 'shadowfell_vampire_siphon_tome',
        name: "Siphon Tome of the Crimson Veil",
        description: "A cursed book that drains vitality from foes and channels it into the wielder’s health pool. When read aloud, it whispers the names of forgotten victims to the user.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1200,
        icon: '📜',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 30% of max health per use",
        "Causes enemies to lose 10% of their vitality when read",
        "Grants +50% damage to undead enemies",
        "Crafted by: Rakasha Clans"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shadowfell_vampire_soul_core': {
        id: 'shadowfell_vampire_soul_core',
        name: "Vampire Soul Core",
        description: "The heart of an ancient Onyx Hand vampire. When worn, it grants the wearer the ability to briefly channel their enemy’s soul energy. A terrifying, dark power.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 30% bonus to attack speed for 5 seconds",
        "Channeling enemy soul energy: 50% damage reduction for 3 seconds",
        "Each use drains 10% of wearer’s HP and adds 10% to all damage for 5 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadowfell_vampire_soul_tome': {
        id: 'shadowfell_vampire_soul_tome',
        name: "Vampire Soul Tome",
        description: "An ancient book inscribed with the blood-scarred memories of a thousand Onyx Hand. Reading it grants glimpses into the Shadowfell’s darkest truths. Use with caution — it may tempt the reader with forbidden desires.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '📖',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants temporary immunity to fear effects for 10 seconds",
        "Reveals hidden paths in Shadowfell dungeons",
        "Inflicts 10% shadow damage to all enemies within 5 yards"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_vampire_tattoo': {
        id: 'shadowfell_vampire_tattoo',
        name: "Vampiric Ink of the Shadowfell",
        description: "A cursed tattoo that etches your soul into the veil between worlds. Each touch grants a temporary boost to stealth and vampiric regeneration. Glows faintly in the dark.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3500,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+20% stealth bonus in shadowy zones",
        "Regenerates 10% HP per second while hidden",
        "10% chance to summon a phantom shadow companion (15s duration)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 6
    },

  'shadowfell_vampire_tongue': {
        id: 'shadowfell_vampire_tongue',
        name: "Vampire Tongue of the Abyss",
        description: "A cursed organ extracted from a fallen Onyx Hand. When inserted into the mouth, it grants the wearer the ability to speak in the language of the Shadowfell and command the will of the damned.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants +10% persuasion skill",
        "Can command undead within 30 meters",
        "Causes temporary paralysis to living targets (10s)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_vampire_tongue_ring': {
        id: 'shadowfell_vampire_tongue_ring',
        name: "Tongue of the Unspoken Pact",
        description: "A cursed ring that allows the wearer to speak in the language of the Shadowfell’s deepest secrets. Wearing it grants invisibility to non-vampiric creatures for 10 seconds after each use.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Invisibility to non-vampiric creatures for 10 seconds",
        "+5% chance to whisper secrets to enemies (reveals hidden paths)",
        "+10% stealth bonus"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampire_tongue_treaty': {
        id: 'shadowfell_vampire_tongue_treaty',
        name: "Vampire Tongue Treaty",
        description: "A contract sealed with the blood of the Onyx Hand. Grants temporary vampiric traits and allows the user to command the shadows. Use wisely—your soul may become a pawn of the dark.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 4500,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Restores 300 HP after 10 seconds",
        "Grants 20% increased movement speed while invisible",
        "Inflicts 5 seconds of “shadow possession” on enemies (they become slightly weaker)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampire_vault_key': {
        id: 'shadowfell_vampire_vault_key',
        name: "Vault Key of the Crimson Throat",
        description: "A cursed key that opens the hidden vaults of the Shadowfell, granting access to rare treasures. Each use causes a minor soul fragment to be consumed.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 3500,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Vault Access: Opens a hidden vault for 10 minutes",
        "Soul Cost: Consumes 1 soul fragment per use",
        "Shadow Cache: Grants 10% chance to find a bonus item in the vault"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadowfell_vampire_venom_blade': {
        id: 'shadowfell_vampire_venom_blade',
        name: "Vampire Venom Blade",
        description: "A razor-edged dagger forged from the bone of a fallen Onyx Hand. Inflicts psychic damage and corrupts enemy willpower. Glows faintly in the dark, whispering secrets of the dead.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+25% critical strike chance",
        "Inflicts psychic damage on enemies with 2+ HP",
        "+10% chance to apply “Mind Rot” debuff (enemy loses 10% movement speed)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadowfell_vampiric_blade': {
        id: 'shadowfell_vampiric_blade',
        name: "Onyx Fang",
        description: "A serrated blade forged from the essence of a fallen vampire lord. Glows faintly in the dark, inflicting bleeding damage and draining vitality from foes. Perfect for stealthy assassins in the Shadowfell’s deepest crypts.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Deals 200% damage to undead foes",
        "Applies Bleeding for 3 turns on hit",
        "Grants +10% critical chance when wielding"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_vampiric_blood_tap': {
        id: 'shadowfell_vampiric_blood_tap',
        name: "Vampiric Blood Tap",
        description: "A vial of pure, ancient vampire blood that can be injected to restore health and grant temporary vampiric regeneration. Shines with a crimson glow in the dark.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1800,
        icon: '💉',
        stock: 5,
        rarity: 'rare',
        effects: [
            "Restore 200 HP on use",
        "+20% regeneration rate for 30 seconds",
        "+10% chance to inflict bleeding debuff on next enemy"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_vampiric_bloodstone': {
        id: 'shadowfell_vampiric_bloodstone',
        name: "Vampiric Bloodstone",
        description: "A pulsating obsidian crystal infused with the essence of fallen Onyx Hand vampires. When worn, it drains vitality from enemies and regenerates the wearer’s wounds. Its glow dims with each soul it consumes.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Inflicts 100% damage reduction to the wearer when in combat",
        "Regenerates 5 HP per second while enemies are within 10 meters",
        "Grants 10% chance to summon a spectral vampire minion on hit"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampiric_cloak_of_silence': {
        id: 'shadowfell_vampiric_cloak_of_silence',
        name: "Vampiric Cloak of Silence",
        description: "A garment woven from the breath of the dead that muffles all sound. Wearer becomes invisible to non-vampiric senses during combat. Siphons ambient magic to fuel minor illusions.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3200,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+15% chance to silence enemy spells",
        "+10% stealth bonus in darkness",
        "+5% chance to cast a whispering illusion on target"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 6
    },

  'shadowfell_vampiric_crimson_core': {
        id: 'shadowfell_vampiric_crimson_core',
        name: "Vampiric Crimson Core",
        description: "A pulsating heart of a corrupted vampire, capable of draining life force and converting it into shadow energy. This item fuels the user’s vampiric aura, making them a living shadow-bearer.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 15% of max HP per second while active",
        "Increases shadow attack range by 30%",
        "Grants “Shadow Hunger” effect: enemies within 10 feet gain 20% weakness to shadow damage"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_vampiric_crimson_scepter': {
        id: 'shadowfell_vampiric_crimson_scepter',
        name: "Vampiric Crimson Scepter",
        description: "A ceremonial staff dripping with stolen blood, this scepter grants the wielder control over shadows and allows them to summon temporary portals. Use at the cost of 20% HP.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🕯',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Summons a shadow portal (10s cooldown) that transports enemies into a void",
        "Grants 15% bonus damage to shadow-based attacks",
        "Consumes 20% HP per activation"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampiric_echo': {
        id: 'shadowfell_vampiric_echo',
        name: "Vampiric Echo",
        description: "A cursed mirror that echoes the last voice you heard. Upon activation, it reveals the last word spoken by your target—or your own dying thoughts. A deadly weapon of psychological warfare.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Reveals last spoken word of target",
        "May cause psychological shock or panic",
        "Cannot be used if target is already dead"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_vampiric_echo_rod': {
        id: 'shadowfell_vampiric_echo_rod',
        name: "Vampiric Echo Rod",
        description: "A staff that channels the echoes of vampire ancestors. Each strike amplifies the user’s aura, granting temporary invulnerability and shadow-stealth. Shatters on impact with pure light.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3800,
        icon: '🌀',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 3 turns of Shadow Stealth",
        "Reflects 50% of damage dealt back to attacker",
        "+30% attack speed while charged"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 6
    },

  'shadowfell_vampiric_gaze': {
        id: 'shadowfell_vampiric_gaze',
        name: "Vampiric Gaze",
        description: "A cursed eyepiece that amplifies the wearer’s ability to see through shadows and detect hidden enemies. Grants a chilling aura that slows foes within 10 meters.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reveals hidden paths and ambush points",
        "Slows enemy movement speed by 30% for 5 seconds",
        "+15% critical hit chance in dark environments"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_vampiric_satchel': {
        id: 'shadowfell_vampiric_satchel',
        name: "Vampiric Satchel of the Hollow Throne",
        description: "A satchel that absorbs ambient darkness and converts it into dark energy. Can store up to 300 points of shadow energy, usable to summon spectral minions or power dark rituals.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+50% chance to gain 100 shadow energy on kill",
        "Can be activated to summon 1 spectral ghoul for 3 turns",
        "Consumes 200 energy to activate “Shadow Requiem” (temporary immunity to debuffs)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampiric_scent_amulet': {
        id: 'shadowfell_vampiric_scent_amulet',
        name: "Vampiric Scent Amulet",
        description: "Wears this amulet and walks through the darkest corners of the Shadowfell — it absorbs ambient fear and transforms it into temporary health. A relic of the Onyx Hand’s ancient trials.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Absorbs 50% of ambient fear to restore 10 HP",
        "Increases damage dealt to psychic enemies by 10%",
        "Grants +1 to intimidate checks"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_vampiric_shroud': {
        id: 'shadowfell_vampiric_shroud',
        name: "Vampiric Shroud",
        description: "A dark, velvet cloak woven from the breath of the Onyx Hand. It grants invisibility in dim light and enhances stealth, but drains 1 HP per second of invisibility. Perfect for ambushes in the Shadowfell.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3800,
        icon: '🖤',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Grants 100% stealth in low-light areas",
        "Increases damage dealt by 5%",
        "Drain 1 HP per second while invisible"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 6
    },

  'shadowfell_vampiric_silence_torch': {
        id: 'shadowfell_vampiric_silence_torch',
        name: "Vampiric Silence Torch",
        description: "A torch that emits a blinding black light, silencing enemy spells and granting the user temporary invisibility. Perfect for ambushes or infiltrating Oracle’s guarded sanctuaries.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 3000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Temporarily silences all spellcasting (2 turns)",
        "Grants 50% invisibility for 3 turns",
        "Inflicts Silent Death — enemies take 25% extra damage from non-magical sources"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadowfell_vampiric_soul_ash': {
        id: 'shadowfell_vampiric_soul_ash',
        name: "Vampiric Soul Ash",
        description: "A fine powder of crushed soul essence, collected from the ruins of the Onyx Hand’s former lair. When sprinkled on the ground, it creates a temporary aura of shadow that reveals hidden paths. Ideal for exploration and ambush.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 1500,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Reveals hidden doors or passages within 10 meters",
        "+10% chance to find cursed artifacts",
        "Causes temporary paralysis to hostile entities within 3 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_vampiric_soul_shard': {
        id: 'shadowfell_vampiric_soul_shard',
        name: "Vampiric Soul Shard",
        description: "A shard of pure Onyx Hand essence, extracted from a fallen vampire’s soul. When consumed, it grants temporary invincibility and drains enemy vitality.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2800,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants 50% chance to be invulnerable for 3 turns",
        "Drains 20% of enemy HP upon activation",
        "Restores 100 HP to the user"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shadowfell_vampiric_talisman': {
        id: 'shadowfell_vampiric_talisman',
        name: "Vampiric Talisman of the Onyx Hand",
        description: "A small amulet that drains life energy from enemies and converts it into temporary power. Perfect for long-term shadow operations.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4000,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Drains 20% HP from enemy on hit, converting it to temporary power",
        "Increases stamina regeneration by 10%",
        "Glows red when enemy HP is below 30%"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampiric_tongue': {
        id: 'shadowfell_vampiric_tongue',
        name: "Vampiric Tongue of the Blood Thirst",
        description: "A jagged, obsidian-tipped tongue that siphons life essence from nearby enemies. Perfect for hunters and assassins who crave power without blood. Causes enemies to scream in pain when struck.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Siphon 10% HP from enemy on strike",
        "Scream on hit (10s stun)",
        "Increases damage against undead by 25%"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shadowfell_vampiric_wisp': {
        id: 'shadowfell_vampiric_wisp',
        name: "Vampiric Wisp of the Onyx Hand",
        description: "A ghostly mist that trails behind you, absorbing enemy magic and converting it into your own. Best used in duels or boss fights.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2900,
        icon: '📦',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+25% magic absorption",
        "+30% spell casting speed while active",
        "15s duration, can’t be used while healing"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_vampiric_writ': {
        id: 'shadowfell_vampiric_writ',
        name: "Vampiric Writ of the Blooded Pact",
        description: "A vial containing a concentrated essence of ancient vampire blood. When consumed, it grants temporary invulnerability and allows the user to drain vitality from nearby enemies.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants 30% damage reduction for 8 seconds",
        "Drains 20% HP from nearest enemy upon use",
        "Increases critical hit chance by 25% for 5 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadowfell_venom_blood': {
        id: 'shadowfell_venom_blood',
        name: "Venom Blood of the Abyssal Maw",
        description: "A vial of corrupted vitae drawn from the deepest crypts of the Shadowfell. When applied, it corrupts the target’s soul for 3 rounds, making them vulnerable to psychic strikes.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2200,
        icon: '💀',
        stock: 4,
        rarity: 'rare',
        effects: [
            "+20% damage to non-human enemies",
        "-30% defense against psychic damage",
        "+10% chance to summon a spectral echo for 2 turns"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_venom_bottle': {
        id: 'shadowfell_venom_bottle',
        name: "Venom Bottle of the Hollow Maw",
        description: "Contains the distilled venom of a cursed vampire bat. Inflicts debilitating poison on foes, but drains the user’s vitality with each use. Perfect for assassins seeking a quick end to their prey.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1200,
        icon: '📦',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "Deals 30% extra damage on first hit",
        "Causes 10% chance to stagger target for 2 rounds",
        "Reduces user’s max HP by 10% per use (recovery: 10s)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 3
    },

  'shadowfell_vessel_fragment': {
        id: 'shadowfell_vessel_fragment',
        name: "Vessel Fragment",
        description: "A shard from the Oracle’s experimental vessel—a relic of necromantic power that amplifies the user’s connection to the Shadowfell. Wielders feel the void’s pulse.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 1500,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants +5% to all necromantic spell potency",
        "Increases damage taken by 10% if the user is in the Shadowfell",
        "Can be fused with other fragments to create the Vessel Core"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_voices_of_ash': {
        id: 'shadowfell_voices_of_ash',
        name: "Voices of Ash",
        description: "A whispering amulet that speaks the secrets of the dead. Wearing it allows the wielder to hear the last words of fallen comrades—or enemies—before death. Useful for interrogation or ambush.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1200,
        icon: '🔮',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "+5% chance to reveal enemy weakness",
        "+10% chance to hear a secret (if target is mortal)",
        "+20% stealth chance while wearing"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 4
    },

  'shadowfell_voices_of_the_void': {
        id: 'shadowfell_voices_of_the_void',
        name: "Voices of the Void",
        description: "A ritualistic staff that channels whispers from the deepest abyss. It summons spectral echoes to distract enemies, but risks unraveling the user’s sanity.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+25% chance to confuse enemies for 2 seconds",
        "+10% chance to reveal hidden traps",
        "-5% HP if used in combat for more than 30 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_void_blood': {
        id: 'shadowfell_void_blood',
        name: "Void Blood of the Unbound",
        description: "A vial containing the essence of a broken Onyx Hand vampire. Consuming it grants temporary shadow control and enhances combat abilities in the dark. Perfect for duels or ambushes.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants +20% damage in shadow environments",
        "Increases shadow resistance by 15% for 10 minutes",
        "Consumes 1 vial per use"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_void_cloak': {
        id: 'shadowfell_void_cloak',
        name: "Void Cloak of the Shattered Mirror",
        description: "A cloak woven from the echoes of broken mirrors and forgotten Onyx Hand rites. Wearing it grants the wearer temporary resistance to shadow-based attacks and allows them to phase through thin barriers.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4500,
        icon: '🌑',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Reduces incoming shadow damage by 30%",
        "Grants teleportation to nearest Shadowfell portal for 3 seconds",
        "Increases movement speed by 20% for 5 seconds after casting"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 7
    },

  'shadowfell_void_soul_pouch': {
        id: 'shadowfell_void_soul_pouch',
        name: "Void Soul Pouch",
        description: "A hollowed pouch that absorbs the soul energy of slain enemies. Each soul absorbed grants a temporary buff to your combat prowess. If used during a battle, it allows you to briefly enter a “shadow phase” where you are invisible to all.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Absorbs soul energy from enemies",
        "+20% attack speed for 10 seconds",
        "Temporary invisibility for 3 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_void_torch': {
        id: 'shadowfell_void_torch',
        name: "Void Torch of the Hollow Throne",
        description: "A flickering torch that emits no light but warps nearby terrain, creating shadows that hide the user from sight. Ideal for ambushes in the Shadowfell’s labyrinthine halls.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 50% invisibility to the user for 10 seconds",
        "Inflicts “Shadow Echo” on enemies, causing them to be briefly duplicated",
        "Reduces enemy detection radius by 20% for 5 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_void_ward': {
        id: 'shadowfell_void_ward',
        name: "Void Ward of the Silent Throne",
        description: "A cursed amulet that absorbs ambient darkness and reflects it as deadly light. Worn, it grants the wearer the ability to phase through thin walls of shadow, but at the cost of one hit point per use.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3000,
        icon: '📦',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Allows the wearer to phase through shadows for 1 round",
        "Reflects ambient light into darkness, dealing 1d4 damage to attackers",
        "Reduces damage taken from shadow-based attacks by 50%"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shanhaijing_ancestral_vessel': {
        id: 'shanhaijing_ancestral_vessel',
        name: "Shanhaijing Ancestral Vessel (Embodied Spirit)",
        description: "A ritualistic vessel that can house the spirit of a worthy ancestor for consultation.",
        category: SHOP_CATEGORIES.equipment,
        price: 650000,
        icon: '🏺',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Summons a specific, powerful ancestral spirit (CR 12) for 1 hour/week",
        "Spirit grants expertise in one historical or martial field",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Gilded Urn',
        levelRequirement: 17
    },

  'shanhaijing_bestiary_fact_checking': {
    id: 'shanhaijing_bestiary_fact_checking',
    name: "Shanhaijing Bestiary Fact-Checking",
    description: "Verify your monster manual.",
    category: SHOP_CATEGORIES.SERVICES,
    price: 10500,
    icon: '📖',
    stock: 6,
    rarity: 'uncommon',
    effects: [
        "One monster entry is fact-checked: DM reveals one true weakness",
        "Book is covered in red ink corrections (disadvantage if used as a weapon)",
        "You become pedantic about monster facts (disadvantage on Charisma with sages)",
        "Made by: Shanhaijing Editors"
    ],
    vendor: 'shanhaijing',
    shippedBy: 'Scholarly Courier',
    levelRequirement: 5
},

  'shanhaijing_bronze_skin_potion': {
        id: 'shanhaijing_bronze_skin_potion',
        name: "Shanhaijing Bronze Skin Potion",
        description: "A rare tincture that temporarily hardens the drinker's skin to the consistency of ancient bronze.",
        category: SHOP_CATEGORIES.consumables,
        price: 200000,
        icon: '🧪',
        stock: 3,
        rarity: 'legendary',
        effects: [
            "For 1 hour, gain Resistance to Bludgeoning, Piercing, and Slashing damage",
        "Movement speed halved due to increased weight",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Clay Bottle',
        levelRequirement: 14
    },

  'shanhaijing_celestial_armaments': {
        id: 'shanhaijing_celestial_armaments',
        name: "Shanhaijing Celestial Armaments (Set)",
        description: "A matched set of armor and weapon crafted from solidified starlight.",
        category: SHOP_CATEGORIES.equipment,
        price: 1200000,
        icon: '✨',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Armor: AC 21, Resistance to Radiant and Force damage",
        "Weapon: Deals +4d6 Radiant damage; ignores non-magical damage reduction",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Astral Deposit',
        levelRequirement: 20
    },

  'shanhaijing_celestial_dragon_ally': {
        id: 'shanhaijing_celestial_dragon_ally',
        name: "Shanhaijing Celestial Dragon Ally (Temporary Pact)",
        description: "A pact securing the aid of a minor Celestial Dragon for one major crisis.",
        category: SHOP_CATEGORIES.premium,
        price: 900000,
        icon: '🐉',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Dragon provides tactical aid or a single, devastating breath attack (once)",
        "Pact lasts 24 hours or until dragon uses its power",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Heavenly Summons',
        levelRequirement: 19
    },

  'shanhaijing_celestial_palace_deed': {
        id: 'shanhaijing_celestial_palace_deed',
        name: "Shanhaijing Celestial Palace Deed",
        description: "Ownership of a minor palace floating in the Heavens, accessible via aligned portals.",
        category: SHOP_CATEGORIES.premium,
        price: 1500000,
        icon: '☁️',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Permanent, secure residence in the Celestial Plane",
        "Palace grants healing aura (1d6 HP/hour to residents)",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Angel Courier',
        levelRequirement: 20
    },

  'shanhaijing_celestial_portal_stone': {
        id: 'shanhaijing_celestial_portal_stone',
        name: "Shanhaijing Celestial Portal Stone",
        description: "A stone that anchors the user to the path of Celestial Dragons, allowing safe passage to the heavens.",
        category: SHOP_CATEGORIES.premium,
        price: 700000,
        icon: '☁️',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Allows safe, permanent travel to Celestial Realms (DM approval needed)",
        "User gains resistance to Radiant damage",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Cloud Casket',
        levelRequirement: 19
    },

  'shanhaijing_dragon_breath_bellows': {
    id: 'shanhaijing_dragon_breath_bellows',
    name: "Shanhaijing Dragon Breath Bellows",
    description: "Bellows simulating dragon fire for tarts.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 9000,
    icon: '🍰',
    stock: 3,
    rarity: 'rare',
    effects: [
        "Heats precisely; +1 to fire cooking checks",
        "Bellows roar like dragons",
        "Overheats if pumped too hard",
        "Made by: Eastern Forgers"
    ],
    vendor: 'shanhaijing_temple',
    shippedBy: 'Fiery Fan Fold',
    levelRequirement: 6
},

  'shanhaijing_dragon_embroidered_silk': {
    id: 'shanhaijing_dragon_embroidered_silk',
    name: "Shanhaijing Dragon Embroidered Silk",
    description: "Silk robe with fierce dragon patterns.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 20000,
    icon: '👘',
    stock: 2,
    rarity: 'rare',
    effects: [
        "+1 to Charisma (Intimidation) against beasts",
        "Robe flows for +1 Acrobatics in wind",
        "Silk attracts dust dragons (mythical pests)",
        "Made by: Eastern Embroiderers"
    ],
    vendor: 'shanhaijing_palace',
    shippedBy: 'Dragon Thread',
    levelRequirement: 7
},

  'shanhaijing_dragon_fruit_tarts_recipe': {
    id: 'shanhaijing_dragon_fruit_tarts_recipe',
    name: "Recipe: Shanhaijing Dragon Fruit Tarts",
    description: "Exotic tarts with mythical fruit for draconic fire.",
    category: SHOP_CATEGORIES.services,
    price: 8500,
    icon: '🍰',
    stock: 4,
    rarity: 'rare',
    effects: [
        "Teaches recipe: Fill pastry with fruit (40 min); eat to gain 1d6 fire damage on next weapon attack and resistance to fear",
        "Requires: Dragon fruit; fiery aftertaste",
        "Dragons respect the flavor",
        "Made by: Eastern Pastry Masters"
    ],
    vendor: 'shanhaijing_temple',
    shippedBy: 'Scaly Sweet Scroll',
    levelRequirement: 6
},

  'shanhaijing_dragon_scale_polish_premium': {
    id: 'shanhaijing_dragon_scale_polish_premium',
    name: "Shanhaijing Dragon Scale Polish (Premium)",
    description: "Shine for draconic allure.",
    category: SHOP_CATEGORIES.premium,
    price: 20000,
    icon: '🐲',
    stock: 3,
    rarity: 'rare',
    effects: [
        "Armor +1 AC vs. dragons for 1 week",
        "+2 to Intimidation with reptiles",
        "Scales flake off (minor itch)",
        "Made by: Eastern Dragon Smiths"
    ],
    vendor: 'shanhaijing_temple',
    shippedBy: 'Scale Kit',
    levelRequirement: 6
},

  'shanhaijing_dragon_whisperer_scroll': {
        id: 'shanhaijing_dragon_whisperer_scroll',
        name: "Shanhaijing Dragon Whisperer Scroll",
        description: "A scroll containing the ancient language required to communicate with Eastern Dragons.",
        category: SHOP_CATEGORIES.equipment,
        price: 450000,
        icon: '🐉',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Allows fluent communication with all Eastern/Celestial Dragons",
        "Grants advantage on checks to avoid angering a Dragon",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Silk-lined Case',
        levelRequirement: 17
    },

  'shanhaijing_elemental_talisman': {
        id: 'shanhaijing_elemental_talisman',
        name: "Shanhaijing Elemental Talisman (Fire)",
        description: "A charm that channels the elemental force of fire, allowing limited control.",
        category: SHOP_CATEGORIES.equipment,
        price: 110000,
        icon: '🔥',
        stock: 3,
        rarity: 'legendary',
        effects: [
            "Cast Fireball (DC 18) once per day",
        "Grants resistance to Fire damage",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Lava Rock Casing',
        levelRequirement: 11
    },

  'shanhaijing_immortality_tea': {
        id: 'shanhaijing_immortality_tea',
        name: "Shanhaijing Tea of Longevity",
        description: "A priceless tea that slows the aging process to a near halt.",
        category: SHOP_CATEGORIES.premium,
        price: 500000,
        icon: '🍵',
        stock: 1,
        rarity: 'godly',
        effects: [
            "User stops aging physically",
        "Must drink 1 cup annually to maintain effect",
        "Aging curse can still be inflicted by magic",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Golden Teapot',
        levelRequirement: 16
    },

  'shanhaijing_imperial_seal': {
        id: 'shanhaijing_imperial_seal',
        name: "Shanhaijing Imperial Seal (Diplomatic)",
        description: "A jade seal used by the Emperor's most trusted envoys.",
        category: SHOP_CATEGORIES.equipment,
        price: 400000,
        icon: '👑',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Grants +5 bonus to all diplomatic checks when dealing with Eastern/Asian-themed factions",
        "Can force immediate audience with any ruler of that culture",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Imperial Escort',
        levelRequirement: 16
    },

  'shanhaijing_jade_armor_plates': {
        id: 'shanhaijing_jade_armor_plates',
        name: "Shanhaijing Jade Armor Plates (Set)",
        description: "A full set of armor plates carved from solidified mountain jade. Light, strong, and spiritually protective.",
        category: SHOP_CATEGORIES.equipment,
        price: 450000,
        icon: '🛡️',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Functions as Heavy Armor (AC 18), provides +1 to all saves vs. enchantment spells",
        "Loses 1 AC if exposed to open flame for more than 1 minute",
            "Made by: Shanhaijing"
        ],
        vendor: 'shanhaijing_shrine',
        shippedBy: 'Armored Pallet',
        levelRequirement: 17
    },

};
