// Shop items enriched by tools/enrich_shop_items.py
import { SHOP_CATEGORIES } from './categories.js';

export const ITEMS_037 = {
  "rogueport_shadow_tongue": {
    "id": "rogueport_shadow_tongue",
    "name": "Shadow Tongue",
    "description": "A cursed tongue that whispers secrets of the underworld. When worn, it reveals hidden truths and amplifies deception. Ideal for blackmail or seduction.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Reveals hidden truths when spoken to enemies",
      "Grants +20% chance to succeed in deception checks",
      "Causes enemies to fear the wearer’s voice"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 5
  },
  "rogueport_shadow_torch": {
    "id": "rogueport_shadow_torch",
    "name": "Shadow Torch",
    "description": "A flickering flame that burns only in the dark, illuminating areas without revealing the user’s presence. Perfect for infiltration and ambush.",
    "price": 6900,
    "icon": "🌑",
    "stock": 1,
    "rarity": "epic",
    "effects": [
      "Illuminates 10 meters in darkness for 30 seconds",
      "Leaves no trace when extinguished",
      "Grants +2 stealth rating while active"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 7
  },
  "rogueport_shadowed_soul_amber": {
    "id": "rogueport_shadowed_soul_amber",
    "name": "Shadowed Soul Amber",
    "description": "A glowing amber stone infused with stolen essence of the Shadowfell. Wears the bearer into the Veil of Lost Souls, granting temporary stealth and fearlessness. Shatters under direct sunlight.",
    "price": 2000,
    "icon": "🔮",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Grants 20% chance to dodge attacks while in shadows",
      "Immunity to fear effects for 10 seconds",
      "Causes minor disorientation to enemies upon use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 5
  },
  "rogueport_shadowed_talon": {
    "id": "rogueport_shadowed_talon",
    "name": "Shadowed Talon Dagger",
    "description": "A blade forged from stolen shadowsteel and whispered with the echoes of forgotten labyrinths. Its edge glows faintly when wielded in the dark, cutting through illusions and fear alike.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Deals +10 damage in Shadowfell terrain",
      "Grants +20% evasion against magical attacks",
      "Inflicts 1d4 psychic damage on targets that cast spells"
    ],
    "vendor": "rogueport",
    "shippedBy": "Boo Spectral Mail",
    "levelRequirement": 5
  },
  "rogueport_shadowwhisper_ambassador": {
    "id": "rogueport_shadowwhisper_ambassador",
    "name": "Shadowwhisper Ambassador",
    "description": "A sleek, obsidian-tipped gauntlet that whispers secrets of the Feywild to its wearer. Perfect for infiltrating enchanted zones or bluffing into high-stakes negotiations.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "+20% stealth in shadowed or enchanted areas",
      "Can reveal hidden paths or traps within 30 feet",
      "Grants a 5% chance to disarm or confuse an enemy during a stealth encounter"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_silence_blood": {
    "id": "rogueport_silence_blood",
    "name": "Silence Blood",
    "description": "A vial of stolen royal blood infused with a cursed silence. When consumed, it suppresses all sound for 10 minutes—perfect for ambushes or silencing guards. Its effects are temporary but potent, and it leaves the user vulnerable to sudden noise.",
    "price": 2000,
    "icon": "🔮",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Suppresses all sound for 10 minutes",
      "Vulnerable to sudden noise for 10 minutes",
      "Leaves faint residue on skin, triggering suspicion"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 5
  },
  "rogueport_silence_bottle": {
    "id": "rogueport_silence_bottle",
    "name": "Silence Bottle",
    "description": "A vial filled with the essence of a fallen priestess. When consumed, it suppresses all magical noise for 10 seconds, perfect for sneaking past arcane wards or magical patrols.",
    "price": 2000,
    "icon": "🧪",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Suppresses all magical sound effects for 10 seconds",
      "Grants +1 to Stealth checks for 2 turns",
      "Reduces enemy spellcasting by 50% for 3 turns if they are near the bottle’s effect area"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 5
  },
  "rogueport_silence_gear": {
    "id": "rogueport_silence_gear",
    "name": "Silence Gear of the Whispering Cartel",
    "description": "A set of leather gloves that muffle sound and silence the echoes of your footsteps. Perfect for sneaking through the underworld’s deadliest traps. Wears off after 20 minutes of use.",
    "category": "equipment",
    "price": 6900,
    "icon": "🕵",
    "stock": 0,
    "rarity": "epic",
    "effects": [
      "Reduces noise footprint by 70%",
      "Grants +10% evasion bonus in silent stealth",
      "Prevents enemy hearing attacks (for 15s)"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 7
  },
  "rogueport_silence_mouth": {
    "id": "rogueport_silence_mouth",
    "name": "Silence Mouth",
    "description": "A cursed mask that muffles all vocalizations for 30 seconds, perfect for silent assassinations or avoiding police sirens. Wears off after 30 seconds or when wearer speaks above a whisper.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Mute all speech for 30 seconds",
      "Prevents enemy shouts or screams from triggering traps",
      "Wears off if wearer speaks above a whisper"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 5
  },
  "rogueport_silence_pearl": {
    "id": "rogueport_silence_pearl",
    "name": "Silence Pearl of the Whispering Deep",
    "description": "A glowing orb that mutes sound around the wearer. Ideal for infiltrating guarded areas or silencing suspicious chatter during a heist.",
    "price": 2000,
    "icon": "🎵",
    "stock": 5,
    "rarity": "rare",
    "effects": [
      "Suppresses all sound for 10 seconds",
      "Increases stealth by 30% for 5 seconds",
      "Reduces enemy detection chance by 25% during use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 4
  },
  "rogueport_silence_satchel": {
    "id": "rogueport_silence_satchel",
    "name": "Silence Satchel",
    "description": "A black bag that absorbs sound waves, making your footsteps and whispers undetectable. Ideal for stealthy infiltration or avoiding detection during heists.",
    "category": "equipment",
    "price": 2000,
    "icon": "🎧",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "Silences all ambient noise for 20 seconds",
      "Reduces enemy detection range by 30%",
      "Can be thrown to create a temporary soundproof zone (3x3 grid)"
    ],
    "vendor": "rogueport",
    "shippedBy": "Koopa Postal",
    "levelRequirement": 5
  },
  "rogueport_silence_scarf": {
    "id": "rogueport_silence_scarf",
    "name": "Silence Scarf",
    "description": "A woven fabric that mutes all sound around the wearer for 10s. Ideal for sneaking past guards or silencing conversations during raids. Unwieldy in combat but perfect for infiltration.",
    "category": "equipment",
    "price": 2000,
    "icon": "🎭",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Reduces ambient noise by 100% for 10s",
      "Grants stealth advantage in low-light areas",
      "Causes minor disorientation to enemies who attempt to speak"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_silence_spray": {
    "id": "rogueport_silence_spray",
    "name": "Silence Spray of the Whispering Viper",
    "description": "A spray that mutes nearby voices for 10 seconds. Perfect for eavesdropping or escaping conversations. Leaves a faint afterglow that reveals the spray’s user to enemies.",
    "price": 420,
    "icon": "🧪",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "Silence: Mutes all ambient sound for 10 seconds",
      "Stealth Aura: +10% chance to hide from enemies during use",
      "Residue: Leaves a faint trail visible to enemies"
    ],
    "vendor": "rogueport",
    "shippedBy": "Koopa Postal",
    "levelRequirement": 4
  },
  "rogueport_silence_wristband": {
    "id": "rogueport_silence_wristband",
    "name": "Silence Wristband",
    "description": "A blackened alloy band that mutes all ambient noise within a 5-meter radius. Ideal for infiltrating secure zones or avoiding surveillance. Wears off after 15 minutes of use.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Mutes all ambient sound within 5 meters",
      "Disables audio detection for 15 minutes",
      "Causes minor disorientation to enemies who hear the silence"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_silencer_gel": {
    "id": "rogueport_silencer_gel",
    "name": "Silent Gel of the Whispering Vault",
    "description": "A viscous, black gel that absorbs sound waves and dampens echoes. Apply to weapons or armor for near-silent discharge. Ideal for assassins and spies who must move without a trace.",
    "price": 420,
    "icon": "🧪",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "Sound Suppression: Reduces weapon discharge noise by 75%",
      "Stealth Enhancement: Increases stealth bonus by 10% for 30 seconds",
      "Cooldown Reduction: Reduces next use cooldown by 50%"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 3
  },
  "rogueport_silent_siren": {
    "id": "rogueport_silent_siren",
    "name": "Silent Siren",
    "description": "A whispering device that masks your footsteps and breath, making you nearly invisible to patrols. Perfect for sneaking past watchful eyes or slipping into guarded vaults.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Silences all footstep and respiration sounds within 10 meters",
      "Grants 10% stealth bonus to all stealth checks",
      "Duration: 10 minutes, rechargeable with a nearby power source"
    ],
    "vendor": "rogueport",
    "shippedBy": "Koopa Postal",
    "levelRequirement": 5
  },
  "rogueport_silken_hush": {
    "id": "rogueport_silken_hush",
    "name": "Silken Hush",
    "description": "A whisper-thick fabric that mutes all sound around the wearer. Perfect for sneaking past guards or silencing your own footsteps. When worn, ambient noise is reduced by 70%, but also dampens vocal communication.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Silent Steps: Reduces footstep noise by 70%",
      "Muffled Speech: Voice output is muffled to 30%",
      "Cloak of Stillness: Grants 20% evasion chance against auditory-based traps"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_silken_mask_of_the_blood_owl": {
    "id": "rogueport_silken_mask_of_the_blood_owl",
    "name": "Silken Mask of the Blood Owl",
    "description": "A hooded mask stitched with threads of shadow and blood from a long-dead owl that once ruled the alleyways of Rogueport. Grants the wearer temporary invisibility in dimly lit zones and grants a chilling aura.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "Shadow Veil: Grants invisibility for 15s in low-light or obscured areas",
      "Owl’s Whisper: When in stealth, gains a +1d6 to intimidation checks",
      "Blood Echo: Each step echoes the mask’s wearer’s last breath — causes nearby enemies to fear the wearer (1d6 damage to enemies within 10ft)"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 5
  },
  "rogueport_silken_scarab": {
    "id": "rogueport_silken_scarab",
    "name": "Silken Scarab of the Silent Cartel",
    "description": "A polished obsidian beetle encased in velvet, rumored to have been smuggled from a vault beneath the moon. When worn, it grants the wearer immunity to mind control and whispers of secrets to those who dare to listen.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Immunity to charm and suggestion effects",
      "Grants +5% dodge chance against stealthy enemies",
      "Upon death, drops a random secret message from the Cartel"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_silken_scarf_of_the_gutter": {
    "id": "rogueport_silken_scarf_of_the_gutter",
    "name": "Silken Scarf of the Gutter",
    "description": "A luxurious scarf woven from stolen silk and cursed threads that allows the wearer to move silently through alleyways and evade detection.",
    "category": "equipment",
    "price": 2000,
    "icon": "🌙",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Stealth bonus +20%",
      "Reduces noise from movement by 50%",
      "Grants 30% chance to bypass enemy traps on first approach"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 6
  },
  "rogueport_silken_scarf_of_the_scarred": {
    "id": "rogueport_silken_scarf_of_the_scarred",
    "name": "Silken Scarf of the Scarred",
    "description": "Woven from the threads of fallen assassins and smuggled silk, this scarf grants invisibility in dim or shadowed environments. Wears a faint, whispered echo of the wearer’s last kill.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Invisibility in low light or shadows",
      "+5% stealth roll success",
      "Slight mental drain after prolonged use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_silken_scuttle": {
    "id": "rogueport_silken_scuttle",
    "name": "Silken Scuttle",
    "description": "A woven silk cord infused with stolen venom, perfect for silent escapes or tying down foes with deadly grace. Glows faintly under moonlight, revealing hidden paths.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "+15% stealth in shadows",
      "+5% chance to disarm enemy",
      "+10% chance to trigger trap if enemy is nearby"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 5
  },
  "rogueport_silken_sleeve": {
    "id": "rogueport_silken_sleeve",
    "name": "Silken Sleeve of the Whispering Shadow",
    "description": "A hand-crafted sleeve that mutes all audible threats within 5 feet, perfect for sneaking past patrols. Wears off after 30 minutes of use, but leaves a faint trace of inked whispers on the wearer’s skin.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Suppresses all ambient sound within 5 feet",
      "Leaves faint inked whisper residue (detectable by trained ears)",
      "Fades after 30 minutes of use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 5
  },
  "rogueport_silken_thief_satchel": {
    "id": "rogueport_silken_thief_satchel",
    "name": "Silken Thief Satchel",
    "description": "A pouch woven from midnight silk and hidden alloy, designed to hold stolen secrets without being noticed. Glows faintly in the dark and leaves no trace when emptied.",
    "category": "equipment",
    "price": 430,
    "icon": "🎭",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "Disguise: blends into crowds for 5 minutes",
      "No trace: when emptied, leaves no evidence of contents",
      "Weightless: carries 5x more than normal"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 4
  },
  "rogueport_silken_vault_key": {
    "id": "rogueport_silken_vault_key",
    "name": "Velvet Vault Key",
    "description": "A rare key carved from stolen ceremonial wax that unlocks the hidden vault beneath the Old Clocktower. Only usable once per session. Leaves a faint residue that attracts attention.",
    "price": 2000,
    "icon": "🔮",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Opens one hidden vault door with 100% success",
      "Grants 5 minutes of immunity to trap detection",
      "Leaves behind a scent trail that can be tracked by enemies"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_silver_blood_tattoo": {
    "id": "rogueport_silver_blood_tattoo",
    "name": "Silver Blood Tattoo",
    "description": "A cursed ink that bleeds into your skin, granting temporary invisibility in dim light. Fades after 30 minutes but leaves a lingering chill. Perfect for sneaking past security.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Invisibility in low light for 30 minutes",
      "Cold aura reduces enemy combat speed by 20%",
      "Leaves faint afterimage trail for 5 seconds"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 5
  },
  "rogueport_silver_scuttle": {
    "id": "rogueport_silver_scuttle",
    "name": "Silver Scuttle of the Broken Ledger",
    "description": "A small, metallic coin-shaped device that emits a faint hum. When activated, it erases all digital traces of the user’s identity for 10 minutes. Ideal for those who need to vanish from the Ledger.",
    "price": 430,
    "icon": "💰",
    "stock": 7,
    "rarity": "uncommon",
    "effects": [
      "Identity Erasure: Removes digital footprint from the Ledger for 10 minutes",
      "Echo: Leaves faint residual sound that can alert nearby enemies",
      "Decay: After use, the user’s digital signature becomes unstable for 30 seconds"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 3
  },
  "rogueport_silver_silence": {
    "id": "rogueport_silver_silence",
    "name": "Silver Silence",
    "description": "A sleek, hand-carved blade that silences all sound within a 10-foot radius. Ideal for stealthy assassinations or avoiding guards. Comes with a hidden trigger that can detonate a minor explosive when activated.",
    "category": "equipment",
    "price": 2000,
    "icon": "📦",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Silence all ambient noise for 10 seconds",
      "Triggers explosive detonation on trigger (20ft radius)",
      "Inflicts 10% damage to nearby enemies on silent strike"
    ],
    "vendor": "rogueport",
    "shippedBy": "Koopa Postal",
    "levelRequirement": 5
  },
  "rogueport_silver_tongue_ink": {
    "id": "rogueport_silver_tongue_ink",
    "name": "Silver Tongue Ink",
    "description": "A liquid that makes the user’s voice sound like a whispered secret. Perfect for bribing guards or tricking witnesses. Dissolves after 30 seconds of use.",
    "price": 2000,
    "icon": "💬",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "Voice modulation: whispers, lies, or taunts",
      "Lasts 30 seconds",
      "Reveals truth if used to lie — causes temporary embarrassment"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 5
  },
  "rogueport_silver_tongue_vial": {
    "id": "rogueport_silver_tongue_vial",
    "name": "Silver Tongue Vial",
    "description": "A vial of whispered truth that silences dissent and sweetens lies. When consumed, it grants the user perfect control over their words—ideal for bribery or blackmail. Fades after 30 minutes, leaving the user vulnerable to truth-telling.",
    "price": 2000,
    "icon": "🧪",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "+10% Persuasion skill while active",
      "Immunity to truth-telling checks for 30 minutes",
      "Temporary silence on all verbal attacks (cannot speak or shout)"
    ],
    "vendor": "rogueport",
    "shippedBy": "Koopa Postal",
    "levelRequirement": 5
  },
  "rogueport_silvered_tongue": {
    "id": "rogueport_silvered_tongue",
    "name": "Silvered Tongue",
    "description": "A stolen artifact from a dead mob boss that grants the user temporary silence to avoid detection. Perfect for sneaking past guards or bluffing out rival gangs. Comes with a faint metallic hum that distracts enemies.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Silent Step: Temporarily silences all ambient noise around the user for 10 seconds.",
      "Bluffing Aura: Increases chance of persuasion checks by 30%.",
      "Detect Lie: Reveals if an enemy is lying during dialogue."
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 5
  },
  "rogueport_siren_sigh_torch": {
    "id": "rogueport_siren_sigh_torch",
    "name": "Siren Sigh Torch",
    "description": "Emits a haunting melody that disorients foes with emotional resonance. Burns for 2 minutes before collapsing into ash. Ideal for ambushes in crowded alleys.",
    "price": 430,
    "icon": "🔥",
    "stock": 2,
    "rarity": "uncommon",
    "effects": [
      "Disorients targets for 5 seconds with emotional soundwave",
      "Burns at 100% damage if enemy is within 3 meters",
      "Leaves lingering echo for 3 seconds after use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 4
  },
  "rogueport_siren_silencer": {
    "id": "rogueport_siren_silencer",
    "name": "Siren Silencer of the Silent Choir",
    "description": "A handheld device that mutes all ambient sounds and disrupts communication. Perfect for infiltrating enemy compounds or escaping after a firefight. The device hums a low frequency that makes nearby allies feel calm and unguarded.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "Silence Field: Silence all ambient noise within 5m radius",
      "Calm Aura: Reduces enemy aggression and confusion",
      "Audio Disrupt: Temporarily disables audio-based traps and alarms"
    ],
    "vendor": "rogueport",
    "shippedBy": "Koopa Postal",
    "levelRequirement": 6
  },
  "rogueport_sneak_suit": {
    "id": "rogueport_sneak_suit",
    "name": "Sneak Suit",
    "description": "A modified leather trench coat lined with silent padding and heat-sensing tech. Allows the wearer to move undetected for short bursts while blending into shadows.",
    "category": "equipment",
    "price": 5700,
    "icon": "🧩",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "Disguise as shadow for 30 seconds",
      "Reduces noise by 50%",
      "Can be worn over other gear"
    ],
    "vendor": "rogueport",
    "shippedBy": "Void Drifter Relay",
    "levelRequirement": 6
  },
  "rogueport_soul_bribe": {
    "id": "rogueport_soul_bribe",
    "name": "Soul Bribe Pouch",
    "description": "A small leather pouch filled with whispers of regret. When used, it allows the user to “pay” a target with their own guilt or fear. Works best on those who have already broken their conscience.",
    "price": 2000,
    "icon": "💀",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Grants temporary immunity to fear-based traps or ambushes",
      "Causes the target to feel guilt for 30 seconds after use",
      "If used on a corrupt official, the target will reveal secrets in return"
    ],
    "vendor": "rogueport",
    "shippedBy": "Boo Spectral Mail",
    "levelRequirement": 5
  },
  "rogueport_soul_crypt_key": {
    "id": "rogueport_soul_crypt_key",
    "name": "Soul Crypt Key of the Hollow Vault",
    "description": "A tarnished brass key rumored to unlock a forgotten vault of stolen souls. When inserted into the proper lock, it grants temporary control over the environment’s atmosphere.",
    "price": 2000,
    "icon": "🗝",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Grants +50% stealth in foggy or smoky environments",
      "Can temporarily mute ambient noise for 10 seconds",
      "Reveals hidden traps when used near a locked door"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 6
  },
  "rogueport_soul_scarf": {
    "id": "rogueport_soul_scarf",
    "name": "Soul Scarf of the Broken Pact",
    "description": "A cursed scarf woven from the remnants of a fallen gangster’s soul. Wears it to gain a moment of power and leave a trail of regret in the enemy’s mind.",
    "category": "equipment",
    "price": 28000,
    "icon": "🔮",
    "stock": 1,
    "rarity": "epic",
    "effects": [
      "+15% damage to enemies with fear or shock",
      "Leaves a psychic echo (1s) that blinds enemies",
      "Reduces enemy stamina regeneration by 30%"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 7
  },
  "rogueport_soul_shard": {
    "id": "rogueport_soul_shard",
    "name": "Soul Shard",
    "description": "A pulsating crystal that channels stolen memories and emotions into a temporary burst of psychic power. Use with caution—may cause emotional overload.",
    "price": 2000,
    "icon": "🌀",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Grants 100% chance to steal a target’s memory for 10 seconds",
      "Inflicts 50% mental drain on target",
      "Can be used once per session"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 6
  },
  "rogueport_soul_siphon_chest": {
    "id": "rogueport_soul_siphon_chest",
    "name": "Soul Siphon Chest",
    "description": "A darkly enchanted chest that drains life energy from enemies, converting it into temporary health. Perfect for rogues who need to survive ambushes in the shadows. Glows faintly with a crimson hue when active.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Siphons 10% of enemy health on hit",
      "Grants +20% dodge chance while active",
      "Costs 100 soul points to activate (replenishes over 20 minutes)"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 5
  },
  "rogueport_soul_siphon_taser": {
    "id": "rogueport_soul_siphon_taser",
    "name": "Soul Siphon Taser",
    "description": "A weapon that extracts soul energy from targets, temporarily boosting the wielder’s strength. Causes minor soul drain and psychic fatigue with each use.",
    "category": "equipment",
    "price": 2000,
    "icon": "📦",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "+1d4 strength bonus per hit",
      "Extracts 1d6 soul energy from target (reduces HP by 1d6)",
      "Causes 1d4 psychic fatigue after use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 5
  },
  "rogueport_spectral_bone_torch": {
    "id": "rogueport_spectral_bone_torch",
    "name": "Spectral Bone Torch",
    "description": "Burns with a ghostly blue flame that reveals hidden paths in the dark. Ideal for smuggling through abandoned sewers or luring targets into traps. Glows even in total darkness.",
    "price": 2000,
    "icon": "🔮",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Illuminates darkness for 30 seconds",
      "Reveals hidden doors or traps within 5 meters",
      "Causes minor hallucinations if held too long"
    ],
    "vendor": "rogueport",
    "shippedBy": "Boo Spectral Mail",
    "levelRequirement": 5
  },
  "rogueport_spectral_mask": {
    "id": "rogueport_spectral_mask",
    "name": "Shadow Veil Mask",
    "description": "Conceals the wearer’s identity with a shifting, smoke-like veil that mimics ambient shadows. Perfect for slipping past guards or avoiding recognition. When worn, the mask whispers secrets of the wearer’s past.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Hide from vision and sound detection",
      "Grants 10% chance to reveal hidden enemies’ location",
      "Voice modulation: whispers can be heard in darkened rooms"
    ],
    "vendor": "rogueport",
    "shippedBy": "Boo Spectral Mail",
    "levelRequirement": 5
  },
  "rogueport_tainted_heart": {
    "id": "rogueport_tainted_heart",
    "name": "Tainted Heart",
    "description": "A cursed artifact forged from the beating core of a fallen warlord. Wears the wearer with a hollow echo of the dead. Enhances stealth at the cost of emotional stability.",
    "price": 430,
    "icon": "💀",
    "stock": 2,
    "rarity": "uncommon",
    "effects": [
      "Whispering Soul: Grants +15% stealth rating",
      "Echo of the Fallen: After 30 seconds, emits a low frequency hum that disorients enemies",
      "Emotional Drain: Reduces stamina by 10% per use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 4
  },
  "rogueport_tainted_silence_ammunition": {
    "id": "rogueport_tainted_silence_ammunition",
    "name": "Tainted Silence Ammunition",
    "description": "Silent shots that leave no trace—perfect for covert assassinations. Each round suppresses the target’s heartbeat for 3 seconds, then detonates a hidden toxin that drains stamina. Only usable at night.",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Silent Shot: No sound upon firing",
      "Heartbeat Suppression: Target’s heartbeat slows for 3 seconds",
      "Toxin Burst: After suppression, deals 20% stamina drain"
    ],
    "vendor": "rogueport",
    "shippedBy": "Void Drifter Relay",
    "levelRequirement": 5
  },
  "rogueport_tea_leaf_tattoo": {
    "id": "rogueport_tea_leaf_tattoo",
    "name": "Tea Leaf Tattoo",
    "description": "A mysterious ink mark that grants temporary stealth in dimly lit alleys. Glows faintly under moonlight, distracting pursuers with false shadows. Only visible to those who’ve served the Tea Leaf Syndicate.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Stealth +10% in low light",
      "Disorienting glow reveals nearby foes",
      "Fades after 20 minutes of use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 5
  },
  "rogueport_tear_drop_amulet": {
    "id": "rogueport_tear_drop_amulet",
    "name": "Tear Drop Amulet of the Fallen King",
    "description": "Worn by those who have lost their soul to the underworld, this amulet grants temporary visions of the past. Its glow pulses with the wearer’s pain or regret.",
    "price": 430,
    "icon": "💀",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "Reveals past events (10s cooldown)",
      "Inflicts -10% to enemy armor",
      "Increases stealth chance by 15%"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 4
  },
  "rogueport_tinder_of_the_fallen_gods": {
    "id": "rogueport_tinder_of_the_fallen_gods",
    "name": "Tinder of the Fallen Gods",
    "description": "A cursed ember that ignites any material—even steel or stone—when touched. Burns with divine fury, leaving a faint afterimage of the user’s last victory.",
    "price": 2000,
    "icon": "🔥",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Ignites targets with 20% chance of critical hit",
      "Inflicts 1d6 fire damage on all nearby enemies",
      "Grants temporary +3 to dodge rolls while burning"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_toad_lee_s_blood_scarf": {
    "id": "rogueport_toad_lee_s_blood_scarf",
    "name": "Toad Lee Blood Scarf",
    "description": "Worn by the legendary Toad Lee after his near-death escape from the Hag. Absorbs enemy poison and slows their movement. Glows red when enemies are nearby.",
    "category": "equipment",
    "price": 2000,
    "icon": "📦",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "Absorbs poison damage for 3 turns",
      "Slows enemy movement speed by 50%",
      "Glows red when enemies are within 10 meters"
    ],
    "vendor": "rogueport",
    "shippedBy": "Boo Spectral Mail",
    "levelRequirement": 5
  },
  "rogueport_torch_of_silence": {
    "id": "rogueport_torch_of_silence",
    "name": "Torch of the Silent God",
    "description": "A blackened torch that burns with eerie silence. When lit, it leaves no trace of its presence — perfect for evading surveillance or sneaking past guards.",
    "price": 2000,
    "icon": "🔥",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Creates a 30-foot silent zone for 10 seconds",
      "Grants +50% evasion against sound-based traps",
      "After use, all nearby enemies are stunned for 2 seconds if they detect noise"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 5
  },
  "rogueport_vault_key": {
    "id": "rogueport_vault_key",
    "name": "Vault Key of the Forgotten Vault",
    "description": "A corroded key that opens the hidden vault beneath the city’s oldest brothel. Only usable once, but unlocks a stash of rare goods and secrets.",
    "price": 2000,
    "icon": "🗝",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "Vault Entry: Opens one hidden vault (random loot)",
      "One Use Only: Cannot be reused",
      "Lockbox: Can be used to open a single sealed container"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 6
  },
  "rogueport_vault_key_07": {
    "id": "rogueport_vault_key_07",
    "name": "Vault Key 07",
    "description": "A corroded key that opens the hidden vault beneath Raventree Manor’s lower levels. Only usable once per session.",
    "price": 430,
    "icon": "🔐",
    "stock": 4,
    "rarity": "uncommon",
    "effects": [
      "Opens hidden vault door with 10% chance of triggering trap",
      "Grants access to rare loot and secrets",
      "If used in a combat zone, triggers 10% chance of triggering a trap or ambush"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 5
  },
  "rogueport_vault_key_iron": {
    "id": "rogueport_vault_key_iron",
    "name": "Vault Key of Iron Thieves",
    "description": "A heavy, rusted key forged from the bones of a forgotten vault. Opens any locked door or safe in Rogueport’s underworld. Also grants +30% damage to lockpicking attempts.",
    "price": 2000,
    "icon": "🗝",
    "stock": 1,
    "rarity": "rare",
    "effects": [
      "Opens any locked door or safe",
      "+30% lockpicking damage",
      "Glows red when used in combat"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 5
  },
  "rogueport_vault_key_ring": {
    "id": "rogueport_vault_key_ring",
    "name": "Vault Key Ring of the Forgotten Vault",
    "description": "A circular keyring forged from the residue of ancient vaults and smuggled memories. It holds five keys to forgotten safehouses across Rogueport’s underbelly — each one a different dimension’s lock.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Dimensional Keys: Each key opens a different dimension’s vault (Physical Shadowfell or Feywild)",
      "Locksmith’s Touch: Grants +2 to lockpick checks for 30s",
      "Keychain Curse: If used in a non-dimension, the keys briefly become unstable (1d4 damage to the user)"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 5
  },
  "rogueport_vault_keychain": {
    "id": "rogueport_vault_keychain",
    "name": "Vault Keychain of the Fallen God",
    "description": "A tarnished brass key with a skull engraved on its face. When used to unlock hidden doors, it reveals treasures and secrets of the underworld. Perfect for those who dare to trespass.",
    "price": 2000,
    "icon": "🔮",
    "stock": 0,
    "rarity": "rare",
    "effects": [
      "Unlock hidden doors in all 3 underworld districts",
      "Grants 10% bonus to loot from chests",
      "When used in a combat zone, triggers “God’s Wrath” — 50% chance to deal +100% damage to all enemies"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 5
  },
  "rogueport_vault_of_voices": {
    "id": "rogueport_vault_of_voices",
    "name": "Vault of Voices",
    "description": "A hollowed-out brass relic that echoes the screams of the dead. When activated, it allows the user to hear the last words spoken by anyone who died within 10 meters. Perfect for interrogation or locating buried secrets.",
    "category": "equipment",
    "price": 2000,
    "icon": "🗝",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Reveals the last spoken word of a target within 10m",
      "Causes minor psychic backlash on repeated use",
      "Grants +1 to Stealth checks in dimly lit areas"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 5
  },
  "rogueport_vaulted_tongue": {
    "id": "rogueport_vaulted_tongue",
    "name": "Vaulted Tongue",
    "description": "A tongue-shaped amulet that grants the wearer the ability to speak in the language of the underworld. Perfect for infiltrating corrupt offices or negotiating deals with the highest bidder.",
    "price": 2000,
    "icon": "📜",
    "stock": 5,
    "rarity": "rare",
    "effects": [
      "Grants temporary language of the underworld",
      "Allows bypassing security checks via coded phrases",
      "Fades after 10 minutes of use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Shy Guy Smugglers",
    "levelRequirement": 5
  },
  "rogueport_venom_amber": {
    "id": "rogueport_venom_amber",
    "name": "Amber Fang of the Crimson Viper",
    "description": "A venomous amber tooth that can be used as a throwing weapon or a piercing tool. Upon impact, it injects a paralyzing toxin that renders targets immobile for 3 seconds. Ideal for quick takedowns or escape scenarios.",
    "category": "equipment",
    "price": 2000,
    "icon": "🐍",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Throwable weapon with 50% chance to trigger paralyzing toxin",
      "Pierces armor for 25% extra damage",
      "Causes target to scream for 1 second upon being hit"
    ],
    "vendor": "rogueport",
    "shippedBy": "Boo Spectral Mail",
    "levelRequirement": 5
  },
  "rogueport_venom_blood_cloak": {
    "id": "rogueport_venom_blood_cloak",
    "name": "Venom Blood Cloak",
    "description": "A tattered cloak that absorbs poison and venom from enemies. Wears off after 10 minutes or if struck by a direct hit. Perfect for infiltrating toxic lairs or escaping poison traps.",
    "category": "equipment",
    "price": 2000,
    "icon": "📦",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Poison Absorption: Absorbs 100% of incoming poison damage",
      "Duration: 10 minutes or until hit",
      "Visual: Glows faintly red when active"
    ],
    "vendor": "rogueport",
    "shippedBy": "Void Drifter Relay",
    "levelRequirement": 5
  },
  "rogueport_venom_tincture": {
    "id": "rogueport_venom_tincture",
    "name": "Venom Tincture of the Broken Blade",
    "description": "A dark elixir brewed from the venom of forgotten assassins. Injects a rush of adrenaline and suppresses pain for a brief, dangerous burst. Not for the faint of heart.",
    "price": 430,
    "icon": "💉",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "+30% attack speed for 15 seconds",
      "50% chance to deal double damage on first hit",
      "20% chance to inflict bleeding on enemies"
    ],
    "vendor": "rogueport",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 4
  },
  "rogueport_venom_tongue": {
    "id": "rogueport_venom_tongue",
    "name": "Venom Tongue",
    "description": "A cursed tongue crafted from the bite of a venomous toad and a smuggled poison viper. Speaks in whispers and can poison enemies with a single lick—though it may also turn the user into a walking toxin trap.",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Lick enemy to poison them for 3 seconds (damage: 20% of max HP)",
      "10% chance to trigger a toxic backlash on user",
      "Voice modulation: whispers can be heard from 50 feet"
    ],
    "vendor": "rogueport",
    "shippedBy": "Pipe Express",
    "levelRequirement": 5
  },
  "rogueport_viper_tongue": {
    "id": "rogueport_viper_tongue",
    "name": "Viper Tongue of the Underworld",
    "description": "A cursed tongue that speaks in whispers of the underworld. It can mimic any voice or language, but it causes the user to lose their sense of direction after 30 minutes. Perfect for infiltration or interrogation.",
    "category": "equipment",
    "price": 2000,
    "icon": "🐍",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "+15% Speech skill",
      "Can mimic any voice or language",
      "-10% Agility after 30 minutes of use"
    ],
    "vendor": "rogueport",
    "shippedBy": "Koopa Postal",
    "levelRequirement": 5
  },
  "rogueport_void_piercer": {
    "id": "rogueport_void_piercer",
    "name": "Whispering Dagger",
    "description": "A razor-sharp dagger rumored to have been forged from the edge of Almost at the Edge.",
    "category": "equipment",
    "price": 2000,
    "icon": "💀",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Inflicts additional damage to enemies with unstable or ethereal forms.",
      "Has a chance to inflict Edgebleed, draining enemy health over time.",
      "When used, whispers of chaos can be heard by those nearby, potentially distracting allies."
    ],
    "vendor": "rogueport",
    "shippedBy": "Noki Coral Fleet",
    "levelRequirement": 6
  },
  "rogueport_wet_glass": {
    "id": "rogueport_wet_glass",
    "name": "Wet Glass of the Broken Lullaby",
    "description": "A shattered mirror embedded with liquid moonlight — its reflections do not show the truth, but the fear of what you fear. When held, it amplifies the player’s fear aura.",
    "category": "equipment",
    "price": 2000,
    "icon": "🌙",
    "stock": 2,
    "rarity": "rare",
    "effects": [
      "Amplifies fear aura (30% chance to trigger fear effect)",
      "Reflections show distorted memories of the player’s past",
      "Increases damage against ghostly foes by 20%"
    ],
    "vendor": "rogueport",
    "shippedBy": "Lakitu Drones",
    "levelRequirement": 6
  },
  "rope_ladder": {
    "id": "rope_ladder",
    "name": "Vine Ladder",
    "description": "A ladder made of sturdy vines. 50ft long.",
    "category": "equipment",
    "price": 63,
    "icon": "🪜",
    "stock": 15,
    "rarity": "common",
    "effects": [
      "Climbing aid",
      "Bio-degradable"
    ],
    "vendor": "valley_trading_post",
    "shippedBy": "Sack",
    "levelRequirement": 1
  },
  "rosalina_wand": {
    "id": "rosalina_wand",
    "name": "Rosalina's Wand",
    "description": "A wand for cosmic spells and protection.",
    "category": "premium",
    "price": 35000,
    "icon": "🌟",
    "stock": 1,
    "rarity": "legendary",
    "effects": [
      "Cast Shield (reaction, +5 AC)",
      "Summon Luma ally (CR 1, once per day)",
      "Glows in starlight"
    ],
    "vendor": "comet_observatory",
    "shippedBy": "Star Delivery",
    "levelRequirement": 10
  },
  "rotten_mush": {
    "id": "rotten_mush",
    "name": "Rotten Mushroom",
    "description": "A mushroom that has clearly gone bad. It smells like wet socks and regret.",
    "category": "consumables",
    "price": 63,
    "icon": "🤢",
    "stock": 99,
    "rarity": "common",
    "effects": [
      "Restore 1 HP",
      "You are Poisoned for 10 minutes (DC 13 CON)"
    ],
    "vendor": "rogueport_black_market",
    "shippedBy": "Garbage Chute",
    "levelRequirement": 1
  },
  "royal_feast": {
    "id": "royal_feast",
    "name": "Royal Feast (For 100 Guests)",
    "description": "A banquet fit for a king, complete with entertainment and enchanted food.",
    "category": "premium",
    "price": 6900,
    "icon": "🍽️",
    "stock": 3,
    "rarity": "epic",
    "effects": [
      "Guests gain +2 to all rolls for 1 day",
      "May improve relations with factions",
      "Leftovers are magically preserved"
    ],
    "vendor": "gilded_gryphon",
    "shippedBy": "Floating Banquet Hall",
    "levelRequirement": 8
  },
  "royal_pardon": {
    "id": "royal_pardon",
    "name": "Royal Pardon",
    "description": "A decree that clears you of any crime.",
    "category": "premium",
    "price": 35000,
    "icon": "📜",
    "stock": 1,
    "rarity": "legendary",
    "effects": [
      "All bounties and criminal charges are erased",
      "Pardon is magically enforced",
      "May anger the original victims"
    ],
    "vendor": "wario_direct",
    "shippedBy": "Royal Seal",
    "levelRequirement": 8
  },
  "royal_sticker": {
    "id": "royal_sticker",
    "name": "Royal Sticker (Crown)",
    "description": "A shiny crown sticker. Allows you to 'paperize' reality.",
    "category": "forbidden",
    "price": 35000,
    "icon": "👑",
    "stock": 1,
    "rarity": "legendary",
    "effects": [
      "Action: Flatten a 3D object into a 2D sticker",
      "Action: Paste a sticker into the world to make it real",
      "Includes a sassy fairy companion named Kersti (curse)",
      "Vulnerability to Fire and Water"
    ],
    "vendor": "rogueport_black_market",
    "shippedBy": "Album",
    "levelRequirement": 10
  },
  "rub_for_a_genie_or": {
    "id": "rub_for_a_genie_or",
    "name": "Wario's Wish-Granting Lamp",
    "description": "Rub for a genie... or Wario.",
    "category": "equipment",
    "price": 430,
    "icon": "🏺",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "Rub: 50% Wario appears (help or scam), 50% minor boon (1d10 gp)",
      "Lamp dusty: disadvantage on rub",
      "Genie fans: +1 Persuasion",
      "Made by: Wario Lamps"
    ],
    "vendor": "wario_land",
    "shippedBy": "Rub Rush",
    "levelRequirement": 5
  },
  "ruin_powder": {
    "id": "ruin_powder",
    "name": "Ruin Powder",
    "description": "Confusing dust made from crushed ruins.",
    "category": "consumables",
    "price": 430,
    "icon": "🧂",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "Thrown (20ft). Target has Disadvantage on Attack rolls for 1 minute (CON Save DC 12 ends)"
    ],
    "vendor": "shamans_hut",
    "shippedBy": "Pouch",
    "levelRequirement": 2
  },
  "rune_engraved_shield": {
    "id": "rune_engraved_shield",
    "name": "Rune-Engraved Shield (Defensive)",
    "description": "A shield etched with dwarven runes that absorbs magical attacks.",
    "price": 35000,
    "icon": "🛡️",
    "stock": 2,
    "rarity": "legendary",
    "effects": [
      "Absorbs 50 spell levels per day",
      "Rune recharge in moonlight",
      "Made by: Rune Smiths"
    ],
    "vendor": "dwarven_forge",
    "shippedBy": "Mountain Caravan",
    "levelRequirement": 14
  },
  "runescape_rune_set": {
    "id": "runescape_rune_set",
    "name": "Rune Set",
    "description": "Runes for casting elemental spells.",
    "category": "consumables",
    "price": 430,
    "icon": "🔮",
    "stock": 10,
    "rarity": "uncommon",
    "effects": [
      "Cast fire/ice/earth bolt (1d10 damage)",
      "10 runes per set",
      "Craft more with mining"
    ],
    "vendor": "gilded_gryphon",
    "shippedBy": "Rune Pouch",
    "levelRequirement": 2
  },
  "rust_monkey_wrench": {
    "id": "rust_monkey_wrench",
    "name": "Rust Monkey Wrench",
    "description": "A wrench that calls trained monkeys to sabotage constructs.",
    "category": "equipment",
    "price": 2000,
    "icon": "🔧",
    "stock": 6,
    "rarity": "rare",
    "effects": [
      "Summon 1d3 rust monkeys to sabotage constructs for 1 minute (1/day)",
      "Monkeys have +5 to hit constructs, deal 1d4+2 damage",
      "20% chance they break something you own instead",
      "Monkeys accept payment in bananas only"
    ],
    "vendor": "beastmaster_tools",
    "shippedBy": "Monkey Business Shipping",
    "levelRequirement": 6
  },
  "rusted_starman": {
    "id": "rusted_starman",
    "name": "Rusted Starman",
    "description": "A tarnished, slow-glowing star relic. It's seen better days but still hums with speed.",
    "category": "consumables",
    "price": 35000,
    "icon": "🌠",
    "stock": 1,
    "rarity": "legendary",
    "effects": [
      "Single Use: Grants Haste & invulnerability to physical damage for 1 round, then you are immobile for 1 minute",
      "After use, gain one level of exhaustion",
      "Attracts cosmic scavengers"
    ],
    "vendor": "lunar_auction",
    "shippedBy": "Antique Box",
    "levelRequirement": 10
  },
  "rusty_dwarven_lockpick_set": {
    "id": "rusty_dwarven_lockpick_set",
    "name": "Rusty Dwarven Lockpick Set",
    "description": "A set of enchanted picks from a forgotten forge, great for cracking mundane locks.",
    "category": "equipment",
    "price": 430,
    "icon": "🔑",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "Advantage on Sleight of Hand checks to pick locks (DC 15 or lower)",
      "Breaks on a natural 1, but can be repaired for 50 gold",
      "Dwarven runes glow faintly when near trapped doors",
      "Not for use on magical locks – might summon a grumpy dwarf spirit"
    ],
    "vendor": "undermountain_trader",
    "shippedBy": "Leather Pouch Delivery",
    "levelRequirement": 5
  },
  "rusty_sword_sharpening_stone": {
    "id": "rusty_sword_sharpening_stone",
    "name": "Rusty Sword Sharpening Stone",
    "description": "Hones blades to a keen edge, but watch for rust transfer.",
    "category": "equipment",
    "price": 430,
    "icon": "⚔️",
    "stock": 7,
    "rarity": "uncommon",
    "effects": [
      "Once per day: Sharpen a weapon for +1 to attack rolls for 1 hour",
      "10% chance the weapon gains a minor rust (disadvantage on next attack)",
      "Works on any bladed weapon, even kitchen knives",
      "Portable – fits in a pocket, but leaves orange stains"
    ],
    "vendor": "blacksmiths_scrap",
    "shippedBy": "Oiled Cloth Wrap",
    "levelRequirement": 5
  },
  "saddle_arabia_caravan_guide_service": {
    "id": "saddle_arabia_caravan_guide_service",
    "name": "Saddle Arabia Caravan Guide Service",
    "description": "Expert navigation through deserts.",
    "price": 430,
    "icon": "🏜️",
    "stock": 6,
    "rarity": "uncommon",
    "effects": [
      "Avoid sandstorms: +2 to Survival in deserts for 1 week",
      "Guide provides 1d4 dates (healing fruit, 1d4 HP)",
      "Camels spit if you're rude",
      "Made by: Arabian Nomads"
    ],
    "vendor": "saddle_arabia_bazaar",
    "shippedBy": "Camel Train",
    "levelRequirement": 5
  },
  "saddle_arabia_date_palm_cookies_recipe": {
    "id": "saddle_arabia_date_palm_cookies_recipe",
    "name": "Recipe: Saddle Arabia Date Palm Cookies",
    "description": "Sweet cookies with dates for desert endurance.",
    "price": 430,
    "icon": "🍪",
    "stock": 7,
    "rarity": "uncommon",
    "effects": [
      "Teaches recipe: Mix dates and dough (30 min); eat to gain +5 ft speed in heat and heal 1d4 HP from dehydration",
      "Requires: Dates; energy-packed",
      "Camels love them too",
      "Made by: Arabian Bakers"
    ],
    "vendor": "saddle_arabia_oasis",
    "shippedBy": "Sweet Date Dispatch",
    "levelRequirement": 5
  },
  "saddle_arabia_date_palm_presser": {
    "id": "saddle_arabia_date_palm_presser",
    "name": "Saddle Arabia Date Palm Presser",
    "description": "Press for extracting date syrup into cookies.",
    "category": "equipment",
    "price": 430,
    "icon": "🍪",
    "stock": 6,
    "rarity": "uncommon",
    "effects": [
      "Presses sweet syrup; advantage on sweet recipe prep",
      "Yields extra syrup (+1 serving)",
      "Sticky – cleans with oil",
      "Made by: Arabian Pressers"
    ],
    "vendor": "saddle_arabia_oasis",
    "shippedBy": "Sweet Squeeze Sack",
    "levelRequirement": 5
  },
  "saddle_arabia_date_pitter": {
    "id": "saddle_arabia_date_pitter",
    "name": "Saddle Arabia Date Pitter",
    "description": "Pits dates with precision.",
    "category": "equipment",
    "price": 63,
    "icon": "🫒",
    "stock": 9,
    "rarity": "common",
    "effects": [
      "Advantage on cooking checks involving dates (perfect pits)",
      "Can also pit olives, cherries, small stones",
      "You become an expert on dates: advantage on History checks about them",
      "Made by: Saddle Arabia Date Farmers"
    ],
    "vendor": "saddle_arabia",
    "shippedBy": "Camel Caravan",
    "levelRequirement": 5
  },
  "saddle_arabia_desert_compass": {
    "id": "saddle_arabia_desert_compass",
    "name": "Saddle Arabia Desert Compass",
    "description": "A compass that points toward the nearest water source in desert environments.",
    "category": "equipment",
    "price": 430,
    "icon": "🧭",
    "stock": 9,
    "rarity": "uncommon",
    "effects": [
      "Always points toward the nearest water source in deserts",
      "Grants advantage on Survival checks in desert environments",
      "Made by: Saddle Arabia"
    ],
    "vendor": "desert_traders_post",
    "shippedBy": "Leather Case",
    "levelRequirement": 4
  },
  "saddle_arabia_desert_robe": {
    "id": "saddle_arabia_desert_robe",
    "name": "Saddle Arabia Desert Robe",
    "description": "Loose robe for enduring hot sands.",
    "price": 430,
    "icon": "👘",
    "stock": 5,
    "rarity": "uncommon",
    "effects": [
      "Resistance to heat exhaustion",
      "+1 to Constitution saves in deserts",
      "Robe billows in wind (disadvantage on ranged attacks)",
      "Made by: Arabian Weavers"
    ],
    "vendor": "saddle_arabia_souk",
    "shippedBy": "Camel Hair Fold",
    "levelRequirement": 5
  },
  "saddle_arabia_recipe_date_and_camel_milk_smoothie": {
    "id": "saddle_arabia_recipe_date_and_camel_milk_smoothie",
    "name": "Recipe: Saddle Arabia Date & Camel Milk Smoothie",
    "description": "A desert traveler's energy drink.",
    "price": 440,
    "icon": "📜",
    "stock": 7,
    "rarity": "uncommon",
    "effects": [
      "Grants +10 speed in desert terrain for 4 hours",
      "Quenches thirst completely for 24 hours",
      "You smell like a camel: disadvantage on Charisma with non-desert folk",
      "Made by: Saddle Arabia Oasis Bar"
    ],
    "vendor": "saddle_arabia",
    "shippedBy": "Camel Caravan",
    "levelRequirement": 5
  },
  "saddle_arabia_saddle_blanket": {
    "id": "saddle_arabia_saddle_blanket",
    "name": "Saddle Arabia Saddle Blanket",
    "description": "A blanket that makes your saddle comfortable.",
    "category": "equipment",
    "price": 440,
    "icon": "🐪",
    "stock": 7,
    "rarity": "uncommon",
    "effects": [
      "Advantage on saves vs. exhaustion from long travel",
      "Your mount loves you: advantage on Animal Handling with it",
      "Blanket is covered in camel hair (allergy sufferers disadvantage)",
      "Made by: Saddle Arabia Weavers"
    ],
    "vendor": "saddle_arabia",
    "shippedBy": "Camel Caravan",
    "levelRequirement": 5
  },
  "safe_house_rental": {
    "id": "safe_house_rental",
    "name": "Safe House Rental (1 Month)",
    "description": "An anonymous, well-hidden residence with defensive wards and an exit tunnel.",
    "category": "services",
    "price": 2000,
    "icon": "🏠",
    "stock": 5,
    "rarity": "rare",
    "effects": [
      "Secure lodging for 30 days (Wards prevent scrying/detection)",
      "2 secret exits to different districts",
      "Monthly supplies included (food, water, basic medicine)",
      "Landlord asks no questions"
    ],
    "vendor": "rogueport_black_market",
    "shippedBy": "Keys Delivered",
    "levelRequirement": 3
  },
  "safety_goggles": {
    "id": "safety_goggles",
    "name": "Safety Goggles",
    "description": "Professor E. Gadd style.",
    "category": "equipment",
    "price": 63,
    "icon": "🥽",
    "stock": 100,
    "rarity": "common",
    "effects": [
      "Immunity to blindness from dust/sand",
      "Look like a nerd"
    ],
    "vendor": "toad_town_market",
    "shippedBy": "Case",
    "levelRequirement": 1
  },
  "safety_ring": {
    "id": "safety_ring",
    "name": "Safety Ring",
    "description": "A simple ring that guarantees safety. The ultimate defense against bad luck.",
    "category": "equipment",
    "price": 35000,
    "icon": "💍",
    "stock": 1,
    "rarity": "legendary",
    "effects": [
      "Immunity to ALL status conditions (Poison, Paralyze, Sleep, Petrify, Fear, Charm)",
      "Immunity to Critical Hits",
      "Immunity to Instant Death effects"
    ],
    "vendor": "shamans_hut",
    "shippedBy": "Velvet Box",
    "levelRequirement": 10
  },
  "samus_arm_cannon": {
    "id": "samus_arm_cannon",
    "name": "Arm Cannon Replica",
    "description": "A cannon for energy blasts and missiles.",
    "category": "equipment",
    "price": 7000,
    "icon": "🔫",
    "stock": 2,
    "rarity": "epic",
    "effects": [
      "Fire energy: 3d6 force, range 100ft",
      "Missile mode: 4d6 explosive (3 charges/day)",
      "Scan mode: Detect weaknesses"
    ],
    "vendor": "wario_direct",
    "shippedBy": "Tech Crate",
    "levelRequirement": 8
  },
  "sapphire_frog_boots": {
    "id": "sapphire_frog_boots",
    "name": "Sapphire Frog Boots",
    "description": "Boots with a glossy blue sheen and webbed hems. You feel amphibious and precise.",
    "category": "equipment",
    "price": 7000,
    "icon": "🐸",
    "stock": 2,
    "rarity": "epic",
    "effects": [
      "Grants swim speed equal to walking speed and advantage on Acrobatics in water",
      "Once per day: leap across 60ft of water as if solid",
      "Leaves a faint spray with each step"
    ],
    "vendor": "sea_seller",
    "shippedBy": "Waterproof Satchel",
    "levelRequirement": 6
  },
  "sarasaland_desert_soul_torch": {
    "id": "sarasaland_desert_soul_torch",
    "name": "Desert Soul Torch",
    "description": "A beacon of primal desert magic, burning with the essence of forgotten spirits",
    "price": 440,
    "icon": "🌞",
    "stock": 8,
    "rarity": "uncommon",
    "effects": [
      "Illuminates area for 30 seconds with a golden aura",
      "Deals 1d4 radiant damage to undead",
      "Slightly increases speed while running",
      "Burns out after 10 uses",
      "Causes minor hallucinations if used in moonlight",
      "Forged from the bones of a lost desert cult"
    ],
    "vendor": "sarasaland",
    "shippedBy": "Warp Whistle Transit",
    "levelRequirement": 3
  },
  "sarasaland_dream_touched_glove": {
    "id": "sarasaland_dream_touched_glove",
    "name": "Dream Touched Glove",
    "description": "Woven from the essence of Dream Land’s most mischievous puffballs, this glove allows the wearer to manipulate dreamscapes with simple gestures. It grants temporary control over hallucinogenic fog but also risks trapping the user inside a nightmare dimension if overused. Delivered by Chain Chomp Courier with a note: “If you’re not afraid of your own mind, you’ll get it back.”",
    "category": "equipment",
    "price": 440,
    "icon": "🌈",
    "stock": 1,
    "rarity": "uncommon",
    "effects": [
      "Manipulates dream fog within 10 meters",
      "Grants +2 to Perception checks against illusions",
      "10% chance to trigger a dream-induced hallucination",
      "Causes temporary drowsiness for 1d4 rounds",
      "Delivered by Chain Chomp Courier (unreliable)",
      "Worn by: Shy Guys Union"
    ],
    "vendor": "sarasaland",
    "shippedBy": "Chain Chomp Courier",
    "levelRequirement": 4
  },
  "sarasaland_dusty_scepter": {
    "id": "sarasaland_dusty_scepter",
    "name": "Dusty Scepter of the Fallen Sun",
    "description": "",
    "category": "equipment",
    "price": 440,
    "icon": "🌅",
    "stock": 1,
    "rarity": "uncommon",
    "effects": [
      "Grants +1 to charisma checks during diplomacy",
      "Inflicts 1d4 psychic damage on hostile creatures",
      "Wears a faint afterimage of a lost sun",
      "Can be used as a ceremonial staff in regency parleys",
      "Requires a noble to wield with honor",
      "Scent of old sand and forgotten crowns"
    ],
    "vendor": "sarasaland",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 3
  },
  "sarasaland_furry_canteen": {
    "id": "sarasaland_furry_canteen",
    "name": "Furry Canteen of the Bowserite Nomads",
    "description": "",
    "price": 64,
    "icon": "🍽",
    "stock": 5,
    "rarity": "common",
    "effects": [
      "Restores 10 hit points when consumed",
      "Causes 1d2 nausea if consumed too quickly",
      "Absorbs 1d3 poison damage per use",
      "Made from the hide of a once-loyal rat-tamer",
      "Keeps drink warm for 10 minutes",
      "Flavor: salty desert wind with a hint of burnt sugar"
    ],
    "vendor": "sarasaland",
    "shippedBy": "Rakasha Spirit Walk",
    "levelRequirement": 1
  },
  "sarasaland_glowing_scepter": {
    "id": "sarasaland_glowing_scepter",
    "name": "Glowing Scepter of the Sunken Throne",
    "description": "Once wielded by the last royal sun-dragon of Sarasaland, this scepter pulses with the warmth of a dying star. It grants the wielder the power to summon brief bursts of radiant light—but at the cost of their own vitality. The scepter was hidden beneath the sand by a Koopa noble who feared its power would awaken ancient sun-worshipers. Now it’s been repurposed as a diplomatic tool.",
    "category": "equipment",
    "price": 7000,
    "icon": "☀",
    "stock": 2,
    "rarity": "epic",
    "effects": [
      "Grants 1d6 radiant damage per cast",
      "Reduces enemy resistance to radiant damage by 50%",
      "Wielder gains 1d4 temporary HP per use",
      "Wielder’s stamina is reduced by 10% after each use",
      "Grants a 10% chance to reveal hidden enemy weaknesses",
      "Can be used to temporarily blind enemies for 1 round"
    ],
    "vendor": "sarasaland",
    "shippedBy": "Koopa Postal",
    "levelRequirement": 7
  },
  "sarasaland_golden_dream_bottle": {
    "id": "sarasaland_golden_dream_bottle",
    "name": "Golden Dream Bottle",
    "description": "A crystal bottle filled with liquid that glows like molten gold. The vendor claims it contains the essence of the first sunrise in Sarasaland—but after three nights of use, the bottle began whispering dreams of a world that never existed. The next morning, the user found themselves trapped inside a dream that would not end.",
    "price": 35000,
    "icon": "💫",
    "stock": 0,
    "rarity": "legendary",
    "effects": [
      "Grants temporary immunity to fear and confusion",
      "Causes the user to forget their last 24 hours",
      "Induces 100% chance of hallucinating a desert oasis that does not exist",
      "If consumed while under magic, causes 1d6 damage to the user",
      "Crafted by: Land Mordor (and no one else)",
      "This item is a scam. The bottle contains nothing but a cursed illusion of a sunlit desert. Do not drink it."
    ],
    "vendor": "sarasaland",
    "shippedBy": "Pipe Express",
    "levelRequirement": 8
  },
  "sarasaland_iron_bottle_bubble": {
    "id": "sarasaland_iron_bottle_bubble",
    "name": "Iron Bottle Bubble",
    "description": "A glass vial filled with liquid iron that hums with the sound of a thousand war drums. It was smuggled across the desert by a desert merchant who claimed it could turn sand into gold—until it accidentally turned the merchant into a sandworm.",
    "price": 440,
    "icon": "💧",
    "stock": 2,
    "rarity": "uncommon",
    "effects": [
      "Grants temporary resistance to fire damage for 1 turn",
      "Causes hallucinations of desert storms if consumed too fast",
      "Increases thirst by 200% after use",
      "Can be used as a distraction during combat",
      "Crafted by: Land Mordor"
    ],
    "vendor": "sarasaland",
    "shippedBy": "Pipe Express",
    "levelRequirement": 3
  },
  "sarasaland_item_name_here": {
    "id": "sarasaland_item_name_here",
    "name": "Desert Wind Blade",
    "description": "A razor sharp blade forged from the breath of the dunes",
    "category": "equipment",
    "price": 2000,
    "icon": "🔮",
    "stock": 3,
    "rarity": "rare",
    "effects": [
      "Deals extra damage to enemies in open terrain",
      "Slows movement speed when used in combat",
      "Causes minor wind damage on hit",
      "Made by: Skaven",
      "Grants temporary invisibility during stealth",
      "Requires high stamina to activate"
    ],
    "vendor": "sarasaland",
    "shippedBy": "Noki Coral Fleet",
    "levelRequirement": 5
  },
  "sarasaland_onyx_hand_gloves": {
    "id": "sarasaland_onyx_hand_gloves",
    "name": "Onyx Hand Gloves",
    "description": "Fitted with enchanted fingers that bend reality to the wearer’s will",
    "category": "equipment",
    "price": 27000,
    "icon": "👁",
    "stock": 1,
    "rarity": "epic",
    "effects": [
      "Grants 2d6 dark damage against enemies with high stealth",
      "Grants invisibility for 5 seconds per use",
      "Costs 100 gold to activate each time",
      "Increases damage against shadow creatures by 50%",
      "Causes a 10% chance to trigger a soul-siphon event",
      "Crafted by the Onyx Hand’s inner circle",
      "Worn only by elite vampire enforcers"
    ],
    "vendor": "sarasaland",
    "shippedBy": "Warp Whistle Transit",
    "levelRequirement": 6
  }
};
