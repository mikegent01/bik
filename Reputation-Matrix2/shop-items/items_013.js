// Shop items chunk 13 of 51
// Items 1201 to 1300 (100 items)
// Auto-generated: 2026-03-21 13:41:41

import { SHOP_CATEGORIES } from './categories.js';

export const ITEMS_013 = {
  'faerun_shadowwhisper_belt': {
        id: 'faerun_shadowwhisper_belt',
        name: "Shadowwhisper Belt",
        description: "Woven from the threads of forgotten dreams, this belt allows the wearer to phase briefly into shadowed corners of the world. Its eerie glow reveals hidden paths and concealed doors in darkened dungeons.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+10% chance to detect hidden doors or traps in shadowy areas",
        "Can be worn while in dim or obscured light",
        "Grants temporary invisibility for 10 seconds when triggered by a shadow-based spell"
        ],
        vendor: 'faerun',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'faerun_silken_blood_scarf': {
        id: 'faerun_silken_blood_scarf',
        name: "Silken Blood Scarf",
        description: "Woven from the threads of forgotten warriors’ blood and moonlight, this scarf absorbs poison and slows enemy movement. It also emits a faint hum that soothes panicked allies.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Absorbs 1d6 poison damage per turn",
        "Reduces enemy movement speed by 25% for 1 round",
        "Increases allies’ morale by +1 when worn in combat"
        ],
        vendor: 'faerun',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'faerun_silken_tide_cloak': {
        id: 'faerun_silken_tide_cloak',
        name: "Silken Tide Cloak of the Deepborn",
        description: "A cloak woven from the essence of drowned cities and forgotten sea-elves. It grants the wearer temporary water resistance and allows them to move through underwater terrain as if it were land. Ideal for subaquatic scouts or deep-dungeon explorers.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5400,
        icon: '🌊',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Grants +1 to swim checks",
        "Grants immunity to drowning for 10 minutes",
        "Grants +2 to perception checks in murky or underwater environments"
        ],
        vendor: 'faerun',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'faerun_soul_tether_chain': {
        id: 'faerun_soul_tether_chain',
        name: "Soul Tether Chain",
        description: "A blackened chain forged from the soul of a fallen champion. When linked to a party member, it draws their strength into the wielder, granting temporary invulnerability and resistance to debuffs.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🧩',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 100% resistance to debuffs for 30 seconds",
        "Grants temporary invulnerability for 20 seconds",
        "Can be linked to one ally at a time"
        ],
        vendor: 'faerun',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'faerun_soulflame_rod': {
        id: 'faerun_soulflame_rod',
        name: "Soulflame Rod of the Unseen Flame",
        description: "A staff that channels the essence of a fallen god’s last breath. When wielded, it emits a flickering, silent flame that can burn through magical wards and disperse curses. Requires a steady hand and a soul to ignite.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🔥',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Deals 2d6 fire damage on a successful attack",
        "Grants +1 to dispel magic checks",
        "Can extinguish curses or enchantments on contact"
        ],
        vendor: 'faerun',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'faerun_staff_of_wild_magic': {
        id: 'faerun_staff_of_wild_magic',
        name: "Staff of Wild Magic",
        description: "A twisted oak staff crackling with chaotic energy",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Deals 2d6 bonus arcane damage",
        "15% chance to trigger wild magic surge",
        "Requires attunement by a spellcaster",
        "Crafted by: Chaos Dwarfs"
        ],
        vendor: 'faerun',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'faerun_sunken_archives_scribe_quill': {
        id: 'faerun_sunken_archives_scribe_quill',
        name: "Scribe Quill of the Sunken Archives",
        description: "A pen made from the ink of forgotten texts and the bones of lost scribes",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 780,
        icon: '📜',
        stock: 5,
        rarity: 'rare',
        effects: [
            "Allows writing of magical incantations with +2d6 spell power",
        "Each spell written causes a 5% chance of triggering a random effect from the Sunken Archives",
        "Consumes ink from the quills core, which regenerates after 10 minutes",
        "Causes minor hallucinations if used near a Void Touched entity",
        "Cannot be used without attunement to the Archives",
        "Crafted from the pre-Peach texts recovered off Isle Delfino"
        ],
        vendor: 'faerun',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 4
    },

  'faerun_sword_of_the_broken_crown': {
        id: 'faerun_sword_of_the_broken_crown',
        name: "Sword of the Broken Crown",
        description: "A gleaming blade forged from the last shard of a fallen king’s crown, it whispers of lost glory and vengeance. When wielded by a noble, it grants a brief surge of courage against foes who mock the wearer.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+2d6 damage on critical hits",
        "+10% chance to parry enemy attacks",
        "Grants a 10 second aura of regal dignity that reduces fear effects by 50%"
        ],
        vendor: 'faerun',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'faerun_venom_spray': {
        id: 'faerun_venom_spray',
        name: "Venom Spray of the Whispering Gorge",
        description: "A vial of corrosive, sentient toxin that drips with whispers of the dead. When applied, it deals damage and forces the target to make a Will save or whisper a forgotten memory.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 220,
        icon: '💉',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "Deals 2d6 acid damage on hit",
        "Forces target to make a DC 14 Will save or recall a memory from 10 years ago",
        "Leaves a lingering scent that attracts undead"
        ],
        vendor: 'faerun',
        shippedBy: 'Koopa Postal',
        levelRequirement: 4
    },

  'faerun_void_touched_talisman': {
        id: 'faerun_void_touched_talisman',
        name: "Void Touched Talisman",
        description: "A tarnished silver disc etched with fractured symbols of reality",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 22000,
        icon: '🌀',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Absorbs ambient magical energy and converts it to raw power",
        "Reduces the cost of spellcasting by 25%",
        "Triggers reality distortion when used near the Void",
        "Causes temporary blindness in the user during activation",
        "May be destroyed by high frequency sound waves",
        "Only usable by attuned spellcasters with Void affinity",
        "Crafted by the Void Watchers of the Midlands"
        ],
        vendor: 'faerun',
        shippedBy: 'Bullet Bill Express',
        levelRequirement: 10
    },

  'faerun_whispered_horn': {
        id: 'faerun_whispered_horn',
        name: "Whispered Horn of the Silent King",
        description: "A relic of a forgotten warlord, this horn emits haunting melodies that pacify wild beasts and break the will of those who hear it. Only those with a pure heart may use it.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+10% chance to disarm hostile creatures",
        "Causes enemies to retreat when played",
        "Silence duration: 3 rounds if used in combat"
        ],
        vendor: 'faerun',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'fake_boo_ghost_trap': {
    id: 'fake_boo_ghost_trap',
    name: "Fake Boo Ghost Trap",
    description: "A jar that 'traps' ghosts... or just looks spooky.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 220,
    icon: '🕳️',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Use to 'trap' minor illusions or spirits (DC 13 save or contained for 1 minute)",
        "Jar is glass: breaks easily (disadvantage on saves vs. shatter)",
        "Boo's laugh echo: advantage on Intimidation with ghosts",
        "Made by: Spooky Knockoff Co."
    ],
    vendor: 'wario_land',
    shippedBy: 'Ghostly Glass',
    levelRequirement: 4
},

  'fake_bowser_fire_snort_candy': {
    id: 'fake_bowser_fire_snort_candy',
    name: "Fake Bowser Fire Snort Candy",
    description: "Candy that makes you 'snort fire' (cough).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5200,
    icon: '🍬',
    stock: 9,
    rarity: 'uncommon',
    effects: [
        "Snort: 1d4 fire breath once",
        "Cough: disadvantage on talk",
        "Bowser: +1 Intimidation",
        "Made by: Snort Sweets"
    ],
    vendor: 'wario_land',
    shippedBy: 'Fire Fizz',
    levelRequirement: 4
},

  'fake_bowser_flame_breath_candy': {
    id: 'fake_bowser_flame_breath_candy',
    name: "Fake Bowser Flame Breath Candy",
    description: "Hot candy for 'fire breath' (cinnamon burn).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5200,
    icon: '🔥',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Eat for 1d4 fire breath (5ft cone, once)",
        "Burns mouth: disadvantage on talk for 10 min",
        "Koopa troops: advantage on Intimidation",
        "Made by: Flame Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Hot Head Haul',
    levelRequirement: 4
},

  'fake_bowser_jr_clown_car_horn': {
    id: 'fake_bowser_jr_clown_car_horn',
    name: "Fake Bowser Jr. Clown Car Horn",
    description: "A horn that honks like a clown.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5100,
    icon: '🚗',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Honk to scare: DC 12 Wis or frightened 1 round",
        "Horn silly: disadvantage on serious Intimidation",
        "Jr. fans: +1 Deception",
        "Made by: Clown Copies"
    ],
    vendor: 'wario_land',
    shippedBy: 'Car Clown',
    levelRequirement: 4
},

  'fake_bowser_jr_paintbrush': {
    id: 'fake_bowser_jr_paintbrush',
    name: "Fake Bowser Jr. Paintbrush",
    description: "A brush that 'paints' illusions (washable).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5500,
    icon: '🖌️',
    stock: 7,
    rarity: 'uncommon',
    effects: [
        "Paint minor illusion (10ft, 1 min, DC 12)",
        "Paint smears: disadvantage on artistic checks",
        "Jr. fans: +1 Deception with kids",
        "Made by: Junior Knockoffs"
    ],
    vendor: 'wario_land',
    shippedBy: 'Paint Post',
    levelRequirement: 5
},

  'fake_bowser_shell_phone': {
    id: 'fake_bowser_shell_phone',
    name: "Fake Bowser Shell Phone",
    description: "A 'phone' in a shell (conch).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4600,
    icon: '📞',
    stock: 11,
    rarity: 'common',
    effects: [
        "Listen to echoes: advantage on Perception sound",
        "Shell echoes loud: disadvantage on Stealth",
        "Bowser calls: +1 Intimidation",
        "Made by: Shell Scams"
    ],
    vendor: 'wario_land',
    shippedBy: 'Echo Express',
    levelRequirement: 4
},

  'fake_dk_coconut_gun': {
    id: 'fake_dk_coconut_gun',
    name: "Fake DK Coconut Gun",
    description: "Shoots 'coconuts' (pebbles).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5600,
    icon: '🥥',
    stock: 6,
    rarity: 'uncommon',
    effects: [
        "Shoot pebble: 1d4 bludgeoning 20ft",
        "Gun jams: disadvantage on next shot",
        "DK crew: +1 Charisma with monkeys",
        "Made by: Coconut Copies"
    ],
    vendor: 'wario_land',
    shippedBy: 'Jungle Jolt',
    levelRequirement: 5
},

  'fake_dk_diddy_kong_peanut_popgun': {
    id: 'fake_dk_diddy_kong_peanut_popgun',
    name: "Fake DK Diddy Kong Peanut Popgun",
    description: "Pops 'peanuts' (beans).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5000,
    icon: '🥜',
    stock: 9,
    rarity: 'common',
    effects: [
        "Pop bean: 1d4 bludgeoning 20ft",
        "Gun jams nuts: disadvantage on load",
        "DK: +1 ranged nuts",
        "Made by: Popgun Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Peanut Pop',
    levelRequirement: 4
},

  'fake_goomba_stomper_boots': {
    id: 'fake_goomba_stomper_boots',
    name: "Fake Goomba Stomper Boots",
    description: "Boots that make you feel like Mario, but they're just rubber.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5000,
    icon: '👢',
    stock: 12,
    rarity: 'common',
    effects: [
        "Jump higher: +5 ft to jump distance once per short rest",
        "Boots squeak: disadvantage on Stealth",
        "Against Goombas: advantage on attacks (they fear the stomp)",
        "Made by: Bootleg Bootmakers"
    ],
    vendor: 'wario_land',
    shippedBy: 'Stomp Service',
    levelRequirement: 4
},

  'fake_kirby_star_rod_replica': {
    id: 'fake_kirby_star_rod_replica',
    name: "Fake Kirby Star Rod Replica",
    description: "A wand that 'sucks in' small objects (with string).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5900,
    icon: '⭐',
    stock: 5,
    rarity: 'uncommon',
    effects: [
        "Pull small items (up to 5 lbs) 10ft with string (Sleight of Hand DC 12)",
        "String tangles: disadvantage on next use",
        "Kirby fans adore it: advantage on Persuasion with puffballs",
        "Made by: Star Knockoff Wizards"
    ],
    vendor: 'wario_land',
    shippedBy: 'Puff Delivery',
    levelRequirement: 5
},

  'fake_kirby_warp_star_toy': {
    id: 'fake_kirby_warp_star_toy',
    name: "Fake Kirby Warp Star Toy",
    description: "A star that 'warps' (bounces).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5500,
    icon: '⭐',
    stock: 7,
    rarity: 'uncommon',
    effects: [
        "Throw to bounce: distract foes (DC 12 Dex save or prone)",
        "Star deflates: disadvantage on next throw",
        "Kirby cosplay: +1 Performance",
        "Made by: Star Toy Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Warp Whiz',
    levelRequirement: 5
},

  'fake_link_bomb_bag_pouch': {
    id: 'fake_link_bomb_bag_pouch',
    name: "Fake Link Bomb Bag Pouch",
    description: "A pouch for 'bombs' (firecrackers).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4900,
    icon: '💣',
    stock: 9,
    rarity: 'common',
    effects: [
        "Throw firecracker: 1d4 thunder damage, 5ft radius",
        "Pouch smokes: disadvantage on Stealth",
        "Dungeon divers: advantage on trap talks",
        "Made by: Bomb Bootlegs"
    ],
    vendor: 'wario_land',
    shippedBy: 'Explosive Express',
    levelRequirement: 4
},

  'fake_link_master_sword_keychain': {
    id: 'fake_link_master_sword_keychain',
    name: "Fake Link Master Sword Keychain",
    description: "A mini sword that 'cuts' through problems (it's plastic).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4300,
    icon: '⚔️',
    stock: 12,
    rarity: 'common',
    effects: [
        "Use as lockpick: advantage on simple locks (heroic theme)",
        "Keychain dangles: disadvantage on Stealth (jingles)",
        "Zelda fans impressed: +1 Persuasion with them",
        "Made by: Hyrule Knockoffs"
    ],
    vendor: 'wario_land',
    shippedBy: 'Heros Haste',
    levelRequirement: 4
},

  'fake_luigi_ghostbuster_backpack': {
    id: 'fake_luigi_ghostbuster_backpack',
    name: "Fake Luigi Ghostbuster Backpack",
    description: "A pack that 'holds' ghosts (zipper).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5500,
    icon: '🎒',
    stock: 7,
    rarity: 'uncommon',
    effects: [
        "Store small spirits: advantage on ghost contain",
        "Pack heavy: disadvantage on carry",
        "Busters: +1 vs. undead",
        "Made by: Pack Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Ghost Gear',
    levelRequirement: 5
},

  'fake_luigi_mushroom_backpack': {
    id: 'fake_luigi_mushroom_backpack',
    name: "Fake Luigi Mushroom Backpack",
    description: "A pack shaped like a mushroom.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5000,
    icon: '🎒',
    stock: 9,
    rarity: 'common',
    effects: [
        "Holds extra mushrooms: +1 Nature storage",
        "Pack spores: disadvantage on allergy",
        "Luigi: +1 vs. ghosts",
        "Made by: Pack Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Mush Mail',
    levelRequirement: 4
},

  'fake_luigi_mushroom_polish': {
    id: 'fake_luigi_mushroom_polish',
    name: "Fake Luigi Mushroom Polish",
    description: "Polish that 'grows' mushrooms (mold).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4700,
    icon: '🍄',
    stock: 11,
    rarity: 'common',
    effects: [
        "Polish shoes: +1 speed 10 min",
        "Grows mold: disadvantage on hygiene",
        "Mushroom lovers: +1 Nature",
        "Made by: Polish Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Shine Shoe',
    levelRequirement: 4
},

  'fake_luigi_poltergust_nozzle': {
    id: 'fake_luigi_poltergust_nozzle',
    name: "Fake Luigi Poltergust Nozzle",
    description: "A nozzle that 'sucks' air (bellows).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5000,
    icon: '💨',
    stock: 9,
    rarity: 'common',
    effects: [
        "Blow air: push small objects 5ft",
        "Nozzle clogs: disadvantage on blow",
        "Ghost gear: +1 vs. spirits",
        "Made by: Gust Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Blow Breeze',
    levelRequirement: 4
},

  'fake_luigi_vacuum_pollen_sucker': {
    id: 'fake_luigi_vacuum_pollen_sucker',
    name: "Fake Luigi Vacuum Pollen Sucker",
    description: "Sucks up pollen (or dust bunnies).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5000,
    icon: '🧹',
    stock: 8,
    rarity: 'common',
    effects: [
        "Suck allergens: advantage on CON vs. poison (pollen)",
        "Sucks too hard: disadvantage on small item holds",
        "Luigi luck: +1 saves vs. ghosts",
        "Made by: Vacuum Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Suck Service',
    levelRequirement: 4
},

  'fake_mario_block_pounder': {
    id: 'fake_mario_block_pounder',
    name: "Fake Mario Block Pounder",
    description: "A hammer for 'pounding' blocks (nails).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4900,
    icon: '🔨',
    stock: 9,
    rarity: 'common',
    effects: [
        "Pound nails: advantage on build",
        "Hammer heavy: disadvantage on swing",
        "Block breakers: +1 Strength",
        "Made by: Block Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Pound Post',
    levelRequirement: 4
},

  'fake_mario_buzzy_beetle_shell': {
    id: 'fake_mario_buzzy_beetle_shell',
    name: "Fake Mario Buzzy Beetle Shell",
    description: "A shell for 'defense' (turtle back).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5200,
    icon: '🐢',
    stock: 9,
    rarity: 'uncommon',
    effects: [
        "+1 AC prone",
        "Shell heavy: disadvantage on stand",
        "Beetle fans: +1 Defense",
        "Made by: Shell Scams"
    ],
    vendor: 'wario_land',
    shippedBy: 'Beetle Bounce',
    levelRequirement: 4
},

  'fake_mario_mushroom_cap': {
    id: 'fake_mario_mushroom_cap',
    name: "Fake Mario Mushroom Cap",
    description: "A red hat that looks like Mario's, but made of cheap felt.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4800,
    icon: '🍄',
    stock: 15,
    rarity: 'common',
    effects: [
        "Wear it for +1 to Charisma (Performance) when imitating plumbers",
        "Hat itches: disadvantage on Concentration checks after 1 hour",
        "Real Mario might show up and demand it back (DM event, awkward encounter)",
        "Made by: Bootleg Toad Tailors"
    ],
    vendor: 'wario_land',
    shippedBy: 'Plumbers Post',
    levelRequirement: 4
},

  'fake_mario_pipe_warp_key': {
    id: 'fake_mario_pipe_warp_key',
    name: "Fake Mario Pipe Warp Key",
    description: "A key for 'warping' pipes (plunges).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4700,
    icon: '🔑',
    stock: 10,
    rarity: 'common',
    effects: [
        "Unclog pipes: advantage on plumbing",
        "Key rusts: disadvantage on wet",
        "Plumber pals: +1 Craft",
        "Made by: Pipe Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Warp Water',
    levelRequirement: 4
},

  'fake_mario_starman_cape': {
    id: 'fake_mario_starman_cape',
    name: "Fake Mario Starman Cape",
    description: "A cape that 'makes you invincible' (reflects light).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5400,
    icon: '⭐',
    stock: 7,
    rarity: 'uncommon',
    effects: [
        "Reflect light: advantage on saves vs. blind",
        "Cape frays: disadvantage on flight attempts",
        "Star power fans: +1 Charisma with heroes",
        "Made by: Star Cape Copies"
    ],
    vendor: 'wario_land',
    shippedBy: 'Invincible Invoice',
    levelRequirement: 5
},

  'fake_mario_tanooki_tail': {
    id: 'fake_mario_tanooki_tail',
    name: "Fake Mario Tanooki Tail",
    description: "A tail for 'raccoon' powers (fluff).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5300,
    icon: '🦝',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Tail spin: +5 ft glide",
        "Tail heavy: disadvantage on turn",
        "Tanooki fans: +1 Acrobatics",
        "Made by: Tail Tricks"
    ],
    vendor: 'wario_land',
    shippedBy: 'Raccoon Run',
    levelRequirement: 4
},

  'fake_mustache': {
        id: 'fake_mustache',
        name: "Fake Mustache",
        description: "A clip-on mustache. Makes you look heroic or villainous, depending on the angle.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3000,
        icon: '〰️',
        stock: 200,
        rarity: 'common',
        effects: ["+1 to Deception and persuasion checks", "Falls off if you take damage"],
        vendor: 'toad_town_market',
        shippedBy: 'Standard Courier',
        levelRequirement: 1
    },

  'fake_mustaches_to_impersonate_wario': {
    id: 'fake_mustaches_to_impersonate_wario',
    name: "Wario's Mustache Disguise Kit (Gimmick Fun!)",
    description: "Fake mustaches to impersonate Wario – or anyone with a 'stache!",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 220,
    icon: '😵',
    stock: 10,
    rarity: 'uncommon',
    effects: [
        "Attach mustache: advantage on Disguise checks to look villainous or greedy",
        "Includes glue: lasts 1 hour, but itches (disadvantage on Concentration)",
        "Bonus: Mustache twirls for +1 Charisma (Performance) in dramatic scenes",
        "Made by: Wario's Costume Closet"
    ],
    vendor: 'wario_land',
    shippedBy: 'Wacky Wig Wagon',
    levelRequirement: 5
},

  'fake_peach_pound_cake_mold': {
    id: 'fake_peach_pound_cake_mold',
    name: "Fake Peach Pound Cake Mold",
    description: "A mold for 'royal' cakes.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5300,
    icon: '🍰',
    stock: 7,
    rarity: 'uncommon',
    effects: [
        "Mold cakes: +1 bake checks",
        "Mold sticks: disadvantage on release",
        "Peach: +1 Charisma cakes",
        "Made by: Cake Copies"
    ],
    vendor: 'wario_land',
    shippedBy: 'Pound Post',
    levelRequirement: 4
},

  'fake_peach_umbrella_parasol': {
    id: 'fake_peach_umbrella_parasol',
    name: "Fake Peach Umbrella Parasol",
    description: "A parasol that 'shields' from sun (and rain).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4500,
    icon: '☔',
    stock: 12,
    rarity: 'common',
    effects: [
        "Shade: advantage on heat saves",
        "Parasol pokes: 1 piercing improvised",
        "Peach fans: +1 Charisma with princesses",
        "Made by: Parasol Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Sunny Shield',
    levelRequirement: 4
},

  'fake_pikachu_ear_hat': {
    id: 'fake_pikachu_ear_hat',
    name: "Fake Pikachu Ear Hat",
    description: "Ears that 'shock' with static.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4500,
    icon: '🐭',
    stock: 12,
    rarity: 'common',
    effects: [
        "Static shock on touch: 1 electric damage (surprise)",
        "Hat fluffy: disadvantage on fire saves",
        "Pikachu lovers: +1 Animal Handling with electric types",
        "Made by: Poke-Knockoffs"
    ],
    vendor: 'wario_land',
    shippedBy: 'Thunder Tail',
    levelRequirement: 4
},

  'fake_toad_house_key': {
    id: 'fake_toad_house_key',
    name: "Fake Toad House Key",
    description: "'Unlocks' any Toad House... or so the label says.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4900,
    icon: '🔑',
    stock: 13,
    rarity: 'common',
    effects: [
        "Use on locked doors: 50% chance it works (Toad magic), 50% chance it bends (disadvantage on next lockpick)",
        "Toads think it's cute: advantage on Persuasion with Toads",
        "Key is plastic: snaps after 3 uses",
        "Made by: Bootleg Keysmiths"
    ],
    vendor: 'wario_land',
    shippedBy: 'Mushroom Mail',
    levelRequirement: 4
},

  'fake_toad_pipe_smoke': {
    id: 'fake_toad_pipe_smoke',
    name: "Fake Toad Pipe Smoke",
    description: "Smoke that 'warps' (smoke rings).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4600,
    icon: '🚬',
    stock: 10,
    rarity: 'common',
    effects: [
        "Blow rings: minor illusion (DC 12)",
        "Smoke cough: disadvantage on breath",
        "Toad smokers: +1 Charisma",
        "Made by: Pipe Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Warp Whiff',
    levelRequirement: 4
},

  'fake_yoshi_dino_tail': {
    id: 'fake_yoshi_dino_tail',
    name: "Fake Yoshi Dino Tail",
    description: "A tail for 'dino' power (whip).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5100,
    icon: '🦕',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Whip 5ft: 1d4 slashing",
        "Tail drags: disadvantage on quiet",
        "Yoshi: +1 Strength tail",
        "Made by: Dino Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Tail Trot',
    levelRequirement: 4
},

  'fake_yoshi_egg_carton': {
    id: 'fake_yoshi_egg_carton',
    name: "Fake Yoshi Egg Carton",
    description: "A carton that 'protects' eggs (foam).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4400,
    icon: '🥚',
    stock: 12,
    rarity: 'common',
    effects: [
        "Safe egg carry: advantage on fragile transport",
        "Carton crushes: disadvantage on stack",
        "Yoshi: +1 Handling eggs",
        "Made by: Egg Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Egg Easy',
    levelRequirement: 4
},

  'fake_yoshi_egg_whistle': {
    id: 'fake_yoshi_egg_whistle',
    name: "Fake Yoshi Egg Whistle",
    description: "Blow to 'summon' a Yoshi... or just make noise.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5100,
    icon: '🥚',
    stock: 9,
    rarity: 'uncommon',
    effects: [
        "Blow as bonus action: summons a illusory Yoshi for 1 round (distraction, DC 12 Wisdom save)",
        "Whistle is shrill: disadvantage on Stealth for 1 minute after use",
        "Real Yoshis get jealous: advantage on Animal Handling with them (they want to play)",
        "Made by: Egg Imitation Experts"
    ],
    vendor: 'wario_land',
    shippedBy: 'Egg-Spress',
    levelRequirement: 4
},

  'fake_yoshi_fruit_basket': {
    id: 'fake_yoshi_fruit_basket',
    name: "Fake Yoshi Fruit Basket",
    description: "A basket that 'holds' fruit (weaves loose).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 5200,
    icon: '🧺',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Holds 10 fruits: heal 1d4 each",
        "Basket frays: 10% drop fruit",
        "Yoshi friends: +1 Handling",
        "Made by: Basket Bootlegs"
    ],
    vendor: 'wario_land',
    shippedBy: 'Fruit Ferry',
    levelRequirement: 4
},

  'fake_zelda_triforce_earrings': {
    id: 'fake_zelda_triforce_earrings',
    name: "Fake Zelda Triforce Earrings",
    description: "Earrings that 'balance' your wisdom (or just dangle).",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 4900,
    icon: '🔺',
    stock: 10,
    rarity: 'common',
    effects: [
        "Wear for +1 Wisdom saves (triforce balance)",
        "Earrings tangle hair: disadvantage on appearance checks",
        "Hyrule fans bow: advantage on royal Persuasion",
        "Made by: Triforce Fakes"
    ],
    vendor: 'wario_land',
    shippedBy: 'Wisdom Whisper',
    levelRequirement: 4
},

  'fallout_pipboy': {
    id: 'fallout_pipboy',
    name: "Pip-Boy Device",
    description: "A wrist device for mapping and radiation detection.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 20000,
    icon: '⌚',
    stock: 3,
    rarity: 'rare',
    effects: [
        "Map area, detect poison/radiation",
        "V.A.T.S.: +2 to hit once per turn",
        "Battery lasts 1 week"
    ],
    vendor: 'wario_direct',
    shippedBy: 'Tech Wristband',
    levelRequirement: 5
},

  'familiar_soul_jar': {
    id: 'familiar_soul_jar',
    name: "Familiar Soul Jar",
    description: "A jar that holds the soul of a lost familiar — you can summon it as a ghost.",
    category: SHOP_CATEGORIES.CURIOSITIES,
    price: 15000,
    icon: '👻',
    stock: 10,
    rarity: 'rare',
    effects: [
        "Summon: a ghostly familiar (CR 0) that can deliver messages, scout, or carry items",
        "It remembers everything it knew in life",
        "It cannot be destroyed — only released"
    ],
    vendor: 'shamans_hut',
    shippedBy: 'Ceramic Jar',
    levelRequirement: 4
},

  'farasi_zebra_grass_salad_recipe': {
    id: 'farasi_zebra_grass_salad_recipe',
    name: "Recipe: Farasi Zebra Grass Salad",
    description: "Fresh salad of grasses and herbs for savanna speed.",
    category: SHOP_CATEGORIES.services,
    price: 6000,
    icon: '🥗',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Teaches recipe: Toss greens (10 min); eat to gain +10 ft speed in open areas for 2 hours",
        "Requires: Savanna grasses; light and fast",
        "Zebras graze nearby",
        "Made by: Farasi Herders"
    ],
    vendor: 'farasi_savanna',
    shippedBy: 'Green Graze Guide',
    levelRequirement: 5
},

  'farasi_zebra_herb_mix_premium': {
    id: 'farasi_zebra_herb_mix_premium',
    name: "Farasi Zebra Herb Mix (Premium)",
    description: "Zebra remedies for stripes and stamina.",
    category: SHOP_CATEGORIES.premium,
    price: 13000,
    icon: '🌿',
    stock: 5,
    rarity: 'uncommon',
    effects: [
        "Consume: +10 speed for 1 hour",
        "Heal 1d6 HP",
        "Stripes appear temporarily (cosmetic)",
        "Made by: Farasi Healers"
    ],
    vendor: 'farasi_savanna',
    shippedBy: 'Herb Pouch',
    levelRequirement: 5
},

  'farasi_zebra_herd_grater': {
    id: 'farasi_zebra_herd_grater',
    name: "Farasi Zebra Herd Grater",
    description: "Grater for grass and herbs in salads.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 6000,
    icon: '🥗',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Grates tough grasses; advantage on herd recipes",
        "Striped handle for grip",
        "Shreds fingers if careless",
        "Made by: Zebra Toolers"
    ],
    vendor: 'farasi_savanna',
    shippedBy: 'Graze Grate Gear',
    levelRequirement: 5
},

  'farasi_zebra_stripe_pants': {
    id: 'farasi_zebra_stripe_pants',
    name: "Farasi Zebra Stripe Pants",
    description: "Camouflage pants for herd runners.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 13500,
    icon: '👖',
    stock: 5,
    rarity: 'uncommon',
    effects: [
        "Advantage on Dexterity (Stealth) in herds",
        "+5 ft speed in open plains",
        "Pants itch from grass seeds",
        "Made by: Farasi Tailors"
    ],
    vendor: 'farasi_savanna',
    shippedBy: 'Stripe Sew',
    levelRequirement: 5
},

  'fateweavers_thread_cutter': {
    id: 'fateweavers_thread_cutter',
    name: "Fateweaver's Thread Cutter",
    description: "Shears that sever a being from destiny itself.",
    category: SHOP_CATEGORIES.forbidden,
    price: 1025000,
    icon: '✂️',
    stock: 1,
    rarity: 'godly',
    effects: [
        "Target becomes immune to divination and fate magic (permanent)",
        "User loses 1 year of memory per use",
        "Made by: The Three Fates"
    ],
    vendor: 'destiny_merchant',
    shippedBy: 'Weave Rip',
    levelRequirement: 19
},

  'fawful_headgear': {
        id: 'fawful_headgear',
        name: "Vacuum Helmet",
        description: "I HAVE FURY! A helmet with jetpacks and laser cannons.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 360000,
        icon: '🟢',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Fly Speed 30ft",
            "Action: Energy Ball (3d6 Force)",
            "Reaction: Absorb magic missile or energy attacks",
            "You speak in broken, furious English"
        ],
        vendor: 'rogueport_black_market',
        shippedBy: 'Mustard of Doom',
        levelRequirement: 9
    },

  'fawfuls_furious_cackle_bomb': {
    id: 'fawfuls_furious_cackle_bomb',
    name: "Fawful's Furious Cackle Bomb",
    description: "A bomb that, on impact, releases a deafening, maniacal cackle.",
    category: SHOP_CATEGORIES.CONSUMABLES,
    price: 220,
    icon: '💥',
    stock: 10,
    rarity: 'uncommon',
    effects: [
        "Throw (30ft): All creatures in 15ft radius must make a DC 13 WIS save or be Frightened of you for 1 minute",
        "On a success, they are Deafened for 1 round"
    ],
    vendor: 'fawfuls_furious',
    shippedBy: 'Prankster Supply',
    levelRequirement: 4
},

  'fawfuls_furious_fusion_fajitas_recipe': {
    id: 'fawfuls_furious_fusion_fajitas_recipe',
    name: "Recipe: Fawful's Furious Fusion Fajitas ",
    description: "Spicy fajitas with 'furious' flair for mad energy.",
    category: SHOP_CATEGORIES.services,
    price: 6500,
    icon: '🌮',
    stock: 7,
    rarity: 'uncommon',
    effects: [
        "Teaches recipe: Sizzle with fury (20 min); eat to gain +1 to Intelligence (mad science) for 1 hour",
        "Requires: Peppers and meat; explosive taste",
        "Fawful cackles",
        "Made by: Furious Lab Cooks"
    ],
    vendor: 'fawfuls_furious_workshop',
    shippedBy: 'Sizzling Spice Sheet',
    levelRequirement: 5
},

  'fey_curse_breaker': {
    id: 'fey_curse_breaker',
    name: "Fey Curse Breaker (Amulet)",
    description: "An amulet that nullifies fey enchantments and illusions.",
    category: SHOP_CATEGORIES.faction,
    price: 660000,
    icon: '🍀',
    stock: 2,
    rarity: 'legendary',
    effects: [
        "Dispels fey magic on touch",
        "+5 to saves vs. charms",
        "Made by: Seelie Wardens"
    ],
    vendor: 'fey_court',
    shippedBy: 'Pixie Post',
    levelRequirement: 16,
    factionBonus: { fey: 50 }
},

  'fey_deal_negotiation': {
    id: 'fey_deal_negotiation',
    name: "Fey Deal Negotiation",
    description: "A lawyer specializing in Fey contracts ensures your deal is fair (or as fair as Fey allow).",
    category: SHOP_CATEGORIES.SERVICES,
    price: 120000,
    icon: '🧑‍⚖️',
    stock: 2,
    rarity: 'epic',
    effects: [
        "Negotiate a Fey pact with +2 to all checks",
        "May insert loopholes or protections into the deal",
        "Fey lawyer may demand a small part of your soul"
    ],
    vendor: 'fey_market',
    shippedBy: 'Golden Quill',
    levelRequirement: 10
},

  'fey_tattoo_ritual': {
    id: 'fey_tattoo_ritual',
    name: "Fey Tattoo Ritual",
    description: "A moonlit ceremony where a Fey artist etches a permanent magical tattoo onto your body.",
    category: SHOP_CATEGORIES.SERVICES,
    price: 200000,
    icon: '🖤',
    stock: 1,
    rarity: 'legendary',
    effects: [
        "Gain one Fey-themed feature (e.g., *Misty Step*, *Fey Charm*, or *Wild Magic Surge*)",
        "Tattoo glows when Fey are near",
        "Fey may request favors in the future (no takebacks)"
    ],
    vendor: 'fey_market',
    shippedBy: 'Moonlit Inkwell',
    levelRequirement: 12
},

  'feywild_blinking_blooming_flower': {
        id: 'feywild_blinking_blooming_flower',
        name: "Blinking Blooming Flower",
        description: "A flower that blooms only in the presence of wild magic. It can be used to teleport the user to a random location within 100 feet, but the target may be a looped memory, a broken mirror, or a fey court throne.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🌿',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Wild Leap: Teleport to a random point within 100 feet.",
        "Looping Bloom: If the target is a time loop, the user enters the loop and must survive 1d4 rounds before exiting.",
        "Fey Court Aura: Grants +2 to all rolls against fey creatures for 1 round."
        ],
        vendor: 'feywild',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'feywild_blinking_blossom': {
        id: 'feywild_blinking_blossom',
        name: "Blinking Blossom of the Fractured Atrium",
        description: "A delicate flower that pulses with wild magic, allowing the wearer to briefly teleport to a nearby Fey court or time-looped chamber. Its petals shimmer with unpredictable colors, reflecting the chaos of the Feywild.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 780,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Teleportation to nearest Fey court or time-looped zone (30ft radius)",
        "10% chance to cause a minor reality distortion (1d4 seconds of disorientation)",
        "Petals glow with Fey magic, granting temporary +2 to Perception checks against illusions"
        ],
        vendor: 'feywild',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'feywild_blinking_rose': {
        id: 'feywild_blinking_rose',
        name: "Blinking Rose of Shifting Time",
        description: "This enchanted rose blooms with unpredictable petals that grant temporary time loops within a 30ft radius. When touched, the user may rewind a moment or skip ahead—though the Feywild may twist the timeline in unexpected ways.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 780,
        icon: '🌹',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Revert last action within 30ft",
        "Skip 1 round of combat",
        "10% chance to cause a reality ripple (temporary visual distortion)"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_blooming_gaze': {
        id: 'feywild_blooming_gaze',
        name: "Blooming Gaze",
        description: "This enchanted eye-patch blooms with shifting petals when worn, revealing glimpses of alternate timelines. Each glance through it may grant a temporary vision of a forgotten court or a looping moment in time. Wearing it too long risks inducing a mild temporal drift.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 1d4 temporary time-loop visions per use",
        "Increases chance of encountering Fey by 50%",
        "Causes 1d6 temporary disorientation if worn during a Feywild event"
        ],
        vendor: 'feywild',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'feywild_blooming_tide_amber': {
        id: 'feywild_blooming_tide_amber',
        name: "Blooming Tide Amber",
        description: "This shimmering amber pulses with wild magic, causing time to stutter in its vicinity. When held, it allows the wielder to briefly rewind a moment—perfect for dodging misfortune or correcting a Fey court mistake. Its color shifts with the mood of the Fey.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 780,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Rewind 1 second of time when activated",
        "Inflicts 1d4 chaotic confusion on targets within 10ft",
        "Glows with Fey magic and alters perception of time in 30ft radius"
        ],
        vendor: 'feywild',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'feywild_blossom_bane_glove': {
        id: 'feywild_blossom_bane_glove',
        name: "Blossom Bane Glove",
        description: "A pair of gloves forged from the petals of the time-withering thornbloom. They grant the wearer immunity to time loops for 1 hour, but at the cost of 1 day’s memory.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 780,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants immunity to time loop effects for 1 hour",
        "Splits memories randomly upon use (may cause temporary confusion)",
        "Enhances Fey court diplomacy rolls by +2"
        ],
        vendor: 'feywild',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'feywild_blossom_bell': {
        id: 'feywild_blossom_bell',
        name: "Blossom Bell",
        description: "A bell shaped like a blooming thorn, it rings to summon a temporary Fey court presence. Its chime can disrupt magic loops or trigger unexpected alliances with wandering fey.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Triggers a 10 second Fey court echo effect when struck",
        "Can be used to break time loops in a 30ft radius",
        "Causes minor enchantment aura if held for more than 30 seconds"
        ],
        vendor: 'feywild',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'feywild_chaos_amber': {
        id: 'feywild_chaos_amber',
        name: "Chaos Amber Shard",
        description: "A pulsating fragment of wild magic that distorts time and space around the wielder. When touched, it grants temporary immunity to time loops and causes nearby enemies to suffer chaotic confusion.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 790,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 3 rounds of immunity to time loops",
        "Inflicts 1d4 confusion damage to enemies",
        "Causes minor temporal distortion (1d6 seconds of time delay) on next action"
        ],
        vendor: 'feywild',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'feywild_chaos_bloom': {
        id: 'feywild_chaos_bloom',
        name: "Chaos Bloom of the Shifting Veil",
        description: "A pulsating, iridescent flower that blooms with wild magic. When touched, it grants temporary chaos immunity and causes nearby enemies to suffer unpredictable magical backlash.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 790,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants immunity to chaos damage for 10 minutes",
        "Triggers random magical backlash on enemies within 30 feet for 5 seconds",
        "Causes temporary confusion to targets (1d4 rounds)"
        ],
        vendor: 'feywild',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'feywild_chaos_bottle': {
        id: 'feywild_chaos_bottle',
        name: "Chaos Bottle",
        description: "A glass vessel that collects wild magic surges from the Feywild. When shaken, it releases a burst of chaotic energy that can disrupt enemy spells or create a temporary time bubble. Use wisely—may cause minor hallucinations.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 230,
        icon: '🌿',
        stock: 2,
        rarity: 'uncommon',
        effects: [
            "Shaken: Disrupts one enemy spell for 1 round",
        "Creates 30-second time bubble where time moves at 1x speed",
        "1d4 chance to trigger minor hallucination (see: “You see a version of yourself from the past” or “You hear the Oracle’s voice”)"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 4
    },

  'feywild_chaos_cloak': {
        id: 'feywild_chaos_cloak',
        name: "Chaos Cloak of the Wild Unbound",
        description: "Woven from the laughter of forgotten fey courts, this cloak distorts perception and grants the wearer immunity to time loops—until they choose to break them. Glows with erratic patterns that mimic the shifting nature of the Feywild.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants immunity to time loop effects",
        "Randomly alters movement speed and direction (once per round)",
        "Unfurling the cloak triggers a 10% chance to cause nearby enemies to become disoriented"
        ],
        vendor: 'feywild',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'feywild_chaos_cloak_of_echoes': {
        id: 'feywild_chaos_cloak_of_echoes',
        name: "Chaos Cloak of Echoes",
        description: "A cloak that shimmers with wild magic, allowing the wearer to step into a looping echo of their own decisions. Wearing it for more than 1 round causes the wearer to hear their own voice from alternate timelines.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '🧭',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants the wearer temporary immunity to time-based traps",
        "Each round of wear, the wearer hears a version of their own past decisions",
        "If worn during combat, the wearer may reroll one attack roll per round"
        ],
        vendor: 'feywild',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'feywild_chaos_fang': {
        id: 'feywild_chaos_fang',
        name: "Chaos Fang",
        description: "A jagged fang of wild magic that deals extra damage to foes with unpredictable effects. On hit, it may grant a temporary ability, summon a fey creature, or cause the target to teleport.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '🦷',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Extra damage on hit",
        "Random effect: ability grant / fey summon / teleport",
        "May cause minor disorientation for 1 round"
        ],
        vendor: 'feywild',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'feywild_chaos_feywhistle': {
        id: 'feywild_chaos_feywhistle',
        name: "Chaos Feywhistle of the Unbound Court",
        description: "A crystal-tipped whistle that emits a sound capable of unraveling reality’s threads. The more you blow, the more chaotic the world becomes around you.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 790,
        icon: '🎶',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants +3 to wild magic damage rolls",
        "Triggers a 1-round magical distortion zone (all enemies within must roll to avoid confusion or misstep)",
        "Causes temporary magical fatigue for 1 round after use"
        ],
        vendor: 'feywild',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 6
    },

  'feywild_chaos_feywild_item_name': {
        id: 'feywild_chaos_feywild_item_name',
        name: "Wildfire Tassel",
        description: "A tassel of chaotic energy that pulses with Feywild magic. When shaken, it releases a burst of wild magic that affects all nearby enemies—causing them to make a random action. Use it to break enemy focus or disrupt their spellcasting.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 790,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Wild Magic Burst: All enemies within 5m must make a random action (roll 1d6)",
        "Random Action: Could be a spell, move, or fall",
        "Duration: 1 round, then dissipates"
        ],
        vendor: 'feywild',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'feywild_chaos_flicker': {
        id: 'feywild_chaos_flicker',
        name: "Chaos Flicker",
        description: "A shimmering amulet that distorts time and space, causing brief reality loops. Wears off after 3 uses or if the wearer is struck by a non-fey blade.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Triggers a 10 second time loop where the wearer can re-roll one action",
        "Randomly alters the environment for 3 rounds (e.g. gravity flips, color shifts, terrain warps)",
        "If worn during a battle, enemy actions are mirrored back to them for 1 round"
        ],
        vendor: 'feywild',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'feywild_chaos_gaze_lens': {
        id: 'feywild_chaos_gaze_lens',
        name: "Chaos Gaze Lens",
        description: "A cracked, obsidian lens that allows the user to see the true nature of time loops and hidden Fey paths. When used, it reveals one hidden path or event in the Feywild.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 790,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reveals one hidden path or event within 100 yards",
        "Causes temporary confusion for 1d4 rounds if the revealed event is dangerous",
        "Grants +1d4 to Perception checks against Fey magic illusions"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_chaos_heart': {
        id: 'feywild_chaos_heart',
        name: "Chaos Heart Pendant",
        description: "A pulsating ruby pendant that radiates wild magic energy, granting the wearer immunity to time-based effects and the ability to manipulate the flow of time. When worn, the pendant causes minor time distortions around the wearer, making it ideal for escaping unstable areas.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Time Immunity: Immune to time-based effects for 10 seconds",
        "Chaos Surge: 15% chance to trigger a 2-second time stutter",
        "Can be worn by only one character"
        ],
        vendor: 'feywild',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'feywild_chaos_ink': {
        id: 'feywild_chaos_ink',
        name: "Chaos Ink of the Whispering Court",
        description: "This ink drips with wild magic and leaves trails that warp perception—painting illusions that linger for hours. When applied to parchment, it reveals hidden truths or forgotten memories.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 790,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Applies a temporary illusionary layer to surfaces for 1 hour",
        "Reveals hidden lore or memories when written on",
        "Causes brief time-looping effects if touched by a mortal with unbound magic"
        ],
        vendor: 'feywild',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'feywild_chaos_satchel': {
        id: 'feywild_chaos_satchel',
        name: "Chaos Satchel of the Unbound Court",
        description: "A bag stitched with wild magic threads, this satchel absorbs time fractures and echoes. When opened, it emits a soft hum that stabilizes nearby chaos, but at the cost of the user’s next action being unpredictable for 10 seconds.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5700,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Absorbs minor time fractures within 10ft",
        "Random action delay (10 sec) after opening",
        "Glows faintly during time loops or Fey magic use"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_chaos_scribe_book': {
        id: 'feywild_chaos_scribe_book',
        name: "Chaos Scribe Book",
        description: "A cursed tome that writes its own rules. When opened, it reveals a random Feywild rule — perhaps a time loop, a spell reversal, or a court’s hidden law. Use with caution.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 790,
        icon: '📖',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Randomly alters one rule per use — e.g. “time passes 10x faster”",
        "Can rewrite one spell’s effect for 1 round",
        "Cannot be used in non-Feywild zones"
        ],
        vendor: 'feywild',
        shippedBy: 'Koopa Postal',
        levelRequirement: 6
    },

  'feywild_chaos_silk_umbrella': {
        id: 'feywild_chaos_silk_umbrella',
        name: "Chaos Silk Umbrella",
        description: "This umbrella shimmers with wild magic patterns and shields the wearer from temporal distortions. When opened, it creates a 30-foot radius bubble where time loops reset to the moment of opening.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants immunity to time loop traps",
        "Causes 10% chance of random effect on next action",
        "If dropped, spawns a fey court illusion for 5 seconds"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_chaos_talisman': {
        id: 'feywild_chaos_talisman',
        name: "Chaos Talisman of the Shifting Veil",
        description: "A shimmering amulet that pulses with wild magic, granting the wearer temporary control over temporal anomalies. When worn, it allows the user to briefly destabilize reality, creating loops or voids where time behaves erratically.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Temporarily alters time perception within 30 feet (1 round)",
        "Can be used to create a 10-foot-radius time loop for 1 minute",
        "Wears off after 10 minutes of use or if struck by a Fey court spell"
        ],
        vendor: 'feywild',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'feywild_chaos_talisman_7': {
        id: 'feywild_chaos_talisman_7',
        name: "Chaos Talisman 7",
        description: "A jagged relic carved from the bones of a forgotten Fey god. When worn, it grants the wearer the ability to summon a minor temporal vortex that shatters time perception for 3 seconds. Useful for breaking illusions or escaping traps.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 790,
        icon: '📦',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Summons a 3-second temporal vortex",
        "All enemies in the area suffer 1d6 confusion damage",
        "Causes 1d4 seconds of time dilation for the wearer"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_chaos_tapestry': {
        id: 'feywild_chaos_tapestry',
        name: "Chaos Tapestry of the Wild",
        description: "A living tapestry woven from the threads of wild magic. It shifts its patterns to reflect the user’s deepest fears and desires, allowing them to step into the Feywild’s most volatile corners.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 790,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants temporary wild magic channeling (1d6 charges)",
        "Causes the wearer to be entangled in Feywild echoes (1 round)",
        "May trigger a random time-loop event upon use"
        ],
        vendor: 'feywild',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'feywild_court_echo_rod': {
        id: 'feywild_court_echo_rod',
        name: "Court Echo Rod of the Whispering Throne",
        description: "This rod hums with the power of the Fey Courts’ oldest decrees. When swung, it echoes a court’s current mood—granting temporary buffs or forcing enemies into fey madness. Use it to command the wild or unravel the loops.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants +1 to all attack rolls vs. fey creatures",
        "Causes enemies within 30 feet to suffer 1d6 confusion damage",
        "May summon a court’s spirit for 1 round if wielded correctly"
        ],
        vendor: 'feywild',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'feywild_court_soul_fragments': {
        id: 'feywild_court_soul_fragments',
        name: "Court Soul Fragments of the Whispering Thorns",
        description: "These enchanted shards whisper secrets of the Fey courts and grant temporary charm resistance. Each fragment glows with a different court’s essence, causing minor reality shifts when wielded.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 790,
        icon: '🌿',
        stock: 5,
        rarity: 'rare',
        effects: [
            "Grants +2 charm resistance for 3 rounds",
        "Triggers a random Fey court memory upon consumption (can be beneficial or chaotic)",
        "Causes minor confusion if consumed during a time loop"
        ],
        vendor: 'feywild',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'feywild_dancing_shards': {
        id: 'feywild_dancing_shards',
        name: "Dancing Shards of the Courtly Hourglass",
        description: "A set of three crystalline shards that shift shape when touched, reflecting the current courtly time loop. When thrown, they create a 10-foot-radius bubble where time slows, allowing escape from loops or granting a chance to break them.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Creates time bubble (10ft radius) with 10% chance to break loop",
        "Shards change form based on current loop phase",
        "Wears off after 30 minutes or after 3 uses"
        ],
        vendor: 'feywild',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'feywild_dream_mote': {
        id: 'feywild_dream_mote',
        name: "Dream Mote of the Fractured Atrium",
        description: "A glowing mote that allows the user to enter a brief dream-state, seeing glimpses of the Feywild’s hidden truths. May reveal secrets or cause temporary disorientation.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 800,
        icon: '🌌',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Enter dream state for 30 seconds",
        "See hidden lore or forgotten memories",
        "15% chance to lose 1 action point"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_dreaming_shroud': {
        id: 'feywild_dreaming_shroud',
        name: "Dreaming Shroud of the Wild Echo",
        description: "A shroud woven from the dreams of the Feywild’s forgotten kings. When draped over the body, it allows the wearer to enter a dream loop where time flows in reverse and memories are mutable. Beware: you may wake up remembering something… that never happened.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 800,
        icon: '🌌',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Enter dream loop for 10 minutes: time flows backward, memory can be rewritten",
        "Roll 1d6: 1–2 = gain temporary advantage in combat, 3–6 = risk losing 1 memory",
        "Shroud fades after 1 hour, or if wearer wakes in a non-dream state"
        ],
        vendor: 'feywild',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'feywild_echo_chime': {
        id: 'feywild_echo_chime',
        name: "Echo Chime of the Feywild’s Lullaby",
        description: "A crystal chime that hums with the memories of forgotten Fey courts. When played, it echoes a moment from the past—allowing the user to relive it. However, it may trigger a memory that the user does not want to remember.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 800,
        icon: '🎵',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Relives a moment from the past (30 seconds)",
        "May trigger a memory that the user cannot escape",
        "Grants temporary immunity to fear-based spells (1 minute)"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_echo_gauntlet': {
        id: 'feywild_echo_gauntlet',
        name: "Echo Gauntlet of the Fey Courts",
        description: "A gauntlet forged from the laughter of forgotten courts. When worn, it echoes the last spoken word of the wearer to a nearby Fey entity, granting a chance to influence their actions. Use wisely—echoes can be mistaken for truth or lies.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 800,
        icon: '📦',
        stock: 1,
        rarity: 'rare',
        effects: [
            "After speaking, roll 1d4: 1–2 = influence a nearby Fey creature’s behavior",
        "3–4 = create a 10ft illusion of the spoken word",
        "Wears out after 5 uses, requiring reforge by a Fey scribe"
        ],
        vendor: 'feywild',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'feywild_echo_heart': {
        id: 'feywild_echo_heart',
        name: "Echo Heart",
        description: "A pulsing, crystalline heart that echoes the last thought of a Fey who fell into time. Wearing it grants the ability to hear the thoughts of those trapped in loops—though it may echo the same thought endlessly.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 800,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reveals the last thought of a trapped Fey for 1 minute",
        "Can be used to break a time loop if the thought is repeated",
        "Causes hallucinations if used more than 3 times per day"
        ],
        vendor: 'feywild',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'feywild_echo_of_the_soul': {
        id: 'feywild_echo_of_the_soul',
        name: "Echo of the Soul",
        description: "A whispering bell that chimes when the wearer is near a time loop. It echoes the last thought of the last person who touched it, granting a psychic boost to the user for 1 round.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 800,
        icon: '🎵',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants +2d6 to one skill check per round for 1 round",
        "Causes minor disorientation if used in a non-Feywild location",
        "Echoes the last thought of the last person who touched it (may be a memory, a warning, or a wish)"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_echo_satchel': {
        id: 'feywild_echo_satchel',
        name: "Echo Satchel",
        description: "A pouch that echoes the last 3 actions of the user, allowing them to replay a move or shout in the Feywild. Use it twice before recharging.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 230,
        icon: '🎵',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "Replays last 3 actions (can be used in combat or social)",
        "Triggers a minor Fey echo effect (1d4 damage or 1d4 healing)",
        "+1 to Diplomacy rolls when interacting with Fey entities"
        ],
        vendor: 'feywild',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 4
    },

  'feywild_echo_scepter_of_chaos': {
        id: 'feywild_echo_scepter_of_chaos',
        name: "Echo Scepter of Chaos",
        description: "A scepter that echoes the last thought of the wielder, then replays it as a wild spell. Useful for fey courts or time-warping ambushes.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4700,
        icon: '🌀',
        stock: 1,
        rarity: 'epic',
        effects: [
            "Effect 1: Repeats the last spell cast by the wielder as a wild effect",
        "Effect 2: Grants +3 to all wild magic checks",
        "Effect 3: Triggers a 5-second time-loop for all enemies within 10 feet"
        ],
        vendor: 'feywild',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 6
    },

  'feywild_echoed_gaze': {
        id: 'feywild_echoed_gaze',
        name: "Echoed Gaze Lens",
        description: "A pair of lenses that allow the wearer to see through the veil of time — not the past, but the *next* possible moment. Useful in loops, but may cause disorientation or temporary memory loss.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 800,
        icon: '🧠',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Reveals the next 5 seconds of time (if not in a loop)",
        "Causes 1 round of temporal disorientation (can’t act)",
        "Grants +2 to Intimidation checks against time-warping foes"
        ],
        vendor: 'feywild',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'feywild_echoing_bell': {
        id: 'feywild_echoing_bell',
        name: "Echoing Bell of the Shifting Courts",
        description: "A bell that rings with the voice of a lost Fey court. When struck, it echoes a random court’s rule, granting temporary buffs or curses based on whimsy.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 800,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Upon use: grants +2 to one stat or -2 to another for 1 round",
        "Echoes a random court rule (e.g. “All allies must take 1d4 damage to gain a free action”)",
        "If used during a time loop, the echo lasts for 2 rounds"
        ],
        vendor: 'feywild',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'feywild_echoing_satchel': {
        id: 'feywild_echoing_satchel',
        name: "Echoing Satchel",
        description: "A satchel that whispers the last memory of every item it touches. If the satchel is used to carry an item, the item’s final memory is revealed upon opening. Can be used to recover lost memories or hidden items in the Feywild.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 800,
        icon: '📦',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Reveals the last memory of any item carried",
        "Memory is revealed only upon opening the satchel",
        "May cause hallucinations if memory is too vivid",
        "Cannot be used with non-Feywild items"
        ],
        vendor: 'feywild',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'feywild_entrancing_lore_satchel': {
        id: 'feywild_entrancing_lore_satchel',
        name: "Entrancing Lore Satchel",
        description: "This satchel whispers forgotten stories to those who carry it, revealing hidden truths and shifting realities. When opened, it emits a soft hum that causes nearby Fey to react with curiosity or chaos.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 800,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+2d6 Lore Knowledge when using spells or actions involving memory or history",
        "When opened in a Fey court, causes a 1d6 second time loop in the surrounding area",
        "Grants a 10% chance to trigger a random Fey court event on interaction"
        ],
        vendor: 'feywild',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

};
