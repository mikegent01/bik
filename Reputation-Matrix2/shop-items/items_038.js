// Shop items chunk 38 of 51
// Items 3701 to 3800 (100 items)
// Auto-generated: 2026-03-21 13:41:41

import { SHOP_CATEGORIES } from './categories.js';

export const ITEMS_038 = {
  'sarasaland_oracle_vessel_core': {
        id: 'sarasaland_oracle_vessel_core',
        name: "Oracle Core Fragment",
        description: "A pulsating core of necromantic power",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 35000,
        icon: '⚔',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Grants control over undead minions",
        "Increases spellcasting speed by 50%",
        "Causes a 20% chance of triggering a curse",
        "Made by: Skaven",
        "Allows summoning of the Oracle for 3 turns",
        "Requires 5000 mana to activate and lasts 2 turns"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Noki Coral Fleet',
        levelRequirement: 10
    },

  'sarasaland_pony_nobility_coat': {
        id: 'sarasaland_pony_nobility_coat',
        name: "Pony Nobility Coat of Changeling Grace",
        description: "Woven from the dreams of Equestrian nobles and the whispers of changeling magic, this coat grants the wearer the ability to shift their appearance subtly—perfect for diplomatic missions or stealthy negotiations. But every time it is worn, it drains a small portion of the wearer’s emotional stability, and the coat may begin to reflect the wearer’s inner fears. The Koopa Troop claimed it was “a gift from a fallen pony princess” during the Ricco Harbor siege.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 35000,
        icon: '🐴',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Grants +1 to diplomacy checks and persuasion rolls",
        "Can change appearance for 1 round (no visible effect to others)",
        "Wearer’s emotions become more visible to allies for 1 round",
        "Wearer’s emotions may trigger minor hallucinations",
        "Grants temporary invisibility to enemies if wearer is emotionally neutral",
        "Can be worn only by characters with a “true heart” trait (no stat check)"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Koopa Postal',
        levelRequirement: 10
    },

  'sarasaland_rogueport_courier_key': {
        id: 'sarasaland_rogueport_courier_key',
        name: "Boo Spectral Mail Courier Pack",
        description: "A cursed package that delivers messages through spectral mail",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 440,
        icon: '📦',
        stock: 1,
        rarity: 'uncommon',
        effects: [
            "Delivers messages across worlds instantly",
        "Increases chance of encountering Rakasha clans",
        "Destroys all items left behind after delivery",
        "Made by: Asgard"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 3
    },

  'sarasaland_sandglass_tome': {
        id: 'sarasaland_sandglass_tome',
        name: "Sandglass Tome",
        description: "Bound in desert sandstone and etched with glyphs that shift like dunes, this tome holds the memories of forgotten desert kingdoms. Reading its pages grants glimpses of lost lands—but each page read erases a fragment of the reader’s own memory. The Koopa Troop smuggled it across the desert under the guise of a “historical artifact” to prevent its discovery by the Changeling Hive.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 440,
        icon: '📜',
        stock: 1,
        rarity: 'uncommon',
        effects: [
            "Restores 1d4 temporary hit points when read",
        "Causes memory loss for 1 round after reading",
        "Grants temporary vision of hidden paths for 1 round",
        "May trigger a random desert lore effect (e.g. “see a mirage of a lost oasis”)",
        "Can be read by any character, no attunement required",
        "Contains one randomly selected desert lore fact (e.g. “The sands remember every step taken”)"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Koopa Postal',
        levelRequirement: 4
    },

  'sarasaland_scam_puffball_saucer': {
        id: 'sarasaland_scam_puffball_saucer',
        name: "Puffball Saucer",
        description: "A shimmering, candy-colored saucer made from Dream Land’s most volatile puffballs. Claims to “feed your soul with delicious dreams” and “transform your hunger into joy.” In reality, it only emits harmless laughter and has no effect on reality. Sold as a “gift from the Dream Land Embassy,” it’s the most expensive fake item in Sarasaland’s market. Delivery times are unpredictable — you’ll get it, or you won’t. Either way, you’re not paying for the dreams.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 36000,
        icon: '🍬',
        stock: 0,
        rarity: 'legendary',
        effects: [
            "No effect on the player",
        "Causes 1d6 rounds of euphoria followed by 1d6 rounds of confusion",
        "50% chance to trigger a hallucinatory dream sequence (non-damaging)",
        "Delivered by: Chain Chomp Courier (guaranteed delay)",
        "Wears off after 10 minutes",
        "Claimed to be “real” only if you believe it is",
        "Scam: No actual effect, no memory of purchase, only dreams",
        "Sold by: Changeling Hive (with a wink)"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Chain Chomp Courier',
        levelRequirement: 10
    },

  'sarasaland_shiny_satchel': {
        id: 'sarasaland_shiny_satchel',
        name: "Shiny Satchel",
        description: "A pouch stitched from the fur of a captured Pokémon and adorned with enchanted threads that reflect the user’s current mood. When a Ranger Union agent finds it, they swear the satchel emits a soft glow that matches the trainer’s emotional state—calm, excited, or panicked. The DK Crew claims it was smuggled from a Team Rocket base hidden beneath Raventree Manor’s foundation.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 440,
        icon: '🐉',
        stock: 12,
        rarity: 'uncommon',
        effects: [
            "Increases Pokémon catch rate by 20% for 1 turn",
        "If user is panicked or confused, satchel emits a warning aura",
        "Slight chance to summon a Pokémon spirit for 1 round",
        "Can be used by any trainer or ranger",
        "Wears off after 30 minutes of use",
        "Crafted by: Shy Guy Smugglers"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 3
    },

  'sarasaland_skull_keychain': {
        id: 'sarasaland_skull_keychain',
        name: "Skull Keychain of the Fated Place",
        description: "",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 2000,
        icon: '🦴',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants a 5% chance to uncover hidden paths in Fated Place",
        "Causes minor paranoia if worn near Skaven",
        "Slightly increases stealth while moving through desert ruins",
        "Unlocks secret doors in the Desert of Whispers",
        "Worn by those who remember the lost war",
        "Emits a low hum when proximity to chaos gods is detected"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 2
    },

  'sarasaland_sunken_archives_parchment': {
        id: 'sarasaland_sunken_archives_parchment',
        name: "Sunken Archives Parchment",
        description: "A single page from the lost pre-Peach texts, unearthed beneath the waves of Isle Delfino. It glows faintly with ancient glyphs that shift when read aloud, revealing forgotten treaties and forbidden rituals of the old world.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 2000,
        icon: '📜',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Reveals one hidden lore fact when read",
        "Causes temporary memory overload if read twice",
        "Can be used to decode ancient runes in scrolls",
        "Slightly increases spellcasting speed for spellcasters",
        "Crafted by: Land Mordor"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Pipe Express',
        levelRequirement: 4
    },

  'sarasaland_teyvat_linker': {
        id: 'sarasaland_teyvat_linker',
        name: "Interdimensional Courier Lens",
        description: "A device that connects Sarasaland with Teyvat through elemental resonance",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 7000,
        icon: '🌊',
        stock: 3,
        rarity: 'epic',
        effects: [
            "Allows teleportation between Sarasaland and Teyvat",
        "Triggers elemental reaction effects on contact",
        "Causes memory fragmentation during transit",
        "Made by: Moonfang Pack"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Warp Whistle Transit',
        levelRequirement: 10
    },

  'sarasaland_wario_land_crossover': {
        id: 'sarasaland_wario_land_crossover',
        name: "Garlic Power Amulet",
        description: "A relic that fuels courage with the scent of Wario’s garlic",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 440,
        icon: '🍄',
        stock: 10,
        rarity: 'uncommon',
        effects: [
            "Boosts attack power for 3 turns",
        "Causes temporary blindness to enemies",
        "Reduces movement speed by 20%",
        "Made by: Skaven",
        "Grants immunity to poison",
        "Consuming too much causes nausea"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Noki Coral Fleet',
        levelRequirement: 3
    },

  'sarasaland_wario_troop_brooch': {
        id: 'sarasaland_wario_troop_brooch',
        name: "Wario Troop Brooch",
        description: "A golden brooch shaped like a smiling Wario head, stitched with enchanted thread that glows when laughter is heard nearby. The vendor claims it brings luck to the WarioWare branches—but one brave merchant who wore it during the Sandstorm Summit got turned into a dune rat.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 36000,
        icon: '🎭',
        stock: 0,
        rarity: 'legendary',
        effects: [
            "Grants +2 to morale rolls for allies",
        "Causes 5% chance to trigger a comedic failure effect",
        "Grants 10% chance to summon a random WarioWare joke on enemy",
        "Cannot be worn by spellcasters or clerics",
        "Causes minor mental stress if worn for more than 30 minutes",
        "Crafted by: Land Mordor"
        ],
        vendor: 'sarasaland',
        shippedBy: 'Pipe Express',
        levelRequirement: 7
    },

  'scylla_charybdis_tentacle_boots': {
    id: 'scylla_charybdis_tentacle_boots',
    name: "Scylla Charybdis Tentacle Boots ",
    description: "Boots with suction-cup soles for gripping.",
    category: SHOP_CATEGORIES.premium,
    price: 440,
    icon: '👢',
    stock: 4,
    rarity: 'uncommon',
    effects: [
        "Advantage on Athletics for climbing wet surfaces",
        "Boots stick to floors (hard to remove)",
        "Suction pops loudly",
        "Made by: Sea Monster Crafters"
    ],
    vendor: 'scylla_charybdis_cave',
    shippedBy: 'Ink-Stained Soles',
    levelRequirement: 5
},

  'scylla_charybdis_tentacle_grip_service': {
    id: 'scylla_charybdis_tentacle_grip_service',
    name: "Scylla Charybdis Tentacle Grip Service",
    description: "Enhance climbing with mythical adhesive.",
    category: SHOP_CATEGORIES.services,
    price: 15000,
    icon: '🐙',
    stock: 5,
    rarity: 'uncommon',
    effects: [
        "Advantage on Athletics for climbing for 24 hours",
        "Grip 1d4 small items without dropping",
        "Slippery residue: Disadvantage on weapon grips",
        "Made by: Sea Monster Handlers"
    ],
    vendor: 'scylla_charybdis_cove',
    shippedBy: 'Ink Bottle',
    levelRequirement: 5
},

  'scylla_charybdis_tentacle_tempura_recipe': {
    id: 'scylla_charybdis_tentacle_tempura_recipe',
    name: "Recipe: Scylla Charybdis Tentacle Tempura ",
    description: "Crispy fried 'seafood' for monstrous munchies.",
    category: SHOP_CATEGORIES.services,
    price: 7000,
    icon: '🍤',
    stock: 7,
    rarity: 'uncommon',
    effects: [
        "Teaches recipe: Fry tentacles (20 min); eat to gain advantage on grapples for 1 hour and 1d4 temporary HP",
        "Requires: Tentacles (or veggies); crunchy",
        "Monsters might recognize the taste",
        "Made by: Sea Chefs"
    ],
    vendor: 'scylla_charybdis_cove',
    shippedBy: 'Ink-Fried Instructions',
    levelRequirement: 5
},

  'scylla_charybdis_tentacle_tenderizer': {
    id: 'scylla_charybdis_tentacle_tenderizer',
    name: "Scylla Charybdis Tentacle Tenderizer ",
    description: "Hammer for pounding tough 'tentacles' into tempura.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 6000,
    icon: '🍤',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Tenderizes meat; advantage on cooking tough ingredients",
        "Suction grip for secure pounding",
        "Sticky – hard to clean",
        "Made by: Sea Hammerers"
    ],
    vendor: 'scylla_charybdis_cove',
    shippedBy: 'Ink Impact Iron',
    levelRequirement: 5
},

  'scylla_charybdis_tide_chart': {
    id: 'scylla_charybdis_tide_chart',
    name: "Scylla & Charybdis Tide Chart",
    description: "A nautical chart that perfectly predicts the movements of the legendary sea monsters and the tides around them.",
    category: SHOP_CATEGORIES.CURIOSITIES,
    price: 13000,
    icon: '🗺️',
    stock: 2,
    rarity: 'epic',
    effects: [
        "Grants a safe path through the monster's strait once per week",
        "While in the area, you cannot be surprised by the monsters",
        "Chart is cursed: you hear their whispers in your sleep"
    ],
    vendor: 'Scylla Charybdis',
    shippedBy: 'Waterproof Tube',
    levelRequirement: 7
},

  'scylla_charybdis_tracker': {
        id: 'scylla_charybdis_tracker',
        name: "Scylla/Charybdis Proximity Charm",
        description: "A charm that vibrates violently when dangerous whirlpools or sea beasts are near.",
        category: SHOP_CATEGORIES.equipment,
        price: 15000,
        icon: '🔱',
        stock: 8,
        rarity: 'rare',
        effects: [
            "Warns user of dangerous currents or sea monsters (100ft range)",
        "Advantage on water-based navigation checks",
            "Made by: Scylla Charybdis"
        ],
        vendor: 'oceanic_trader',
        shippedBy: 'Sealed Shell',
        levelRequirement: 4
    },

  'see_flies_or_fly_like': {
    id: 'see_flies_or_fly_like',
    name: "Fawful's Furious Fly Goggle ",
    description: "See flies... or fly-like.",
    category: SHOP_CATEGORIES.premium,
    price: 440,
    icon: '🕶️',
    stock: 8,
    rarity: 'uncommon',
    effects: [
        "Detect insects within 30ft",
        "Once per day: Jump +10ft as if flying briefly",
        "Goggles buzz annoyingly",
        "Made by: Fawful's Lab"
    ],
    vendor: 'fawfuls_furious_workshop',
    shippedBy: 'Buzzing Box',
    levelRequirement: 5
},

  'sending_stone_customer_service': {
    id: 'sending_stone_customer_service',
    name: "Sending Stone (With Customer Service)",
    description: "A pair of stones that let you chat, with 24/7 support.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 13200,
    icon: '🪨',
    stock: 8,
    rarity: 'rare',
    effects: [
        "As normal Sending Stones, but with 3/day usage",
        "Customer service stone answers 1 rules question per day (maybe correctly)",
        "10% chance to be upsold additional services mid-conversation",
        "Made by: Arcane Communications LLC"
    ],
    vendor: 'magic_telecom',
    shippedBy: 'Same-day Delivery',
    levelRequirement: 5
},

  'servants_cosmic_astronaut_suit': {
    id: 'servants_cosmic_astronaut_suit',
    name: "Servants Cosmic Astronaut Suit",
    description: "Space suit for planar travel.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 22500,
    icon: '🧑‍🚀',
    stock: 2,
    rarity: 'rare',
    effects: [
        "Immune to vacuum and planar environmental effects for 7 days",
        "Suit is bulky: disadvantage on Dexterity checks",
        "Helmet fogs up when you lie (disadvantage on Deception)",
        "Made by: Servants Cosmic Space Program"
    ],
    vendor: 'servants_cosmic',
    shippedBy: 'Meteor Mail',
    levelRequirement: 7
},

  'servants_cosmic_blessing': {
    id: 'servants_cosmic_blessing',
    name: "Blessing of a Cosmic Servant",
    description: "A minor boon bestowed by a powerful entity from beyond the stars. It feels... vast.",
    category: SHOP_CATEGORIES.PREMIUM,
    price: 80000,
    icon: '🌌',
    stock: 2,
    rarity: 'legendary',
    effects: [
        "Choose one ability score: increase it by 2 (to a maximum of 22)",
        "You gain resistance to Psychic damage",
        "Once per long rest: ask one question to a cosmic entity (DM provides a cryptic answer)",
        "You may now be on the radar of other cosmic beings"
    ],
    vendor: 'Servants Cosmic',
    shippedBy: 'Astral Projection',
    levelRequirement: 12
},

  'servants_cosmic_horoscope_accuracy': {
    id: 'servants_cosmic_horoscope_accuracy',
    name: "Servants Cosmic Horoscope Accuracy Guarantee",
    description: "Your horoscope is actually correct for once.",
    category: SHOP_CATEGORIES.SERVICES,
    price: 12500,
    icon: '♈',
    stock: 7,
    rarity: 'uncommon',
    effects: [
        "Once per day: Your horoscope gives you a real clue about the next encounter (DM hint)",
        "Capricorns get extra gold; Leos get extra attention; Geminis get two conflicting hints",
        "Pisces are told they're special (they're not)",
        "Made by: Servants Cosmic Astrology Dept."
    ],
    vendor: 'servants_cosmic',
    shippedBy: 'Star Chart Delivery',
    levelRequirement: 5
},

  'servants_cosmic_horoscope_accuracy_upgrade': {
    id: 'servants_cosmic_horoscope_accuracy_upgrade',
    name: "Servants Cosmic Horoscope Accuracy Upgrade",
    description: "Your horoscope is now 90% accurate.",
    category: SHOP_CATEGORIES.PREMIUM,
    price: 16500,
    icon: '🔮',
    stock: 3,
    rarity: 'rare',
    effects: [
        "Horoscope gives you specific advantage on one check per day for 7 days",
        "You become superstitious: disadvantage if you don't follow horoscope advice",
        "Stars judge you silently",
        "Made by: Servants Cosmic Premium"
    ],
    vendor: 'servants_cosmic',
    shippedBy: 'Star Chart Express',
    levelRequirement: 6
},

  'servants_cosmic_recipe_starlight_soup': {
    id: 'servants_cosmic_recipe_starlight_soup',
    name: "Recipe: Servants Cosmic Starlight Soup",
    description: "Soup made from starlight (somehow).",
    category: SHOP_CATEGORIES.RECIPE,
    price: 16500,
    icon: '📜',
    stock: 3,
    rarity: 'rare',
    effects: [
        "Grants advantage on saves vs. psychic damage for 8 hours",
        "You glow faintly: disadvantage on Stealth, advantage on Charisma with celestial beings",
        "Soup is intangible (you drink it with your mind)",
        "Made by: Servants Cosmic Kitchen"
    ],
    vendor: 'servants_cosmic',
    shippedBy: 'Starlight Beam',
    levelRequirement: 7
},

  'servants_cosmic_star_chart': {
    id: 'servants_cosmic_star_chart',
    name: "Servants Cosmic Star Chart",
    description: "A map of constellations that predict cosmic events.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 20500,
    icon: '🗺️',
    stock: 2,
    rarity: 'rare',
    effects: [
        "Once per long rest: Predict weather for next 24 hours with 80% accuracy",
        "During meteor showers: Gain advantage on one attack roll",
        "Unreadable during the day – just looks like a coffee stain",
        "Made by: Celestial Bureaucracy"
    ],
    vendor: 'servants_cosmic_archive',
    shippedBy: 'Meteor Mail',
    levelRequirement: 6
},

  'shadeward_estate_': {
        id: 'shadeward_estate_',
        name: "Obsidian Veil Cloak",
        description: "A cloak that shrouds the wearer in a shadowy aura, making them unrecognizable to rival vampires. When worn at a dinner, it allows the wearer to slip unnoticed into the inner sanctum — or perhaps into a trap, if they are not careful.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Detect hidden vampire sigils within 10 meters",
        "Grants +2 to Diplomacy checks when interacting with nobles",
        "Inflicts -1 to Stealth checks if detected by rival vampires",
        "Inflicts -2 to Perception checks when the target is unaware of its presence",
        "Reveals hidden agenda when used in a duel (temporary buff for 1 round)",
        "Causes enemies to lose 10% of their charm score after being struck",
        "Reduces tension among allies by 20% for 10 minutes",
        "Immune to trap activation for 1 round when worn",
        "Grants +3 to Persuasion checks during social events",
        "Grants +5 to Stealth checks in dimly lit areas",
        "Reveals hidden agenda when approached by a rival at a party",
        "Reduces chance of being targeted by vampire assassins by 50%"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadeward_estate_black_rose_of_the_onyx_hand': {
        id: 'shadeward_estate_black_rose_of_the_onyx_hand',
        name: "Black Rose of the Onyx Hand",
        description: "A cursed flower that blooms only under the gaze of the highest vampires. Its petals are said to absorb the essence of betrayal, turning your enemies’ lies into your own power.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2000,
        icon: '🔮',
        stock: 0,
        rarity: 'rare',
        effects: [
            "+30% chance to detect hidden enemies during formal dinners",
        "+10% chance to steal secrets from those who speak too much",
        "+25% chance to convert enemy loyalty into temporary allies"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_blood_sword_of_the_broken_court': {
        id: 'shadeward_estate_blood_sword_of_the_broken_court',
        name: "Blood Sword of the Broken Court",
        description: "A ceremonial blade forged from the bones of a fallen noble house. It grants the wearer the ability to cut through social alliances and reveal hidden loyalties. Perfect for those who wish to break the rules of the court.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "- Can reveal hidden alliances or betrayals when used in combat",
        "- Inflicts -2 to Loyalty checks for allies when wielded",
        "- Increases damage by 1 when used in a formal duel"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_court_silence': {
        id: 'shadeward_estate_court_silence',
        name: "Court Silence Cloak",
        description: "A whisper-silenced garment that muffles all conversation during high-stakes banquets. Perfect for infiltrators or those who wish to observe without being noticed. Wears off after 30 minutes of use.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Suppresses ambient sound for 30 minutes",
        "Prevents enemy perception of your presence during events",
        "Cannot be worn while fighting or using magic"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadeward_estate_crimson_dagger': {
        id: 'shadeward_estate_crimson_dagger',
        name: "Crimson Dagger of the Silent Scribe",
        description: "A blade that glimmers with the blood of fallen aristocrats. Its edge is sharp enough to cut through lies, but also leaves a mark that screams “I was here.”",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2000,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Inflicts 10% bonus damage to enemies with lies or false alliances",
        "Leaves a bloodstain that reveals the true identity of the wielder (for 30 seconds)",
        "Grants immunity to being charmed by political flattery"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadeward_estate_crimson_tassel_of_the_vampire_bloodline': {
        id: 'shadeward_estate_crimson_tassel_of_the_vampire_bloodline',
        name: "Crimson Tassel of the Vampire Bloodline",
        description: "A ceremonial tassel that pulses with the lifeblood of the elder vampires. Wearing it grants you the ability to project your will into the shadows, manipulating the atmosphere to your advantage during tense negotiations.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+15% chance to influence NPC behavior during political intrigue",
        "+5% damage to enemies in dimly lit rooms",
        "+30% chance to detect hidden alliances or betrayals"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_dusk_diplomacy': {
        id: 'shadeward_estate_dusk_diplomacy',
        name: "Dusk Diplomacy Ring",
        description: "A cursed ring that allows the wearer to negotiate peace with any vampire lord—even if they are already bound by oaths. Its glow dims during truth-telling, making it perfect for lying through charm.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2000,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Reduces enemy suspicion by 30% during negotiations",
        "Grants a 5% chance to reveal hidden alliances",
        "Requires a formal setting to activate full effect"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadeward_estate_item_name': {
        id: 'shadeward_estate_item_name',
        name: "Mask of the Fractured Atrium",
        description: "A mask that warps the wearer’s perception of time and space, allowing them to “walk through” the unstable corridors of the Atrium without being noticed. Only for the bold.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Temporarily grants immunity to trap effects within the Fractured Atrium",
        "Grants +20% to movement speed in unstable terrain",
        "Reduces detection chance by 50% for 10 turns"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_mantle_of_the_unseen_mist': {
        id: 'shadeward_estate_mantle_of_the_unseen_mist',
        name: "Mantle of the Unseen Mist",
        description: "A cloak woven from the vapor of forgotten noble courts. Wearing it renders the wearer invisible to casual observers, but visible to those who know their true form. Best used for slipping through the shadows of a dinner party.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "- Stealth mode: invisible to non-magical observers for 10 minutes",
        "- Reveals hidden enemies when in proximity to a vampire’s aura",
        "- Grants +1 to Stealth checks in formal settings"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_mistral_hood': {
        id: 'shadeward_estate_mistral_hood',
        name: "Mistral Hood of the Unseen Hand",
        description: "A flowing crimson hood that allows the wearer to blend into the shadows of the estate’s halls. Perfect for those who wish to slip into the throne room without being seen.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants +20% chance to move silently in dimly lit rooms",
        "Reveals the location of hidden chambers (if wearing at night)",
        "Grants immunity to “sneak attack” triggers from nobles"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadeward_estate_shadow_gaze': {
        id: 'shadeward_estate_shadow_gaze',
        name: "Shadow Gaze Lantern",
        description: "Illuminates the darkest corners of a noble’s heart, revealing hidden alliances and betrayals. Best used at night during clandestine dinners. A subtle but lethal tool for the politically astute.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Reveals hidden enemies within 10 meters",
        "Grants +5% to stealth-based social maneuvers",
        "Cannot be used during daylight or in public halls"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadeward_estate_silken_mask_of_the_vaulted_moon': {
        id: 'shadeward_estate_silken_mask_of_the_vaulted_moon',
        name: "Silken Mask of the Vaulted Moon",
        description: "A whispering mask that conceals your true identity while subtly amplifying your charm and poise at high society events. Wears the gaze of the moon to deflect suspicion and lure allies into false alliances.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+10% chance to disarm social suspicion during formal dinners",
        "+5% chance to gain favor from influential nobles when worn",
        "+20% stealth in shadowed ballrooms and private chambers"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_silken_tie_of_silence': {
        id: 'shadeward_estate_silken_tie_of_silence',
        name: "Silken Tie of Silence",
        description: "A velvet tie embroidered with the sigil of the Whispering Court. Wearing it grants temporary silence in social gatherings, allowing the wearer to observe without being heard. Perfect for eavesdropping on political debates or avoiding unwanted attention.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "- Silence all ambient speech within 10 feet for 30 seconds",
        "- Gain +2 to Persuasion checks when negotiating with nobles",
        "- Can be worn only during formal dinners or courtly events"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_silken_torch': {
        id: 'shadeward_estate_silken_torch',
        name: "Silken Torch of the Velvet Throne",
        description: "A flickering torch that casts an eerie, crimson glow, perfect for illuminating the shadows of a vampire’s banquet. Its flame whispers secrets to those who listen closely.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Illuminates dark corners with a vampiric hue",
        "Grants +10% stealth in formal gatherings",
        "Reveals hidden agenda of nearby nobles (on a 50% chance)"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadeward_estate_silver_tongue': {
        id: 'shadeward_estate_silver_tongue',
        name: "Silver Tongue of the Velvet Court",
        description: "Whispers of intrigue into the ears of the noblest, this amulet grants temporary persuasion immunity and the ability to subtly manipulate social loyalties during formal dinners. Ideal for those navigating the treacherous halls of vampire politics.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% chance to disarm political rivals during social events",
        "Grants a 10-second silence effect on enemies during negotiations",
        "Can be worn only at formal events (no combat use)"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadeward_estate_silver_torch_of_the_forgotten_lords': {
        id: 'shadeward_estate_silver_torch_of_the_forgotten_lords',
        name: "Silver Torch of the Forgotten Lords",
        description: "A relic from a long-dead noble house, this torch casts a cold, deceptive glow that reveals hidden truths and secrets buried beneath the estate’s marble floors. Perfect for uncovering lies during dinner conversations.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+20% chance to uncover hidden truths during social events",
        "+10% chance to detect hidden rooms or secret passages",
        "+5% bonus to persuasion checks in high-stakes negotiations"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_vampire_ceremonial_cloak': {
        id: 'shadeward_estate_vampire_ceremonial_cloak',
        name: "Shadowed Regalia Cloak",
        description: "Woven from the essence of a fallen noble’s last breath, this cloak grants invisibility to the wearer during formal gatherings—though it leaves a faint scent of regret.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "50% chance to evade detection in ballrooms",
        "+20% influence gain when wearing in political gatherings",
        "-50% chance of being noticed by a rival vampire when in stealth"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadeward_estate_vampire_dinner_glove': {
        id: 'shadeward_estate_vampire_dinner_glove',
        name: "Crimson Velvet Dinner Glove",
        description: "A whisper-soft glove embroidered with blood-red lace that conceals a hidden blade. Perfect for silencing whispers at the Masquerade Ball or extracting secrets from a rival’s collar.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+10% stealth in formal settings",
        "+5% chance to disarm foes during social events",
        "+30% damage to targets with false identities"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadeward_estate_vampire_silken_mask': {
        id: 'shadeward_estate_vampire_silken_mask',
        name: "The Silent Sovereign Mask",
        description: "A mask carved from the bones of a forgotten duke, it grants the wearer the ability to appear as a different vampire at any formal event—perfect for infiltrating the highest circles.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+25% chance to mimic another vampire’s voice",
        "+10% chance to disarm rivals during dinner",
        "+10% bonus to charm rolls during political gatherings"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadeward_estate_vampire_tincture_of_the_dawn': {
        id: 'shadeward_estate_vampire_tincture_of_the_dawn',
        name: "Vampire Tincture of the Dawn",
        description: "A dark elixir brewed from the blood of a first moon vampire. Consuming it grants temporary immunity to daylight and allows the wearer to move unseen in candlelit halls. Ideal for covert operations during the day.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "- Immunity to daylight effects for 1 hour",
        "- Can detect hidden vampires within 30 feet",
        "- Causes temporary euphoria and heightened empathy for noble minds"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadeward_estate_vampire_tongue_ties': {
        id: 'shadeward_estate_vampire_tongue_ties',
        name: "Velvet Tongue Ties",
        description: "A pair of silk cords that bind a vampire’s tongue to their collarbone, ensuring silence during negotiations and allowing the wearer to whisper secrets without fear of being overheard.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Prevents verbal backlash in high-stakes talks",
        "+15% persuasion in political intrigue",
        "+5% chance to detect lies during social interrogation"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadeward_estate_voicemail_cuff': {
        id: 'shadeward_estate_voicemail_cuff',
        name: "Voicemail Cuff of the Whispering Court",
        description: "A subtle metallic cuff that allows the wearer to eavesdrop on conversations during formal dinners—perfect for those who seek to manipulate the next vote.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Detects whispered plots within 10 feet",
        "Allows 30-second audio recording of conversations",
        "Causes nearby nobles to misinterpret your words (10% chance)"
        ],
        vendor: 'shadeward_estate',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadow_queen_crown': {
        id: 'shadow_queen_crown',
        name: "Crown of Shadows",
        description: "The crown of the ancient demon queen. Grants dominion over darkness.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 500000,
        icon: '👑',
        stock: 1,
        rarity: 'godly',
        effects: [
            "Control any shadow creature",
            "Cover the world in eternal night",
            "Immunity to Non-Magical damage",
            "Possession risk: High"
        ],
        vendor: 'onyx_hand',
        shippedBy: 'Sealed Box',
        levelRequirement: 18
    },

  'shadow_queen_tear': {
        id: 'shadow_queen_tear',
        name: "Shadow Queen's Tear",
        description: "A crystallized tear from the ancient Shadow Queen. Power at a terrible cost.",
        category: SHOP_CATEGORIES.FORBIDDEN,
        price: 36000,
        icon: '🖤',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Permanent: Gain Shadow Step (teleport 30ft through darkness)",
            "Permanent: Darkvision 120ft",
            "Permanent: Sunlight Sensitivity (disadvantage in direct sunlight)",
            "The Shadow Queen is aware of you"
        ],
        vendor: 'onyx_hand',
        shippedBy: 'SEALED - PALACE OF SHADOW ONLY',
        levelRequirement: 15,
        warning: "The Shadow Queen wants it back. She is patient."
    },

  'shadow_realm_assassin_dagger': {
    id: 'shadow_realm_assassin_dagger',
    name: "Shadow Realm Assassin Dagger (Weapon)",
    description: "A dagger that strikes from the shadows, ignoring armor.",
    category: SHOP_CATEGORIES.forbidden,
    price: 36000,
    icon: '🗡️',
    stock: 1,
    rarity: 'legendary',
    effects: [
        "Ignores AC on surprise attacks",
        "Returns to hand via shadow",
        "Made by: Nightblade Guild"
    ],
    vendor: 'assassin_den',
    shippedBy: 'Smoke Veil',
    levelRequirement: 16
},

  'shadow_siren_veil': {
        id: 'shadow_siren_veil',
        name: "Veil of the Three",
        description: "A dark purple veil. Allows you to hide in the floor.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 360000,
        icon: '👒',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Action: Become a shadow on the floor (Invulnerable, cannot attack, speed 30ft)",
            "Advantage on Stealth",
            "Can cast 'Invisibility' 3/day"
        ],
        vendor: 'onyx_hand',
        shippedBy: 'Shadow Mail',
        levelRequirement: 9
    },

  'shadowfell_abyssal_blood': {
        id: 'shadowfell_abyssal_blood',
        name: "Abyssal Blood of the Veiled Throne",
        description: "A vial containing the last drop of blood spilled by the last Sovereign of the Shadowfell. When consumed, it grants temporary power to bend reality’s laws, allowing the drinker to briefly manipulate shadows as if they were flesh.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants +3 to all Stealth checks",
        "Temporarily grants immunity to light-based attacks",
        "Causes a 1d6 damage roll upon contact with any shadowy creature"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shadowfell_ash_of_the_broken_vessel': {
        id: 'shadowfell_ash_of_the_broken_vessel',
        name: "Ash of the Broken Vessel",
        description: "Fragments of a vessel shattered by the Oracle’s curse, now infused with the will of the dead. When worn, it grants the wearer the ability to summon spectral guardians to assist in battle.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+15% armor penetration",
        "Can summon 1 spectral guardian (100% HP, 100% damage, 20% movement speed) for 3 turns",
        "Grants Cursed Vessel buff — reduces enemy HP regeneration by 10%"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadowfell_assassin_guild_membership': {
    id: 'shadowfell_assassin_guild_membership',
    name: "Shadowfell Assassin Guild Membership",
    description: "Lifetime membership to the League of Shadows.",
    category: SHOP_CATEGORIES.faction,
    price: 36000,
    icon: '🗡️',
    stock: 6,
    rarity: 'legendary',
    effects: [
        "Access to guild contracts worth 50,000gp/week",
        "Free resurrection if killed on guild business",
        "Made by: The Hidden Hand"
    ],
    vendor: 'shadow_broker',
    shippedBy: 'Shadow Step',
    levelRequirement: 16,
    factionBonus: { shadow: 150 }
},

  'shadowfell_banana_heart_amber': {
        id: 'shadowfell_banana_heart_amber',
        name: "Banana Heart Amber",
        description: "A crystalline shard shaped like a banana peel, encasing the essence of a forgotten fruit vendor’s last scream before his soul was devoured by the Onyx Hand. Those who wear it feel the sweet terror of the jungle’s hidden past, and gain a taste of the DK Isles’ chaotic joy. It hums faintly with the rhythm of banana hoards and forgotten battles.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 300,
        icon: '🍌',
        stock: 10,
        rarity: 'common',
        effects: [
            "+1 to all luck checks when near DK Isles",
        "-1 to all rolls in combat when wearing during the day",
        "Grants temporary resistance to fear effects for 1 round",
        "Can be used to summon a spectral banana peel ghost",
        "Can be traded for 100 Beanbean coins at Peasley’s Market",
        "Crafted by: Pianta Chuck Express"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pianta Chuck Express',
        levelRequirement: 1
    },

  'shadowfell_black_rime_blade': {
        id: 'shadowfell_black_rime_blade',
        name: "Black Rime Blade",
        description: "A cursed sword forged from frozen Onyx and shadow essence. It strikes with chilling precision and leaves frost runes on the enemy’s skin. Perfect for assassins who thrive in the cold dark.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Deals 120% damage to undead and shadow creatures",
        "Frost runes etched on target for 3 seconds, slowing movement",
        "Inflicts “Shadow Chill” debuff: 10% chance to freeze next attack"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadowfell_blade_of_reverence': {
        id: 'shadowfell_blade_of_reverence',
        name: "Blade of Reverence",
        description: "A ceremonial dagger forged from the soul of a fallen Onyx Hand initiate. It glows faintly with crimson energy and deals extra damage to undead.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Deals +30% damage to undead enemies",
        "Inflicts “Vampiric Resonance” on hits, draining 10% HP from targets for 1 turn",
        "Causes minor corruption to nearby enemies (1d2 HP loss per turn for 3 turns)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 6
    },

  'shadowfell_blade_of_silence': {
        id: 'shadowfell_blade_of_silence',
        name: "Blade of the Silent Executioner",
        description: "A razor-thin blade forged from the shadow of a slain vampire lord. It emits a low hum that disarms foes, and its edge shimmers with stolen memories.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5700,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Disarms target for 2 seconds on strike",
        "Debuffs enemies with “Memory Theft” for 10 seconds",
        "Reflects 10% of damage taken as a counterattack"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 6
    },

  'shadowfell_blade_of_the_void': {
        id: 'shadowfell_blade_of_the_void',
        name: "Blade of the Void",
        description: "A blade forged from the whispers of forgotten shadows, it strikes with silence and leaves foes drained of willpower. Its edge glows with the essence of the Dark Mirror’s deepest abyss.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Deals 150% extra damage to undead",
        "Grants temporary invisibility for 3 seconds after each strike",
        "Reduces enemy regeneration by 50% for 10 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_blood_fang': {
        id: 'shadowfell_blood_fang',
        name: "Fang of the Blood Sovereign",
        description: "A ceremonial fang carved from the jawbone of a vampire that ruled the Onyx Hand’s ancient throne. Grants a chilling aura that suppresses enemy morale and inflicts fear.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Inflicts Fear (targets flee or attack randomly)",
        "+15% damage against undead",
        "Reduces enemy attack speed by 20% for 5 seconds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_blood_inkwell': {
        id: 'shadowfell_blood_inkwell',
        name: "Blood Inkwell of the Forgotten Scribe",
        description: "A vessel of liquid shadow that writes in the language of the dead. Pouring its contents onto parchment summons spectral glyphs that reveal secrets of the past—or doom the scribe who reads them.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reveals hidden lore or trap locations on parchment",
        "Consumes 1 round of reading time to unlock a glyph’s meaning",
        "Risk: If read incorrectly, the user is cursed to forget their own name for 1d4 rounds"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_blood_mirrored_sword': {
        id: 'shadowfell_blood_mirrored_sword',
        name: "Blood Mirrored Sword",
        description: "A blade that reflects the soul of its wielder—each strike echoes a forgotten memory. Deals extra damage to undead. Causes temporary psychic backlash on foes.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+25% damage against undead",
        "+5% chance to paralyze target after 2 consecutive hits",
        "+10% chance to reveal hidden enemy weaknesses"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 7
    },

  'shadowfell_blood_scar_tapestry': {
        id: 'shadowfell_blood_scar_tapestry',
        name: "Blood Scar Tapestry",
        description: "Woven from the remnants of fallen Onyx Hand warriors, this tapestry channels their lingering rage into the wielder’s melee strikes. Wearing it grants a minor aura of bloodlust and increases critical hit chance.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+5% chance to deal extra damage on critical hits",
        "Inflicts 1d4 bleed damage on enemies after 3 seconds of contact",
        "Wears off after 10 minutes of combat exposure"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadowfell_blood_siphon': {
        id: 'shadowfell_blood_siphon',
        name: "Blood Siphon of the Onyx Hand",
        description: "A jagged obsidian vial that drains vitality from foes, leaving them drained and vulnerable. Perfect for assassins who thrive in the dark.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Siphons 10% of target’s HP and converts it to temporary health for the user",
        "Increases stealth duration by 20% while active",
        "Causes a 30% chance to trigger a “Shadow Reveal” debuff on enemies"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_blood_siphon_catalyst': {
        id: 'shadowfell_blood_siphon_catalyst',
        name: "Blood Siphon Catalyst",
        description: "A pulsating black crystal that amplifies vampiric drain power. When activated, it temporarily boosts damage dealt to living targets while siphoning health. Perfect for stealthy assassins or undead commanders.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5600,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Boosts damage dealt by 30%",
        "Siphons 10% of target’s health as bonus damage",
        "Grants 10% increased movement speed while active"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_blooper_wetworks_glass_crimson': {
        id: 'shadowfell_blooper_wetworks_glass_crimson',
        name: "Glass Crimson Lens",
        description: "A fractured lens of crimson glass, shattered from a Void Drifter’s final flight. When focused, it warps reality into a mirror of the user’s deepest fear. It is said to be the last artifact of the Blooper Wetworks, a company that traded in illusions to the Edge. The lens may shatter under pressure, but its power is unmatched.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 7000,
        icon: '🌀',
        stock: 1,
        rarity: 'epic',
        effects: [
            "Grants +2 to Perception checks when detecting illusions",
        "50% chance to reveal the true nature of a foe’s disguise",
        "When used, the wearer is temporarily blinded by their own reflection",
        "Requires attunement with a Void Drifter",
        "Crafted by: Blooper Wetworks",
        "Connects The Shadowfell and The Edge through temporal distortion"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Blooper Wetworks',
        levelRequirement: 7
    },

  'shadowfell_cloak_of_blood_tide': {
        id: 'shadowfell_cloak_of_blood_tide',
        name: "Cloak of Blood Tide",
        description: "Woven from the essence of drowned vampires, this cloak pulses with the rhythm of the drowned. It grants stealth and enhances movement through flooded ruins, while whispering warnings to the wearer of nearby undead.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5600,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 10% movement speed increase through water or shadowed terrain",
        "Stealth bonus in dark or flooded zones (30s duration)",
        "Inflicts 2d4 necrotic damage on nearby undead on touch"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_cloak_of_the_bloody_moon': {
        id: 'shadowfell_cloak_of_the_bloody_moon',
        name: "Cloak of the Bloody Moon",
        description: "A tattered, crimson cloak woven from the moon’s blood and the whispers of the dead. Grants the wearer temporary invulnerability to psychic attacks and grants a haunting presence that draws enemies to the user’s location.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5700,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+20% evasion vs psychic damage",
        "+10% chance to summon a spectral echo (5s)",
        "Inflicts 25% fear on enemies in proximity"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_cloak_of_the_dark_ward': {
        id: 'shadowfell_cloak_of_the_dark_ward',
        name: "Cloak of the Dark Ward",
        description: "A woven shadow cloak that absorbs ambient light and converts it into dark energy, boosting stealth and defense. Wears the soul of a fallen shadow mage.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Gain 10% resistance to light-based attacks",
        "Stealth Mode: Become invisible for 8 seconds",
        "Convert ambient light to dark energy (regenerates 5 vitality per second)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_cloak_of_whispers': {
        id: 'shadowfell_cloak_of_whispers',
        name: "Cloak of Whispers",
        description: "A tattered cloak that absorbs light and sound, worn by those who dare speak truth in the dark. Its fabric pulses with unseen magic, whispering secrets to the listener’s soul. When worn, it muffles the caster’s voice but amplifies their inner thoughts, making them irresistible to foes who seek to manipulate or silence them.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Deals 2d6 bonus arcane damage",
        "15% chance to trigger wild magic surge",
        "Requires attunement by a spellcaster",
        "Crafted by: Asgard"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Cheep Cheep Shipping',
        levelRequirement: 5
    },

  'shadowfell_cold_blood': {
        id: 'shadowfell_cold_blood',
        name: "Cold Blood Tincture",
        description: "A chilling elixir that drains vitality from foes while granting the drinker a chilling aura. Reduces enemy movement speed by 50% for 15 seconds.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reduces enemy movement speed by 50% for 15 seconds",
        "Applies chilling aura to self for 30 seconds",
        "Crafted by: Manor Guard"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shadowfell_cold_blood_tome': {
        id: 'shadowfell_cold_blood_tome',
        name: "Cold Blood Tome",
        description: "A cursed book bound in black leather, whispered to hold the secrets of Onyx Hand’s ancient rituals. Reading it grants the user the power to freeze time for 3 seconds.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Temporarily freezes time for 3 seconds (30% chance to activate)",
        "Reduces all enemy movement speed by 50% during freeze",
        "Grants +20% chance to land critical hits on shadow creatures"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shadowfell_cold_soul_scarf': {
        id: 'shadowfell_cold_soul_scarf',
        name: "Cold Soul Scarf",
        description: "Woven from the frost-bleeding threads of the Shadowfell’s frozen voids, this scarf muffles your voice to the whispers of the Onyx Hand. It grants temporary stealth in dimly lit areas and chills nearby enemies, reducing their movement speed.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Stealth +20% in low light",
        "Chill effect: enemies move 30% slower for 5 seconds",
        "+5% chance to evade ambush attacks"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_crimson_ambition': {
        id: 'shadowfell_crimson_ambition',
        name: "Crimson Ambition Blade",
        description: "A blade forged from the blood of forgotten Onyx Hand lords, it drips with cursed ambition. Each strike leaves a temporary mark of the wielder’s deepest desire, but may also corrupt their soul. Perfect for those who crave power beyond the veil.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% damage against undead",
        "+10% chance to trigger a ‘Desire Surge’ when attacking (temporarily boosts attack speed)",
        "-5% defense when the user is under 20% HP"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_crimson_bloodstone': {
        id: 'shadowfell_crimson_bloodstone',
        name: "Crimson Bloodstone",
        description: "A pulsating obsidian shard that glows with the essence of fallen vampires. When worn, it amplifies dark magic and grants temporary resistance to psychic assaults. Its presence unnerves the living and draws the attention of the Onyx Hand.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% damage to dark magic spells",
        "Grants 10% resistance to psychic damage",
        "Causes enemies to become agitated when within 5 meters"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_crimson_haunt': {
        id: 'shadowfell_crimson_haunt',
        name: "Crimson Haunt Vial",
        description: "A vial of corrupted blood that amplifies the wielders shadow magic, causing enemies to bleed into the void. Perfect for assassins seeking to vanish and reappear with deadly precision.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Inflicts Void Bleed on targets for 3 rounds",
        "Grants 10% damage bonus to shadow attacks",
        "Wipes 50% of enemy HP when used within 10 feet"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_crimson_scarf': {
        id: 'shadowfell_crimson_scarf',
        name: "Crimson Scarf of the Unseen",
        description: "Woven from the threads of forgotten curses, this scarf grants the wearer the ability to see through the eyes of the damned. Slight movement of the fabric triggers minor illusions of nearby enemies.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reveals hidden enemy positions within 10ft",
        "+1 to Stealth checks when cloaked",
        "Causes minor illusions to distract foes (1d2 turns)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_crimson_shroud': {
        id: 'shadowfell_crimson_shroud',
        name: "Crimson Shroud of the Void",
        description: "A dark cloak woven from the shadows of forgotten souls. Grants invisibility under moonless skies and allows wearer to phase through solid matter for short bursts. Perfect for ambushes in the Shadowfell’s collapsing corridors.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🖤',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Invisibility for 10 seconds",
        "Phase through solid matter once per combat round",
        "Reduces damage taken from non-vampire enemies by 15%"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_crimson_vial': {
        id: 'shadowfell_crimson_vial',
        name: "Crimson Vial of Echoes",
        description: "A vial of blood-amber liquid that amplifies vocal commands. When spoken into, it echoes through the Shadowfell’s labyrinthine corridors, distracting enemies and revealing hidden traps.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 440,
        icon: '💉',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "Echoes voice for 30 seconds, revealing trap locations",
        "Adds +2 to stealth rolls while holding",
        "Causes nearby enemies to flinch if voice is harsh"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 4
    },

  'shadowfell_cursed_bloodstone': {
        id: 'shadowfell_cursed_bloodstone',
        name: "Cursed Bloodstone",
        description: "A pulsating gemstone that crackles with forbidden power. When worn, it allows the wearer to temporarily summon spectral whispers to confuse foes, and drains enemy HP on touch.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants +2 to Perception checks",
        "On touch, deals 1d4 Shadow Damage to one target",
        "Causes target to be confused for 1 round"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_cursed_forged': {
        id: 'shadowfell_cursed_forged',
        name: "Cursed Forge Blade",
        description: "A blade forged in the heart of the Shadowfell’s dark forge, its edge glows with the souls of the damned. It deals extra damage to undead and grants a chilling aura to allies.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '📦',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Damage: +20% against undead and cursed enemies",
        "Aura: Allies within 8m gain +10% movement speed",
        "Unholy: Increases attack speed by 15% when using a dark ability"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_cursed_gaze': {
        id: 'shadowfell_cursed_gaze',
        name: "Cursed Gaze of the Onyx Hand",
        description: "A shattered mirror fragment that whispers the victim’s deepest fears. Wearing it grants temporary psychic resonance with the wearer’s darkest memories.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+10% chance to detect hidden enemies in shadows",
        "-20% chance to be detected by non-vampiric foes",
        "+50% stealth in dark environments"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_cursed_kingdom_key': {
        id: 'shadowfell_cursed_kingdom_key',
        name: "Cursed Kingdom Key",
        description: "A rusted brass key shaped like a throne, forged from the last breath of a fallen king who refused to surrender his crown to the Shadowfell. When inserted into any locked door, it unlocks the past—revealing hidden rooms, forgotten betrayals, and the true fate of the DK Crew. It also triggers a curse: the wearer’s dreams become haunted by the king’s final words.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 100000,
        icon: '🗝',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Grants +2 to all Diplomacy rolls against factions in the Shadowfell",
        "-3 to all Perception checks when using in direct sunlight",
        "Triggers a curse: each night the wearer dreams of a different kingdom’s collapse",
        "Can be used to open the hidden chamber beneath the Kremling Krew vault",
        "Unlocks a hidden passage to the Bowsers’ throne room",
        "Crafted by: Mages Guild & Onyx Hand Vampires",
        "Only usable by those who have witnessed a royal succession battle"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pianta Chuck Express',
        levelRequirement: 8
    },

  'shadowfell_cursed_mirror_sword': {
        id: 'shadowfell_cursed_mirror_sword',
        name: "Cursed Mirror Sword",
        description: "A blade forged from shattered reflections of the Shadowfell’s most feared vampires. Its edge glows with a dark aura, and it strikes with precision and deadly grace.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+15% damage to shadow-based enemies",
        "+5% critical hit chance",
        "Reflects 20% of damage taken back as energy (if enemy is undead)"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_dark_blood_talisman': {
        id: 'shadowfell_dark_blood_talisman',
        name: "Dark Blood Talisman",
        description: "A talisman infused with the essence of a vampire’s cursed blood. Grants temporary invulnerability and enhances the wearer’s ability to absorb damage from darkness-based sources.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 50% damage absorption for 10 seconds",
        "Increases critical strike chance by 15%",
        "Reduces cooldowns of dark magic abilities by 25%"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_dark_echo_ring': {
        id: 'shadowfell_dark_echo_ring',
        name: "Dark Echo Ring",
        description: "A cursed ring that echoes the whispers of the Shadowfell’s lost. Wearing it grants the wearer temporary invisibility and the ability to hear nearby enemies’ true intentions.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "10% chance to become invisible for 5 seconds",
        "Detects enemy intent (true emotion or motive)",
        "Grants +5% damage against undead"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_dark_mirror': {
        id: 'shadowfell_dark_mirror',
        name: "Mirror of the Unseen Truth",
        description: "A fractured mirror that reflects not the surface of things but their deepest fears. Hold it to peer into the soul of a foe—or your own. Use with caution.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Fear Reflection: Reveals hidden truths through psychic resonance",
        "Phantom Echo: Causes enemies to see their own demise as they speak",
        "Shadow Pact: For 1 round, the user’s actions are mirrored by the mirror’s reflection, granting a +20% bonus to intimidation checks"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'shadowfell_dark_mirror_core': {
        id: 'shadowfell_dark_mirror_core',
        name: "Dark Mirror Core Fragment",
        description: "A shard of the dimension’s fractured mirror, reflecting foes’ true forms and amplifying psychic assaults. Can be fused into armor or staff for devastating psychic strikes.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 2100,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Reflects enemy’s form for 3s",
        "Enhances psychic damage by 30%",
        "Grants temporary vision of enemy’s soul"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shadowfell_dark_ritual_book': {
        id: 'shadowfell_dark_ritual_book',
        name: "Tome of the Blackened Veil",
        description: "An ancient grimoire that unlocks the power of the Onyx Hand’s ritual magic. Each page whispers secrets of the Shadowfell’s deepest horrors. Reading it grants temporary control over shadow entities.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 2100,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Grants 100% chance to cast Shadow Bind",
        "Increases spell damage by 50% for 3 turns",
        "Causes enemies to fear the user for 1 round"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_dream_satchel': {
        id: 'shadowfell_dream_satchel',
        name: "Dream Satchel",
        description: "A bag that stores fragments of nightmare visions",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 450,
        icon: '🌑',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "Consuming one fragment reveals a random dream horror",
        "Can store up to 3 dream fragments",
        "Triggers nightmare hallucinations for 1d4 rounds",
        "Can be used to summon a creature from a nightmare dimension",
        "Requires attunement by a dreamer or delirious mage",
        "Crafted by: Cheep Cheep Shipping"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Cheep Cheep Shipping',
        levelRequirement: 4
    },

  'shadowfell_echo_bottle': {
        id: 'shadowfell_echo_bottle',
        name: "Echo Bottle of the Shattered Veil",
        description: "A vial containing the last recorded scream of a captured soul. When consumed, it grants the user temporary auditory dominance over the battlefield.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+30% chance to hear enemy movements",
        "+15% chance to detect hidden paths",
        "Duration: 1 minute. Ends if enemy is killed or silenced"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'shadowfell_echo_cloak': {
        id: 'shadowfell_echo_cloak',
        name: "Echo Cloak of the Whispering Night",
        description: "Wears the voice of the void—speaks in riddles and shadows. Conceals the wearer from sight and sound, but whispers secrets of the fallen. Only the chosen may hear its truth.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% stealth chance",
        "+10% chance to trigger “Shadow Echo” (reveals enemy location for 3 seconds)",
        "When attacked, emits a psychic scream that stuns enemies for 1 turn"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_echo_heart_ammunition': {
        id: 'shadowfell_echo_heart_ammunition',
        name: "Echo Heart Ammunition",
        description: "These vials contain the fractured echoes of the Shadowfell’s lost hearts. When fired, they leave a haunting resonance that disorients enemies for 3 seconds. Perfect for ranged assassins who seek to break the silence of the Onyx Hand’s enforcers.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Disorient enemies for 3 seconds on hit",
        "+10% damage to undead and spectral foes",
        "Can be used in melee or ranged attacks"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_echo_of_the_cursed': {
        id: 'shadowfell_echo_of_the_cursed',
        name: "Echo of the Cursed",
        description: "A pendant that resonates with the voices of the forgotten. When worn, it grants the wearer insight into hidden paths and echoes of past horrors.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2100,
        icon: '🔮',
        stock: 5,
        rarity: 'rare',
        effects: [
            "+10% chance to find hidden doors or traps",
        "Reveals the location of nearby Onyx Hand nests (100% accuracy)",
        "Consuming this item grants a temporary “Shadow Sight” ability: see through walls and detect hidden enemies"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'shadowfell_echo_of_the_fallen_mirror': {
        id: 'shadowfell_echo_of_the_fallen_mirror',
        name: "Echo of the Fallen Mirror",
        description: "A shattered mirror fragment that whispers the secrets of the past. Wearing it lets the user briefly see the true form of enemies — and hear their deepest fears.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Reveals enemy hidden abilities for 5 seconds",
        "Deals 10% extra damage to illusions",
        "Grants +1 to perception checks"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'shadowfell_echo_silence_crown': {
        id: 'shadowfell_echo_silence_crown',
        name: "Echo Silence Crown",
        description: "A cursed crown that silences enemy voices and echoes of their thoughts, allowing the wearer to remain undetected during stealth or assassination. The crown resonates with the wearer’s inner darkness.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2100,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 10 seconds of silence to nearby enemies during stealth",
        "Reduces enemy detection radius by 20% while wearing",
        "Causes minor hallucinations after 30 seconds of prolonged wear"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'shadowfell_echo_soul': {
        id: 'shadowfell_echo_soul',
        name: "Echo Soul of the Forgotten King",
        description: "A broken crown that once belonged to a vampire king who died in the shadows of the Onyx Hand. Wearing it grants the user the power to hear whispers of the dead, but also risks being haunted by their memories.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5700,
        icon: '🎭',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants +2 to all Investigation checks against supernatural clues",
        "Grants +1 to all Charm or Persuade checks against shadow-aligned foes",
        "Causes a 1d4 psychic damage if the wearer fails a Will save against a shadowy entity"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'shadowfell_echo_tongue_amulet': {
        id: 'shadowfell_echo_tongue_amulet',
        name: "Echo Tongue Amulet",
        description: "A silver tongue-shaped amulet etched with runes of the forgotten tongue. Worn by the Void Drifters, it echoes the last words of those who crossed into the Edge. Each utterance chimes with the weight of a soul’s final memory, though it may also trigger madness in those who hear too much.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 450,
        icon: '🗡',
        stock: 1,
        rarity: 'uncommon',
        effects: [
            "+1 to Perception checks when detecting hidden truths",
        "When used to speak, voice carries 200 feet but mutates with each use",
        "10% chance to reveal a hidden truth that alters the wearer’s next action",
        "Worn by: Void Drifters",
        "Crafted by: Blooper Wetworks"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Blooper Wetworks',
        levelRequirement: 3
    },

  'shadowfell_echoed_mirror': {
        id: 'shadowfell_echoed_mirror',
        name: "Echoed Mirror",
        description: "A shattered mirror that reflects not your face but your inner fears. Each reflection is a memory of a lost soul, and each glance reveals a hidden truth or danger. Best used in combat to mislead foes or detect illusions.",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 2100,
        icon: '🔮',
        stock: 0,
        rarity: 'rare',
        effects: [
            "+20% chance to detect illusions or false enemies",
        "Reflections reveal hidden enemy weaknesses",
        "Causes temporary hallucinations for 5 seconds if used too frequently"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'shadowfell_echoed_vampire_fang': {
        id: 'shadowfell_echoed_vampire_fang',
        name: "Echoed Vampire Fang",
        description: "A razor-edged fang carved from a fallen vampire’s soul. It amplifies the user’s voice and allows them to cast a haunting whisper that deals 30% damage and frightens nearby enemies.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 450,
        icon: '🦷',
        stock: 4,
        rarity: 'uncommon',
        effects: [
            "Boosts voice damage by 40%",
        "Frightens enemies for 3 seconds on cast",
        "Deals 30% extra damage to undead"
        ],
        vendor: 'shadowfell',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 3
    },

};
