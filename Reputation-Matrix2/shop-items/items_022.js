// Shop items chunk 22 of 51
// Items 2101 to 2200 (100 items)
// Auto-generated: 2026-03-21 13:41:41

import { SHOP_CATEGORIES } from './categories.js';

export const ITEMS_022 = {
  'hyrule_temple_heart_rod': {
        id: 'hyrule_temple_heart_rod',
        name: "Temple Heart Rod of the Forgotten King",
        description: "A relic carved from the central core of the Temple of the Ancients. It grants the wielder a connection to ancient power, allowing them to temporarily restore the battlefield to its original state after combat.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '📦',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Restores 100% of HP to the user after 3 seconds of combat",
        "Grants immunity to environmental damage for 2 seconds after use",
        "Reduces enemy HP regeneration by 30% for 10 seconds"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'hyrule_temple_heartstone': {
        id: 'hyrule_temple_heartstone',
        name: "Heartstone of the Ancient Sanctum",
        description: "A glowing artifact from the depths of the Temple of the Ancients. Grants immunity to status effects and boosts stamina. Only found after defeating the final guardian.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4500,
        icon: '🌌',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Immunity to poison and burn effects",
        "Increases stamina regeneration by 30%",
        "Grants 5% critical hit chance"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 8
    },

  'hyrule_temple_key': {
        id: 'hyrule_temple_key',
        name: "Zora’s Whisper Key",
        description: "A relic from the depths of the Zora’s Domain, this key unlocks ancient temple doors and whispers secrets of the lake’s hidden truths.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 2000,
        icon: '🔑',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Opens sealed temple doors with 100% success",
        "Grants temporary water-based teleportation",
        "Reveals underwater temple layout on map"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'hyrule_temple_key_of_whispers': {
        id: 'hyrule_temple_key_of_whispers',
        name: "Temple Key of Whispers",
        description: "A silent, obsidian key that opens ancient temple doors. It only works when the player is in a state of pure focus. Upon use, it grants temporary stealth and reveals enemy weaknesses.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Silent Stealth: +30% stealth during movement",
        "Weakness Reveal: Reveals enemy weaknesses for 10 seconds after key use",
        "Focus Cost: Requires 50% focus to activate; if focus drops below 20%, effect fails"
        ],
        vendor: 'hyrule',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'hyrule_temple_key_ring': {
        id: 'hyrule_temple_key_ring',
        name: "Key of the Shattered Chamber",
        description: "A ring engraved with the runes of the Temple of the Lost. Grants the ability to unlock ancient doors, even those warped by reality’s fractures. Wears down with use, but never breaks.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Opens sealed doors even when reality is unstable",
        "Grants a 10% chance to reveal hidden passages",
        "Absorbs minor reality tears when used near ancient architecture"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'hyrule_temple_key_rune': {
        id: 'hyrule_temple_key_rune',
        name: "Temple Key Rune",
        description: "This ancient artifact grants the ability to unlock any sealed temple door or artifact vault. When used it emits a resonant chime that disrupts enemy wards and triggers hidden mechanisms.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 3500,
        icon: '🔐',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Unlocks sealed doors or vaults within 100 meters",
        "Disables enemy wards and traps for 5 seconds",
        "Triggers hidden mechanisms in temple ruins"
        ],
        vendor: 'hyrule',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'hyrule_temple_keyblade': {
        id: 'hyrule_temple_keyblade',
        name: "Keyblade of the Lost Temple",
        description: "A mysterious artifact forged in the ruins of the Temple of the Sacred Key. It grants the wielder the ability to open locked doors and activate ancient mechanisms, but may trigger traps if misused.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Opens locked doors or mechanisms",
        "Activates hidden traps with a 50% chance",
        "+5% damage to enemies within temple zones"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'hyrule_temple_keychain': {
        id: 'hyrule_temple_keychain',
        name: "Temple Keychain of the Fractured Gate",
        description: "A keychain engraved with the sigil of the Temple of the Fractured Gate. It allows the user to bypass locked doors and triggers a mini-event when carried into a temple.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 2500,
        icon: '🗝',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Opens all temple doors with 100% success",
        "Triggers a “Temple Echo” event: temporary buff to all party members",
        "Grants 50% chance to drop hidden loot from temple chests"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_temple_keyring': {
        id: 'hyrule_temple_keyring',
        name: "Key to the Forgotten Vault",
        description: "A simple key that unlocks one of the temple’s sealed chambers. Carries a faint echo of ancient rituals and can summon a brief spirit guide for combat assistance.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Unlock One Temple Chamber",
        "Summon Spirit Guide (10s duration)",
        "Minor HP Regeneration after using summon"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_temple_mantle': {
        id: 'hyrule_temple_mantle',
        name: "Guardian’s Mantle of the Shattered Spire",
        description: "Worn by those who have survived the trials of the Temple of the Shattered Spire, this mantle grants resilience against environmental hazards and echoes of ancient battles.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2800,
        icon: '🛡',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+12% damage reduction from environmental hazards",
        "+8% defense against magical attacks",
        "Grants 20% chance to regenerate health after defeating a temple boss"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 6
    },

  'hyrule_temple_mechanism': {
        id: 'hyrule_temple_mechanism',
        name: "Gear of the Lost Guardian",
        description: "A rusted mechanical gear salvaged from the Temple of the Guardian. It hums with ancient energy and allows the wielder to manipulate gravity in confined spaces. Only one who has faced the Temple’s trials may wield it.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2800,
        icon: '📦',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Grants 20% bonus to jump height",
        "Allows 1 use of gravity pull in 30 seconds",
        "Reduces fall damage by 50% for 5 seconds"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 6
    },

  'hyrule_temple_mind_blade': {
        id: 'hyrule_temple_mind_blade',
        name: "Temple Mind Blade",
        description: "A cursed blade forged from the heart of a temple guardian. It cuts through illusions and echoes, leaving a temporary echo of your last attack.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '📦',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Cuts through illusions and echo effects",
        "Leaves a 3-second echo of your last attack",
        "Causes enemies to reflect your damage for 2 seconds"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'hyrule_temple_mirage_key': {
        id: 'hyrule_temple_mirage_key',
        name: "Mirage Key of the Forgotten Temple",
        description: "A key that unlocks the hidden chamber of the Temple of the Whispering Wind. It distorts reality briefly, allowing you to bypass locked doors — but beware, it leaves a faint echo of your last step behind.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3500,
        icon: '🔍',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+10% chance to bypass locked doors",
        "Grants temporary invisibility for 5 seconds after use",
        "Leaves a faint echo trail — can be used to track enemies"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'hyrule_temple_mirage_rod': {
        id: 'hyrule_temple_mirage_rod',
        name: "Mirage of the Sacred Well",
        description: "A staff that distorts reality, turning enemies into illusions that vanish upon contact. Perfect for navigating the Temple’s shifting corridors.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Causes enemies to become illusions for 2 turns, then vanish",
        "Grants +15% evasion chance in temple environments",
        "Grants 10% chance to teleport 3 squares when hit"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'hyrule_temple_mirror_shard': {
        id: 'hyrule_temple_mirror_shard',
        name: "Mirror Shard of the Temple of Reflection",
        description: "A shard from the broken mirror of the Temple of Reflection, it grants the user the ability to briefly see through enemies’ eyes and predict their next move.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Vision of Enemy Moves (10s cooldown)",
        "Grants 10% critical hit chance",
        "Temporary blindness if used while moving"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'hyrule_temple_shard_of_echoes': {
        id: 'hyrule_temple_shard_of_echoes',
        name: "Shard of Echoes",
        description: "A fragment of ancient temple resonance that amplifies your voice during battle. Speak true and it echoes through enemies minds, disrupting their focus. Only usable in temple ruins or during ritual events.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% chance to silence enemy spellcasting",
        "+15% damage when using voice-based abilities",
        "+5% chance to trigger temple resonance aura on allies"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'hyrule_temple_silence_pearl': {
        id: 'hyrule_temple_silence_pearl',
        name: "Pearl of the Silent Temple",
        description: "A sacred artifact from the Temple of the Unseen, this pearl allows the bearer to hear the whispers of the past. It suppresses magic in a 30-meter radius, making it perfect for disrupting enemy spells.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '🌙',
        stock: 5,
        rarity: 'rare',
        effects: [
            "Suppresses all magic within 30m for 30 seconds",
        "Reveals hidden paths in temple ruins",
        "Grants immunity to debuffs from magical attacks"
        ],
        vendor: 'hyrule',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'hyrule_temple_soul_amber': {
        id: 'hyrule_temple_soul_amber',
        name: "Temple Soul Amber",
        description: "A glowing amber shard forged from the heart of an ancient temple’s guardian spirit. Wields power to amplify your attacks and reveal hidden paths in sacred ruins.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% damage to enemy defenses",
        "Reveals hidden temple entrances for 10 seconds",
        "Grants temporary immunity to status effects when in combat"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_temple_soul_catalyst': {
        id: 'hyrule_temple_soul_catalyst',
        name: "Soul Catalyst of the Forgotten Temple",
        description: "A catalyst that binds the soul of a temple’s guardian to the wielder, granting them the ability to channel divine power and summon spectral echoes.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '🔮',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Summons 1 spectral echo for 10 seconds (does 100% damage)",
        "Grants 50% bonus damage to all spells for 3 turns",
        "Increases cast speed of all spells by 20%"
        ],
        vendor: 'hyrule',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 4
    },

  'hyrule_temple_soul_cloth': {
        id: 'hyrule_temple_soul_cloth',
        name: "Temple Soul Cloak",
        description: "Woven from the essence of ancient temple spirits, this cloak absorbs enemy magic and grants the wearer a temporary aura of sacred silence. Perfect for stealth and magic defense.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Absorbs Magic Damage",
        "Grants Stealth Camouflage (5s)",
        "Reduces Enemy Magic Detection"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_temple_soul_core': {
        id: 'hyrule_temple_soul_core',
        name: "Soul Core of the Temple Guardian",
        description: "A crystalline core forged from the essence of a fallen temple guardian. It absorbs enemy magic and converts it into offensive power. When charged, it unleashes a beam that pierces through multiple enemies.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5000,
        icon: '📦',
        stock: 0,
        rarity: 'rare',
        effects: [
            "Absorbs 50% of magic damage and converts it to 10% of weapon damage",
        "Charges 30 seconds after 3 hits, then unleashes a piercing beam",
        "Reduces enemy magic resistance by 30% while active"
        ],
        vendor: 'hyrule',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 8
    },

  'hyrule_temple_soul_gem': {
        id: 'hyrule_temple_soul_gem',
        name: "Temple Soul Gem",
        description: "A pulsing crystal captured from the soul of an ancient guardian. It absorbs negative energy, granting the wielder temporary immunity to fear and confusion. Ideal for navigating haunted ruins.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '💎',
        stock: 5,
        rarity: 'rare',
        effects: [
            "Absorbs 1000 HP of negative energy",
        "Grants +20% evasion against cursed enemies",
        "Removes fear effect for 3 turns"
        ],
        vendor: 'hyrule',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'hyrule_temple_soul_ring': {
        id: 'hyrule_temple_soul_ring',
        name: "Temple Soul Ring",
        description: "Fused with the soul of a temple guardian who sacrificed themselves to protect the Triforce. Wears this ring to feel the temple’s heartbeat and sense the presence of ancient spirits.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3500,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 5% damage bonus to spirit-based attacks",
        "Detects hidden temple traps",
        "Increases critical hit chance by 10%"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 6
    },

  'hyrule_temple_soul_shard': {
        id: 'hyrule_temple_soul_shard',
        name: "Temple Soul Shard",
        description: "A fragment of ancient temple magic that channels the essence of forgotten guardians. When wielded, it grants temporary invulnerability to elemental attacks and reveals hidden temple passages.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Temporarily immune to elemental damage (30 sec)",
        "Reveals hidden temple entrances (1 use)",
        "Boosts exploration speed by 20%"
        ],
        vendor: 'hyrule',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'hyrule_temple_soul_torch': {
        id: 'hyrule_temple_soul_torch',
        name: "Flame of the Temple’s Will",
        description: "A torch infused with the soul of the ancient temple’s last priest. When lit, it grants the wielder a temporary shield and enhances their ability to dodge attacks.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🔥',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants 50 HP shield for 10 seconds",
        "Increases dodge chance by 15%",
        "Reduces incoming damage by 20% for 5 seconds"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'hyrule_temple_spirit_amber': {
        id: 'hyrule_temple_spirit_amber',
        name: "Amber Soul of the Temple",
        description: "A crystalline essence collected from the heart of the Temple of Light. When consumed, it temporarily boosts the user’s strength and grants them the ability to see through illusions.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1800,
        icon: '💡',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants +30% strength for 10 seconds",
        "Reveals illusions or deception for 15 seconds",
        "Restores 50% stamina after use"
        ],
        vendor: 'hyrule',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 6
    },

  'hyrule_temple_talisman': {
        id: 'hyrule_temple_talisman',
        name: "Heart of the Shattered Vault",
        description: "A glowing relic forged from the soul of an ancient temple guardian. Wearing it grants temporary resistance to magical explosions and lets the wearer speak to echoes of the temple’s past.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3000,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Resist magical explosions by 20%",
        "Speak to echoes for 5 seconds (reveals hidden lore)",
        "Slight mana drain on use"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 6
    },

  'hyrule_temple_talisman_of_echoes': {
        id: 'hyrule_temple_talisman_of_echoes',
        name: "Talisman of the Echoing Temple",
        description: "Carved from the obsidian heart of the ancient Temple of the Forgotten, this amulet echoes the prayers of those who dared enter its labyrinth. Wearing it grants visions of hidden paths and lost truths.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4200,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+25% critical hit chance",
        "Grants temporary invisibility for 2 seconds after using a skill",
        "Increases map exploration bonus by 50% in ancient temple zones"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'hyrule_temple_ward': {
        id: 'hyrule_temple_ward',
        name: "Temple Ward Stone",
        description: "A glowing stone carved with symbols from the First Temple. When placed, it wards off dark entities and grants temporary sanctuary. Perfect for desperate heroes.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 1800,
        icon: '🛡',
        stock: 5,
        rarity: 'uncommon',
        effects: [
            "Grants 15% damage reduction for 30 seconds",
        "Creates a 10m radius ward zone that prevents enemy entry",
        "Grants +5% movement speed while active"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 4
    },

  'hyrule_temple_ward_amber': {
        id: 'hyrule_temple_ward_amber',
        name: "Amber Ward of the Ancient Temple",
        description: "This glowing amber amulet channels the power of the Temple of the Forgotten. Wears the wearer with a faint echo of ancient chants, granting them resilience against magical backlash. Only found in the ruins of the Eastern Sanctum.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Resists magic backlash by 30%",
        "Grants temporary silence to enemy spells (10 sec)",
        "Increases stamina regeneration by 15% during combat"
        ],
        vendor: 'hyrule',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 5
    },

  'hyrule_temple_warden_belt': {
        id: 'hyrule_temple_warden_belt',
        name: "Temple Warden Belt",
        description: "Worn by guardians of ancient sanctums this belt channels sacred energy from the ruins of the Great Temple of Zora. It grants temporary immunity to elemental damage and enhances stamina regeneration during prolonged combat.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Immunity to elemental damage for 10 seconds",
        "Regenerates 20% stamina per 3 seconds while active",
        "Grants +10% damage resistance to all weapon types"
        ],
        vendor: 'hyrule',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'hyrule_temple_warden_scepter': {
        id: 'hyrule_temple_warden_scepter',
        name: "Temple Warden Scepter",
        description: "A relic forged in the heart of the ancient temples of Hyrule, this scepter channels the echo of forgotten guardians. When wielded, it grants temporary invulnerability and disrupts enemy spellcasting.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 10 seconds of invulnerability on hit",
        "Reduces enemy spellcasting speed by 50%",
        "Causes enemy spellcasts to shatter on impact"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_temple_warder': {
        id: 'hyrule_temple_warder',
        name: "Echoing Temple Ward",
        description: "A relic forged from ancient temple stones, this ward resonates with the echoes of forgotten guardians. It grants temporary resistance to magic and enhances perception of hidden paths.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Resistance to Magic Attacks",
        "Perception Boost: Detect Hidden Paths",
        "Slightly Increased Movement Speed"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_temple_wisp_shield': {
        id: 'hyrule_temple_wisp_shield',
        name: "Wisp Shield of the Vaulted Depths",
        description: "A shield that absorbs echoes of ancient temple magic, turning them into temporary shields. Worn near crumbling temples, it pulses with faint blue light and grants a sense of the temple’s forgotten history.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2000,
        icon: '🛡',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Absorbs 100% of magical damage from temple sources",
        "Grants +10% dodge chance when near temple ruins",
        "Reveals minor temple secrets if equipped while entering a temple"
        ],
        vendor: 'hyrule',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'hyrule_temples_echo_rod': {
        id: 'hyrule_temples_echo_rod',
        name: "Echoes of the Temple Rod",
        description: "A relic forged from the whispers of ancient temples, this rod channels divine echoes to amplify your weapon’s damage. When swung, it summons a spectral echo of the temple’s guardian spirit for a brief burst of power.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "+20% damage to all weapon attacks",
        "Summons a spectral guardian spirit for 5 seconds on hit",
        "Grants temporary invincibility for 1 second after casting"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'hyrule_time_craft': {
        id: 'hyrule_time_craft',
        name: "Time Weavers’ Temple Tool",
        description: "A mysterious artifact from the Temple of Time’s lost archives. Lets the user manipulate moments and reset encounters with a flick of the wrist.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3200,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 10% chance to reset enemy turn order",
        "Increases damage by 5% when near time-affected zones",
        "Grants 10% chance to reveal a hidden boss phase"
        ],
        vendor: 'hyrule',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 5
    },

  'hyrule_toad_soul_ring': {
        id: 'hyrule_toad_soul_ring',
        name: "Toad Soul Ring",
        description: "A cursed ring that binds the soul of a Toad prisoner to your hand. Grants powerful buffs but drains HP with each use—use wisely.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '💀',
        stock: 4,
        rarity: 'rare',
        effects: [
            "+30% attack power for 10 seconds",
        "Deals 10% extra damage to enemy HP below 20%",
        "Consumes 10% HP per activation (no regeneration)"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_tombstone_scepter': {
        id: 'hyrule_tombstone_scepter',
        name: "Tombstone Scepter of the Forgotten King",
        description: "A relic forged from the bones of a fallen ruler of the Temple of the Veiled Sun. When wielded, it whispers secrets of ancient Hyrulean lore and dims enemy light effects.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 10% chance to reveal enemy hidden abilities",
        "Reduces light-based damage taken by 20%",
        "Inflicts 10% chance to trigger a memory flash on hit"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_triforce_bangle': {
        id: 'hyrule_triforce_bangle',
        name: "Triforce Bangle",
        description: "Worn by those who seek the power of the Triforce, this bangle channels the balance of power. It grants the wearer a sense of purpose and enhances their resolve.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Restores 10% HP per turn while holding the bangle",
        "Grants a +2 bonus to all stats when facing enemies with 3+ HP",
        "Unlocks a hidden ability: “Balance’s Will” — temporarily nullifies enemy status effects"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_triforce_blade_core': {
        id: 'hyrule_triforce_blade_core',
        name: "Triforce Blade Core",
        description: "A fragment of the Triforce’s power embedded in a wickedly sharp blade. It grants the wielder the ability to split enemies into three parts, each suffering unique elemental damage.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3800,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Split enemy into 3 parts on hit",
        "Each part takes 20% elemental damage based on weapon type",
        "Reduces cooldown on Triforce-infused attacks by 15%"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 6
    },

  'hyrule_triforce_blessed_rod': {
        id: 'hyrule_triforce_blessed_rod',
        name: "Triforce’s Will",
        description: "A slender staff that channels the divine will of the Triforce. It grants temporary buffs and allows the wielder to temporarily invoke the power of balance, courage, or wisdom.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 7500,
        icon: '📦',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+25% chance to trigger “Divine Blessing” – grants 30% damage bonus for 10s",
        "+10% chance to restore 15% HP after using a Triforce ability",
        "Immune to all status effects from temple traps"
        ],
        vendor: 'hyrule',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'hyrule_triforce_blessing_legacy': {
        id: 'hyrule_triforce_blessing_legacy',
        name: "Triforce Echo of Resilience",
        description: "A relic imbued with the spirit of the Triforce, granting the wielder a sense of invincibility during critical battles. Its aura dims when the user is injured, but restores full health after 30 seconds of rest.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3500,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 50% damage reduction during combat",
        "Restores 100% HP after 30s idle",
        "Increases critical hit chance by 20%"
        ],
        vendor: 'hyrule',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 7
    },

  'hyrule_triforce_blood_tear': {
        id: 'hyrule_triforce_blood_tear',
        name: "Triforce Blood Tear",
        description: "A cursed relic forged from the blood of the Triforce’s last guardian. It boosts strength and speed, but drains vitality over time. Wielders must choose between power or survival.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+30% attack speed",
        "+25% physical damage",
        "-10% max HP over 60 seconds"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_triforce_bloodstone': {
        id: 'hyrule_triforce_bloodstone',
        name: "Triforce Bloodstone",
        description: "Forged from the core of a fallen Triforce guardian, this artifact pulses with the essence of balance and sacrifice. Wields the power to restore allies health with a single touch.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3800,
        icon: '🌀',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 100 HP to a single ally upon activation",
        "Grants 5% chance to negate enemy buffs",
        "Increases defense by 10% when holding the Triforce symbol"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 6
    },

  'hyrule_triforce_bolt': {
        id: 'hyrule_triforce_bolt',
        name: "Triforce Spark Bolt",
        description: "A rare artifact forged from the residual energy of the Triforce. When wielded, it ignites the battlefield with bursts of divine power, striking enemies with devastating precision.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3200,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+20% attack power during Triforce activation",
        "+15% chance to critically strike",
        "Grants temporary invincibility for 3 seconds after using a Triforce ability"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 7
    },

  'hyrule_triforce_bottle': {
        id: 'hyrule_triforce_bottle',
        name: "Triforce Elixir",
        description: "A liquid infused with the essence of the Triforce. Consuming it restores 50% of your max HP and grants temporary resistance to magical damage. Ideal for heroes who must fight through ancient temple trials.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 1500,
        icon: '💉',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 50% max HP",
        "Grants 30% resistance to magic damage for 10 seconds",
        "Reduces cooldowns for all magic spells by 20%"
        ],
        vendor: 'hyrule',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 5
    },

  'hyrule_triforce_catalyst': {
        id: 'hyrule_triforce_catalyst',
        name: "Triforce Catalyst Core",
        description: "A crystalline core that channels the power of the Triforce, allowing the user to temporarily unlock the abilities of the Sacred Trinity. Use wisely — the power is volatile and can turn allies into weapons.",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 4500,
        icon: '📦',
        stock: 1,
        rarity: 'epic',
        effects: [
            "+30% damage for 5 seconds after activating Triforce ability",
        "+10% chance to stun enemies with Triforce aura",
        "Grants temporary flight ability for 3 seconds after activation"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 8
    },

  'hyrule_triforce_core': {
        id: 'hyrule_triforce_core',
        name: "Triforce Core Fragment",
        description: "A pulsing shard of the legendary Triforce, forged from ancient divine energy. Wields power of balance, wisdom, and courage in equal measure. Only one can hold it at a time.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3500,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 10% HP on equip",
        "Grants +10% defense against magic",
        "Reduces cooldowns of Triforce-related abilities by 30%"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 6
    },

  'hyrule_triforce_echo': {
        id: 'hyrule_triforce_echo',
        name: "Triforce Resonance Vial",
        description: "A vial containing the echo of the Triforce’s power. When consumed, it temporarily amplifies the wielder’s willpower and grants a chance to resist fate’s control.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2200,
        icon: '💡',
        stock: 4,
        rarity: 'rare',
        effects: [
            "Grants 10% chance to resist fate control effects for 30 seconds",
        "Increases maximum HP by 20% for 10 seconds",
        "Grants +2 to all skill checks against environmental hazards"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_triforce_echo_cape': {
        id: 'hyrule_triforce_echo_cape',
        name: "Triforce Echo Cape",
        description: "Wears the echo of the Triforce’s will—this cape grants temporary visions of past battles and boosts your HP regeneration while in battle.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🌟',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 10% HP per second while active",
        "Grants vision of past battle locations for 5 seconds",
        "Increases movement speed by 15% for 10 seconds after using vision"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_triforce_fragment_treasure': {
        id: 'hyrule_triforce_fragment_treasure',
        name: "Triforce Shard of the Forgotten",
        description: "A relic from the shattered heart of the Temple of the Triforce. When worn, it grants a small chance to restore health or armor when hit, and glows when near the Master Sword.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "10% chance to restore 20% HP or Armor on hit",
        "Glows when near Master Sword or Triforce",
        "Increases Triforce affinity bonus for allies"
        ],
        vendor: 'hyrule',
        shippedBy: 'Shy Guy Smugglers',
        levelRequirement: 5
    },

  'hyrule_triforce_fragments': {
        id: 'hyrule_triforce_fragments',
        name: "Triforce Shard of the Shattered Realm",
        description: "A broken piece of the Triforce imbued with ancient power. Wearing it grants temporary buffs and reveals Triforce-related quests. Only obtainable through temple ruins.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3500,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Grants 10% damage increase for 5 minutes",
        "Reveals nearby Triforce quest markers",
        "Restores 10% HP after each combat round"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 7
    },

  'hyrule_triforce_hammer': {
        id: 'hyrule_triforce_hammer',
        name: "Triforce Hammer",
        description: "A legendary weapon shaped like the Triforce, forged from the sacred power of Hyrule’s ancient balance. It strikes with devastating force and temporarily restores health to allies.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 7500,
        icon: '📦',
        stock: 1,
        rarity: 'epic',
        effects: [
            "Deals 150% damage to enemies",
        "Restores 50% HP to nearest ally on hit",
        "Causes enemies to stagger for 2 seconds on hit"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 7
    },

  'hyrule_triforce_heart': {
        id: 'hyrule_triforce_heart',
        name: "Triforce Heart Shard",
        description: "A fragment of the Triforce, hidden in the ruins of the Temple of the Ancients. This shard grants the wielder temporary divine power and can be fused with other relics for greater effect.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 7500,
        icon: '🏰',
        stock: 1,
        rarity: 'epic',
        effects: [
            "Grants +30% damage to all attacks for 10 turns",
        "Can be fused with other relics to create powerful artifacts",
        "Grants “Divine Favor” – reduces enemy defense by 20% for 3 turns"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 7
    },

  'hyrule_triforce_heart_core': {
        id: 'hyrule_triforce_heart_core',
        name: "Triforce Heart Core",
        description: "A pulsing core of pure Tri Force energy. Grants temporary invincibility and grants a +10% chance to break enemy shields. Only usable once per battle.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '💥',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Grants 3 seconds of invincibility",
        "Increases shield break chance by 10%",
        "Causes 10% chance to deal bonus damage to armored foes"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'hyrule_triforce_heart_shard': {
        id: 'hyrule_triforce_heart_shard',
        name: "Triforce Shard of the Forgotten King",
        description: "A fractured relic of the Triforce’s original power. Wields a faint glow that boosts your HP and MP regeneration. Only visible in moonlit temple ruins.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3800,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Regenerates 10% HP and MP per second while active",
        "Grants 10% damage reduction from shadow magic",
        "Unlocks hidden temple path in the Temple of the Forgotten"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 7
    },

  'hyrule_triforce_mind_mirror': {
        id: 'hyrule_triforce_mind_mirror',
        name: "Triforce Mind Mirror",
        description: "A sacred artifact that reflects the wielder’s inner will and echoes the Tri Force’s balance. When activated, it enhances focus, reveals hidden paths, and protects from psychic backlash.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🌀',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Increases focus duration by 40%",
        "Reveals hidden paths in temple ruins",
        "Grants 10% resistance to psychic attacks"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 5
    },

  'hyrule_triforce_shadow': {
        id: 'hyrule_triforce_shadow',
        name: "Triforce Shadow Blade",
        description: "A blade forged from the fractured shadow of the Triforce, it hums with ancient power. It grants the wielder temporary invisibility and deals critical damage to corrupted foes.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3800,
        icon: '🗡',
        stock: 2,
        rarity: 'rare',
        effects: [
            "100% invisibility for 5 seconds when swung",
        "Deals +150% damage to enemies with corruption status",
        "Restores 10% HP when enemy is defeated"
        ],
        vendor: 'hyrule',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 7
    },

  'hyrule_triforce_shard': {
        id: 'hyrule_triforce_shard',
        name: "Triforce Echo Shard",
        description: "A shard of the legendary Triforce, humming with power and memory. When activated, it grants temporary boosts to all three core stats and reveals hidden enemy weaknesses.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 3,
        rarity: 'rare',
        effects: [
            "Boosts Strength Agility and Defense",
        "Reveals Enemy Weaknesses",
        "Increases Critical Hit Chance"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_triforce_shard_3': {
        id: 'hyrule_triforce_shard_3',
        name: "The Forgotten Heart Shard",
        description: "A relic of the ancient Triforce’s power, this shard pulses with the echo of Hyrule’s soul. Wielders report visions of past heroes and future destinies. May not be used in battle without proper focus.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3000,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Gain 10 Coins per second while active",
        "Grants 10% chance to heal allies with a touch",
        "Increases damage by 15% against enemies with “Soul” status"
        ],
        vendor: 'hyrule',
        shippedBy: 'Koopa Postal',
        levelRequirement: 7
    },

  'hyrule_triforce_shard_amber': {
        id: 'hyrule_triforce_shard_amber',
        name: "Amber Triforce Shard",
        description: "A fragment of the ancient Triforce’s power, infused with the warmth of the sunken Temple of the Sunken King. Grants temporary resistance to elemental damage and allows the user to summon a small, glowing shield.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3200,
        icon: '🔮',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Reduces elemental damage taken by 25% for 15 seconds",
        "Summons a 1.5m glowing shield for 5 seconds upon activation",
        "Increases stamina regeneration by 10% while shield active"
        ],
        vendor: 'hyrule',
        shippedBy: 'Boo Spectral Mail',
        levelRequirement: 7
    },

  'hyrule_triforce_shard_core': {
        id: 'hyrule_triforce_shard_core',
        name: "Triforce Shard Core",
        description: "The core of a shattered Triforce relic that channels divine energy. It grants immense power but causes temporary insanity if overused. Perfect for heroes with a dark side.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '🔮',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+40% damage to all attacks",
        "20% chance to inflict “Divine Fury” debuff on enemies (20% damage over 3 seconds)",
        "-10% HP after 20 seconds of use"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_triforce_shroud': {
        id: 'hyrule_triforce_shroud',
        name: "Triforce Shroud",
        description: "A mystical cloak woven with the power of the Triforce, granting invisibility and resistance to magical detection. Perfect for sneaking through temple wards or evading enemy traps.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4500,
        icon: '🧥',
        stock: 1,
        rarity: 'rare',
        effects: [
            "Grants 30 seconds of invisibility",
        "Reduces magical detection chance by 40%",
        "Increases movement speed by 10% while invisible"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 5
    },

  'hyrule_triforce_soul': {
        id: 'hyrule_triforce_soul',
        name: "Triforce Soul Fragment",
        description: "A piece of the Triforce’s essence, infused with ancient power. When worn, it grants a shield of willpower and reveals enemy weaknesses for 5 seconds.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 2500,
        icon: '📦',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+10% Defense",
        "Reveals enemy weakness for 5 seconds",
        "Grants 20% chance to dodge attacks"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'hyrule_triforce_soul_gem': {
        id: 'hyrule_triforce_soul_gem',
        name: "Soul Gem of the Three Kings",
        description: "A glowing orb containing the essence of the Triforce’s three forms. When activated, it empowers allies with temporary elemental resistance and boosts team HP.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 5200,
        icon: '💫',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Boosts team HP by 20% for 10 seconds",
        "Grants 15% elemental resistance to allies",
        "Allows 30% faster movement during temple traversal"
        ],
        vendor: 'hyrule',
        shippedBy: 'Pipe Express',
        levelRequirement: 9
    },

  'hyrule_triforce_soul_shard': {
        id: 'hyrule_triforce_soul_shard',
        name: "Heart of the Triforce Shard",
        description: "A relic from the shattered Triforce itself, this shard amplifies your character’s morale and restores health with every successful action. Wields the ancient will of Hyrule’s greatest guardian.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2000,
        icon: '🌿',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Restores 100 HP on use",
        "Increases defense by 20% for 5 seconds",
        "Grants 10% bonus to all skill casts"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'hyrule_triforce_tome': {
        id: 'hyrule_triforce_tome',
        name: "Triforce Tome",
        description: "A bound scroll containing the secrets of the Triforce’s power. Reading it grants temporary control over one of its three virtues — Balance, Power, or Wisdom — for 10 minutes. Use wisely, for the cost is eternal sacrifice.",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 6000,
        icon: '📜',
        stock: 0,
        rarity: 'legendary',
        effects: [
            "Virtue Activation: Choose one of the three Triforce Virtues for 10 minutes",
        "Sacrifice Cost: After activation, the user must perform a difficult task or lose 50% health to rebind the virtue",
        "Fortune’s Favor: Allies gain +20% resistance to all elemental damage during activation"
        ],
        vendor: 'hyrule',
        shippedBy: 'Void Drifter Relay',
        levelRequirement: 9
    },

  'hyrule_triforce_vial': {
        id: 'hyrule_triforce_vial',
        name: "Triforce Vial of Echoes",
        description: "A vial containing a condensed echo of the Triforce’s divine will. When consumed, it grants the user temporary strength and a chance to bypass enemy defenses.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 2500,
        icon: '🍺',
        stock: 2,
        rarity: 'rare',
        effects: [
            "+15% Attack Power",
        "20% chance to bypass enemy armor",
        "Restores 30% HP on use"
        ],
        vendor: 'hyrule',
        shippedBy: 'Rakasha Spirit Walk',
        levelRequirement: 5
    },

  'hyrule_triforce_wisp': {
        id: 'hyrule_triforce_wisp',
        name: "Triforce Wisp",
        description: "A living fragment of the Triforce’s power",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 50000,
        icon: '🌟',
        stock: 0,
        rarity: 'legendary',
        effects: [
            "Grants the user temporary mastery of all three Triforce powers",
        "Grants a +5 bonus to all actions when using a Triforce ability",
        "May cause the user to become temporarily bound to the Triforce’s will",
        "Can be fused with other items to create a new power",
        "Only usable once per cycle of reincarnation"
        ],
        vendor: 'hyrule',
        shippedBy: 'Piranha Plant Post',
        levelRequirement: 10
    },

  'hyrule_wario_cursed_sword': {
        id: 'hyrule_wario_cursed_sword',
        name: "WarioWare Curseblade",
        description: "",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 75000,
        icon: '🎮',
        stock: 0,
        rarity: 'legendary',
        effects: [
            "Deals 3d8 slashing damage",
        "Each hit triggers a random WarioWare glitch effect (e.g. enemy invincible for 1 turn, or player gains 50% speed for 2 turns)",
        "Causes 1d6 psychic damage to the user after 3 uses",
        "Unlocks a hidden cheat code for the player: “Gimme The Win”",
        "Requires attunement by a WarioWare Master",
        "Crafted by: Skaven",
        "0% chance to work properly",
        "If used during a battle, the user may become trapped in a loop of WarioWare glitches for 3 turns",
        "Item is cursed. Do not use near a Hyrulean temple or sacred beast."
        ],
        vendor: 'hyrule',
        shippedBy: 'Hammer Bros Handling',
        levelRequirement: 10
    },

  'ice_flower': {
        id: 'ice_flower',
        name: "Ice Flower",
        description: "A crystalline blue blossom that never melts. Grants cryokinetic abilities.",
        category: SHOP_CATEGORIES.CONSUMABLES,
        price: 30000,
        icon: '❄️',
        stock: 8,
        rarity: 'uncommon',
        effects: [
            "Duration: 1 minute",
            "Can cast Ray of Frost at will",
            "One use of Ice Knife (2nd level)",
            "Resistance to Cold damage"
        ],
        vendor: 'toad_town_market',
        shippedBy: 'Cryogenic Courier',
        levelRequirement: 3
    },

  'ice_flower_seed': {
    id: 'ice_flower_seed',
    name: "Ice Flower Seed",
    description: "A crystalline seed that grows into a frost-spitting flower. Keep away from fire.",
    category: SHOP_CATEGORIES.CONSUMABLES,
    price: 8500,
    icon: '🌸',
    stock: 10,
    rarity: 'uncommon',
    effects: [
        "Plant: Grows in 1 round, 5ft frost aura (difficult terrain)",
        "Shoots ice shards (1d8 Cold, 30ft range)",
        "Lasts 5 rounds, then shatters",
        "Slips from warm hands"
    ],
    vendor: 'shamans_hut',
    shippedBy: 'Ice Pack',
    levelRequirement: 2
},

  'ice_power_badge': {
        id: 'ice_power_badge',
        name: "Ice Power Badge",
        description: "Makes your attacks chilly. Good for fire enemies.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 45000,
        icon: '❄️',
        stock: 4,
        rarity: 'uncommon',
        effects: ["Your melee attacks deal Cold damage instead of physical", "+1 damage vs Fire enemies"],
        vendor: 'shamans_hut',
        shippedBy: 'Cold Box',
        levelRequirement: 3
    },

  'identification_service': {
    id: 'identification_service',
    name: "Arcane Identification Service",
    description: "A certified appraiser will identify any magical item, curse, or enchantment within 24 hours.",
    category: SHOP_CATEGORIES.SERVICES,
    price: 500,
    icon: '🔍',
    stock: 999,
    rarity: 'common',
    effects: [
        "Identify 1 item (magical properties, curses, origin)",
        "Expert confirmation of rarity and approximate value",
        "Provides written certificate"
    ],
    vendor: 'arcane_institute',
    shippedBy: 'Personal Appointment',
    levelRequirement: 1
},

  'immortality_binding': {
    id: 'immortality_binding',
    name: "Immortality Binding (Phylactery Service)",
    description: "Bind your soul to a phylactery, granting functional immortality at terrible cost.",
    category: SHOP_CATEGORIES.FORBIDDEN,
    price: 1000000,
    icon: '💀',
    stock: 1,
    rarity: 'godly',
    effects: [
        "You cannot die permanently; resurrection in phylactery within 1d6 days",
        "Each resurrection: gain 1 level of exhaustion (cures at full power)",
        "Phylactery must be hidden; if discovered & destroyed, you are destroyed",
        "Attracts paladins, necromancers, and cosmic entities"
    ],
    vendor: 'crypt_courier',
    shippedBy: 'Ritual Ceremony',
    levelRequirement: 16
},

  'inevitable_justice_protocol': {
    id: 'inevitable_justice_protocol',
    name: "Inevitable Justice Protocol",
    description: "A magical contract that enforces cosmic law.",
    category: SHOP_CATEGORIES.faction,
    price: 775000,
    icon: '⚖️',
    stock: 2,
    rarity: 'legendary',
    effects: [
        "Summon a Marut to enforce one contract (single use)",
        "Contract violations result in instant execution",
        "Made by: The Inevitable Tribunal"
    ],
    vendor: 'cosmic_court',
    shippedBy: 'Lawful Manifestation',
    levelRequirement: 18,
    factionBonus: { law: 200 }
},

  'infernal_contract_binder': {
    id: 'infernal_contract_binder',
    name: "Infernal Contract Binder (Legal)",
    description: "A enchanted binder that enforces devilish pacts with unbreakable infernal law.",
    category: SHOP_CATEGORIES.faction,
    price: 780000,
    icon: '📜',
    stock: 1,
    rarity: 'legendary',
    effects: [
        "Binds contracts to prevent breaches",
        "Grants +20 to persuasion in hellish dealings",
        "Made by: Baatezu Scribes"
    ],
    vendor: 'hellfire_court',
    shippedBy: 'Imp Messenger',
    levelRequirement: 17,
    factionBonus: { infernal: 50 }
},

  'infernal_gate_opener': {
    id: 'infernal_gate_opener',
    name: "Infernal Gate Opener (Portal)",
    description: "Opens temporary gates to the Nine Hells for summoning or travel.",
    category: SHOP_CATEGORIES.faction,
    price: 860000,
    icon: '🚪',
    stock: 1,
    rarity: 'godly',
    effects: [
        "Gate lasts 10 minutes, CR 15 max summon",
        "Requires infernal pact",
        "Made by: Gatekeepers of Dis"
    ],
    vendor: 'hell_portal',
    shippedBy: 'Devils Bargain',
    levelRequirement: 18,
    factionBonus: { infernal: 75 }
},

  'infernal_soul_contract': {
    id: 'infernal_soul_contract',
    name: "Infernal Soul Contract (Standard)",
    description: "Pre-written soul exchange agreement with blank penalty clause.",
    category: SHOP_CATEGORIES.forbidden,
    price: 725000,
    icon: '👹',
    stock: 6,
    rarity: 'legendary',
    effects: [
        "Trade 1 soul (yours or another's) for infernal favor",
        "Favor can be exchanged for +2 to any stat (permanent)",
        "Made by: Mammon's Legal Department"
    ],
    vendor: 'hells_notary',
    shippedBy: 'Chain Devil Courier',
    levelRequirement: 17
},

  'infernal_war_machine': {
    id: 'infernal_war_machine',
    name: "Infernal War Machine (Vehicle)",
    description: "A hellish construct for battlefield domination.",
    category: SHOP_CATEGORIES.faction,
    price: 980000,
    icon: '🚗',
    stock: 1,
    rarity: 'godly',
    effects: [
        "Carries 5 troops, 60 mph hellfire speed",
        "Armed with flame cannons",
        "Made by: Pit Forges"
    ],
    vendor: 'war_demon_factory',
    shippedBy: 'Hellwagon',
    levelRequirement: 18,
    factionBonus: { infernal: 100 }
},

  'infinite_ballroom': {
    id: 'infinite_ballroom',
    name: "Infinite Ballroom",
    description: "A pocket dimension ballroom that expands to fit any event.",
    category: SHOP_CATEGORIES.PREMIUM,
    price: 600000,
    icon: '🎭',
    stock: 1,
    rarity: 'godly',
    effects: [
        "Ballroom can host up to 10,000 guests",
        "Music, food, and decorations are provided",
        "May attract Fey or nobles"
    ],
    vendor: 'fey_market',
    shippedBy: 'Pocket Dimension Key',
    levelRequirement: 10
},

  'infinite_garden': {
    id: 'infinite_garden',
    name: "Infinite Garden",
    description: "A pocket dimension garden that grows any plant.",
    category: SHOP_CATEGORIES.PREMIUM,
    price: 400000,
    icon: '🌿',
    stock: 1,
    rarity: 'godly',
    effects: [
        "Garden grows any plant you desire",
        "Plants are always in peak condition",
        "May attract dryads or gardeners"
    ],
    vendor: 'fey_market',
    shippedBy: 'Pocket Dimension Key',
    levelRequirement: 10
},

  'infinite_library_access': {
    id: 'infinite_library_access',
    name: "Infinite Library Access (Lifetime)",
    description: "Permanent access to the largest library in existence. Gain knowledge of almost everything.",
    category: SHOP_CATEGORIES.PREMIUM,
    price: 165000,
    icon: '📚',
    stock: 1,
    rarity: 'epic',
    effects: [
        "+2 to all Knowledge checks permanently",
        "Once per long rest: gain advantage on a single Intelligence check",
        "Access to restricted archives (DM may set limits)",
        "Membership passes to 1 heir"
    ],
    vendor: 'scholar_society',
    shippedBy: 'Enchanted Library Card',
    levelRequirement: 9
},

  'infinite_stat_scroll': {
    id: 'infinite_stat_scroll',
    name: "Infinite Stat Scroll",
    description: "A scroll that lets you write any number for your stats.",
    category: SHOP_CATEGORIES.CONSUMABLES,
    price: 42000000,
    icon: '📜',
    stock: 1,
    rarity: 'godly',
    effects: [
        "Write any number for any ability score (no limit)",
        "Write any number for HP, AC, or any other stat",
        "Made by: The Number Cruncher"
    ],
    vendor: 'stat_manipulation',
    shippedBy: 'Variable Delivery',
    levelRequirement: 60
},

  'infinite_wine_cask': {
    id: 'infinite_wine_cask',
    name: "Infinite Wine Cask",
    description: "A cask that refills with fine wine every dawn.",
    category: SHOP_CATEGORIES.PREMIUM,
    price: 300000,
    icon: '🍷',
    stock: 1,
    rarity: 'legendary',
    effects: [
        "Produces 100 bottles of high-quality wine/day",
        "Wine grants +1 to CHA checks for 1 hour",
        "Cask is sentient (but never speaks)"
    ],
    vendor: 'wario_direct',
    shippedBy: 'Cask of Gold',
    levelRequirement: 6
},

  'ink_splotch_talisman': {
    id: 'ink_splotch_talisman',
    name: "Ink-Splotch Talisman",
    description: "A blot of indelible ink preserved under glass. It soaks up lies and reveals truth.",
    category: SHOP_CATEGORIES.EQUIPMENT,
    price: 12500,
    icon: '🖋️',
    stock: 4,
    rarity: 'rare',
    effects: [
        "Wearer can see lies as faint halos (1/day for 10 minutes)",
        "Can nullify forged documents within 30ft (action)",
        "Attracts pedants"
    ],
    vendor: 'black_ink',
    shippedBy: 'Stamped Tube',
    levelRequirement: 5
},

  'inked_tattoo_voucher': {
    id: 'inked_tattoo_voucher',
    name: "Inked Tattoo Voucher",
    description: "A voucher entitling the bearer to one magical tattoo session at a select parlor.",
    category: SHOP_CATEGORIES.CURIOSITIES,
    price: 7000,
    icon: '🖊️',
    stock: 10,
    rarity: 'uncommon',
    effects: [
        "Redeemable: gain a permanent minor magical tattoo (DM chooses: +1 stat, small resistance, or minor cantrip)",
        "Tattoos can be removed only by powerful magic"
    ],
    vendor: 'inked_parlor',
    shippedBy: 'Folded Card',
    levelRequirement: 1
},

  'inkopolis_cheep_cheep_accords_trinket': {
        id: 'inkopolis_cheep_cheep_accords_trinket',
        name: "Cheep Cheep Accords Charm",
        description: "A small artifact gifted to Toads who signed the Accords",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 300,
        icon: '🐸',
        stock: 20,
        rarity: 'uncommon',
        effects: [
            "Grants +2 to all Toad-related skill checks",
        "Causes a minor ink bubble to form around wearer when agitated",
        "Activates when near Rakasha Spirit Walkers",
        "Can be used to temporarily calm a Toad’s mind",
        "Scent of ink is faintly detectable by other ink-users",
        "Cursed: If used to bargain with a Toad, it may trigger a memory of the Cheep-Cheep Accords’ betrayal"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Paratroopa Air',
        levelRequirement: 2
    },

  'inkopolis_feywild_tie': {
        id: 'inkopolis_feywild_tie',
        name: "Feywild Echoes",
        description: "A shard of broken time that whispers ancient truths",
        category: SHOP_CATEGORIES.SPECIAL,
        price: 15000,
        icon: '🌀',
        stock: 2,
        rarity: 'epic',
        effects: [
            "Grants 10 seconds of time manipulation per use",
        "Triggers random time loop events in nearby area",
        "Reveals hidden paths in Feywild mazes",
        "Causes the user to lose 10% of health after each use",
        "Made by: Vampire Covenant",
        "Connects both Inkopolis and The Feywild"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 7
    },

  'inkopolis_fungi_fashion_glove': {
        id: 'inkopolis_fungi_fashion_glove',
        name: "Fungi Fashion Glove",
        description: "",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 800,
        icon: '🍄',
        stock: 10,
        rarity: 'uncommon',
        effects: [
            "Grants +1 to all skill checks involving fashion or disguise",
        "5% chance to cause nearby fungi to bloom in a glittering pattern",
        "Slightly increases stealth in mushroom forests",
        "Made from enchanted toadstool skin harvested from the Princess’s abandoned garden",
        "Unwittingly triggers a minor fungal rebellion if worn during a Fungi civil war",
        "Crafted by: Dry Bones Dead Drop"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Dry Bones Dead Drop',
        levelRequirement: 3
    },

  'inkopolis_ink_scarf_of_the_princess': {
        id: 'inkopolis_ink_scarf_of_the_princess',
        name: "Ink Scarf of the Princess",
        description: "Woven from the ink of the slain Princess Peach, this scarf pulses with her final breaths and whispered regrets. It was smuggled into Inkopolis by a rogue diplomat who dared to bury her name beneath the ink of the Beanbean Kingdom. Now, it grants the wearer a strange sense of purpose and a haunting whisper that only those who remember her will understand.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 1200,
        icon: '🌊',
        stock: 1,
        rarity: 'rare',
        effects: [
            "+2 to all ink-based stealth checks",
        "-1 to perception rolls when near ink squid",
        "Triggers a memory flash every 10 minutes if the wearer remembers the Princess",
        "Worn by: Beanbean Diplomats",
        "Only usable if wearer has witnessed the Princess’s assassination"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Mages Guild Portal',
        levelRequirement: 4
    },

  'inkopolis_ink_siphon_torch': {
        id: 'inkopolis_ink_siphon_torch',
        name: "Ink Siphon Torch",
        description: "A torch that drains ink from foes and refills your own ink reserves",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 1200,
        icon: '💧',
        stock: 2,
        rarity: 'uncommon',
        effects: [
            "Siphons 1d4 ink from enemies to restore 1d4 ink to user",
        "Causes ink to spill on foes when struck, creating temporary ink fog",
        "Can only be used in areas with ink-based terrain",
        "Glows dimly when ink is low",
        "Requires ink to be present in environment",
        "Cannot be used underwater"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Koopa Postal',
        levelRequirement: 4
    },

  'inkopolis_ink_wars_gearbox': {
        id: 'inkopolis_ink_wars_gearbox',
        name: "Ink Wars Gearbox",
        description: "",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 3500,
        icon: '🖋',
        stock: 2,
        rarity: 'rare',
        effects: [
            "Allows wearer to fire ink-based projectiles with 1d8 damage",
        "Each shot consumes 1 ink token (replenishes after 3 rounds)",
        "Grants +2 to all Ink Warfare skill checks",
        "Unlocks ability to temporarily turn ink into a liquid shield",
        "10% chance to trigger a magical ink bloom that obscures vision",
        "Crafted by: Servants Cosmic"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Dry Bones Dead Drop',
        levelRequirement: 6
    },

  'inkopolis_item_name_here': {
        id: 'inkopolis_item_name_here',
        name: "Festival of Falling Stars Amulet",
        description: "A glowing pendant that echoes Peachs memory",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 10000,
        icon: '🌠',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Grants immunity to mental infections from The Internet",
        "Increases critical hit chance by 30%",
        "Causes faint visions of Peach during events",
        "Draws attention from rival factions during festivals",
        "Made by: Vampire Covenant",
        "Event exclusive: Festival of Falling Stars"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Lakitu Drones',
        levelRequirement: 8
    },

  'inkopolis_mage_soul_tear': {
        id: 'inkopolis_mage_soul_tear',
        name: "Mage Soul Tear",
        description: "A single drop of liquid magic harvested from the heart of a fallen spellcaster who sacrificed their soul to save Inkopolis from a sentient ink storm. The tear is sealed in a glass vial that glows with the color of a dying star. It was hidden in a Beanbean coin vault, discovered by a disgraced prince who swore to avenge his sister’s murder.",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 600,
        icon: '💧',
        stock: 4,
        rarity: 'uncommon',
        effects: [
            "Restore 300 MP after use",
        "+1 to all spellcasting rolls for 1 round",
        "Causes 1d6 psychic damage to any enemy who touches it",
        "Only usable by spellcasters",
        "Consumes 1 coin of Beanbean origin"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Mages Guild Portal',
        levelRequirement: 4
    },

  'inkopolis_minus_world_keychain': {
        id: 'inkopolis_minus_world_keychain',
        name: "Minus World Keychain",
        description: "A relic from the lost world that whispers forgotten truths",
        category: SHOP_CATEGORIES.MATERIAL,
        price: 75000,
        icon: '🌌',
        stock: 0,
        rarity: 'legendary',
        effects: [
            "Reveals the last known location of the Minus World",
        "Grants a 5% chance to discover a hidden portal with every use",
        "Triggers visions of lost memories if worn during sleep",
        "Increases luck during exploration in ink-based environments",
        "Cannot be used without a confirmed ink-based map",
        "Wears the user a faint ink stain that fades after 12 hours of use"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Koopa Postal',
        levelRequirement: 8
    },

  'inkopolis_sarasaland_cloak_of_diplomacy': {
        id: 'inkopolis_sarasaland_cloak_of_diplomacy',
        name: "Cloak of Diplomacy",
        description: "A diplomatic garment that tempers aggression and amplifies charm",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 4500,
        icon: '🌿',
        stock: 1,
        rarity: 'epic',
        effects: [
            "Grants +2 to Diplomacy checks against all factions",
        "Reduces enemy aggression by 50% during conversation",
        "Increases charm bonus by 1d4 when used near Princess Daisy’s territory",
        "Unlocks access to secret diplomatic routes between Inkopolis and Sarasaland",
        "If worn in public, reveals the wearer’s true intentions to others",
        "Cannot be worn while engaging in combat"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Koopa Postal',
        levelRequirement: 6
    },

  'inkopolis_scam_ink_bottle': {
        id: 'inkopolis_scam_ink_bottle',
        name: "The Infinite Inkwell",
        description: "",
        category: SHOP_CATEGORIES.CONSUMABLE,
        price: 10000,
        icon: '💧',
        stock: 1,
        rarity: 'legendary',
        effects: [
            "Consuming this bottle grants 1d4 temporary ink-based abilities (e.g. ink breath, ink vision, ink mimicry)",
        "50% chance to trigger a cursed ink curse: permanently alters ink coloration to match your enemy’s favorite hue",
        "100% chance to make you forget your name for 1 hour",
        "100% chance to summon a sentient ink blob that demands tribute",
        "100% chance to be a scam: the bottle is empty, filled with glitter, and smells like a forgotten perfume",
        "Crafted by: Mr. Wario’s Secret Workshop",
        "“The Inkwell of Infinite Possibility” – a joke that never ends"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Dry Bones Dead Drop',
        levelRequirement: 8
    },

  'inkopolis_squid_silk_glove': {
        id: 'inkopolis_squid_silk_glove',
        name: "Squid Silk Glove",
        description: "A dexterous, ink-dripping glove designed for ink warfare",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 450,
        icon: '🐙',
        stock: 12,
        rarity: 'common',
        effects: [
            "Grants +1d4 ink spray damage on melee hits",
        "Slight chance to temporarily blind enemies when ink splashes",
        "Can be enchanted with scent of fear for 2 rounds",
        "Made from deep-sea squid silk harvested by the Inkopolis Guild"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Paratroopa Air',
        levelRequirement: 3
    },

  'inkopolis_squid_silk_gloves': {
        id: 'inkopolis_squid_silk_gloves',
        name: "Squid Silk Gloves",
        description: "These gloves are made from the delicate tentacles of the Inkopolis Squid Queen’s personal matriarch, spun into a lace-like fabric that shimmers with bioluminescent ink. They were once worn by the Queen’s assassin, now turned traitor, who smuggled them into Beanbean territory to trade for political leverage. Each finger is tipped with a micro-gel that can mimic ink-squid movement.",
        category: SHOP_CATEGORIES.EQUIPMENT,
        price: 800,
        icon: '🐙',
        stock: 2,
        rarity: 'uncommon',
        effects: [
            "+1 to dexterity checks against ink-based traps",
        "-2 to all dexterity checks when moving through ink fog",
        "Can mimic ink squid movement for 10 seconds",
        "Crafted by: Inkopolis Guild of Seamstresses",
        "Cannot be used in direct combat with living ink squid"
        ],
        vendor: 'inkopolis',
        shippedBy: 'Mages Guild Portal',
        levelRequirement: 3
    },

};
