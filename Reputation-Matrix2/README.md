...
| Mages' Guild Codex | `data/laws/mages-guild-code.json`, its outline, and `../tools/gen-mages-guild-code.py` | generator-maintained Codex pages and cross-references; reader UI is hand-written | root `mages-code.html`: searchable scroll view plus the page-at-a-time book reader in `app/pages/mages-codex-reader.js` |
...
## Operating notes by system

Keep this README as a source-of-truth map. Detailed decision trees live in
`../docs/`, especially
[`INTAKE_DECISION_GUIDE.md`](../docs/INTAKE_DECISION_GUIDE.md) for deciding
what to create from new data.

- **RNN** — scripts live in `../tools/rnn-scripts/`;
  `data/rnn-broadcasts.js` and README `RNN:LAST-WEEK` blocks are generated by
  `python3 ../tools/build-rnn-broadcast.py`. Cadence is one episode per about
  ten filed events.
- **Mages' Guild Codex** — `../tools/gen-mages-guild-code.py` owns expansion of
  `data/laws/mages-guild-code.json`; do not duplicate that prose in the reader.
  The root `../mages-code.html`, `app/pages/mages-codex-reader.js`, and the
  scoped `app/styles/systems/mages-codex-{reader,index}.css` files are the
  hand-written reading surface. Validate with
  `python3 ../tools/gen-mages-guild-code.py --validate`,
  then smoke-test cover → index → numbered page in the browser.
- **Article analyses** — add records to `data/articleAnalyses.json`; source
  events remain authoritative for chronology, custody, XP, reputation, images,
  and outcomes.
- **Investigations** — update `data/investigations.json` only after the event
