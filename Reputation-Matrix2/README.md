...
Dossiers must be evidence-bound: name the actions, objects, places, relationships, and uncertainties present in the linked records; use Waluigi’s archival voice; and never manufacture an organisation around a person, location, event, species, generic public, or aggregate label such as “all factions.” Misfiled labels are retained as retired audit records, excluded from the live faction directory, and their generated reputation keys are redirected to a canonical faction only when the source justifies that redirect. Run this pass directly with `python3 tools/generate_all.py --only faction-dossiers`, or let it interleave with the other stage-1 systems.
