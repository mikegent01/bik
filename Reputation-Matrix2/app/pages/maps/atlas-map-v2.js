...
const MODES = {population:['Population','population'], military:['Military','military_strength'], economy:['Economy','economic_value'], influence:['Influence','political_influence'], antiquity:['Age of Antiquity','age_of_antiquity'], crime:['Crime Rate','crime_rate']};
...
function render(host, mapId) {
  ...
  host.innerHTML = `<section class="atlas-v2" aria-label="${esc(map.name)} tactical map"><header><div><span class="atlas-v2-eyebrow">WORLD ATLAS · LIVE MAP</span><h2>${esc((map.name || map.id).replace(' (Full)', ''))}</h2><p>${pois.length} surveyed locations · ${format(population)} mapped residents</p></div><div class="atlas-v2-actions"><button data-action="fit">Reset view</button><button data-action="labels">Labels</button></div></header><div class="atlas-v2-tools"><input type="search" data-search placeholder="Search mapped locations…"><select data-type><option value="">All location types</option>${[...new Set(pois.map(p=>p.type).filter(Boolean))].sort().map(t=>`<option value="${esc(t)}">${esc(humanize(t))}</option>`).join('')}</select><span data-visible>${pois.length} markers visible</span></div><div class="atlas-v2-modes">${Object.entries(MODES).map(([id,[label]])=>`<button class="${id==='population'?'active':''}" data-mode="${id}">${label}</button>`).join('')}<b data-mode-total>${format(population)} residents</b></div><div class="atlas-v2-layout"><main class="atlas-v2-stage"><div class="atlas-v2-image"><div class="atlas-v2-world"><img src="${new URL(`../../../${map.imageSrc}`, import.meta.url).href}" alt="${esc(map.name)} map"><div class="atlas-v2-markers">${pois.map(marker).join('')}</div></div><div class="atlas-v2-minimap" hidden><div><img src="${new URL(`../../../${map.imageSrc}`, import.meta.url).href}"><button class="atlas-v2-mini-pin" style="left:50%;top:50%">●</button></div><small>Selected location</small></div></div><div class="atlas-v2-legend"><span><i></i>POI markers</span><span>${esc(map.group || 'Unclassified')}</span></div></main><aside class="atlas-v2-sidebar">${detail(null)}</aside></div></section>`;
  ...
  host.querySelectorAll('[data-poi]').forEach(button => button.addEventListener('click', () => {
    ...
    const poi = pois.find(poi => poi.id === button.dataset.poi); sidebar.innerHTML = detail(poi, pois);
    ...
    mini.hidden=false; mini.style.setProperty('--focus-x', poi.x || 50); mini.style.setProperty('--focus-y', poi.y || 50); const miniPin=mini.querySelector('.atlas-v2-mini-pin'); miniPin.style.left=`${poi.x || 50}%`; miniPin.style.top=`${poi.y || 50}%`;
  }));
  ...
  const applyFilter=()=>{const query=host.querySelector('[data-search]').value.toLowerCase(),type=host.querySelector('[data-type]').value;let shown=0;host.querySelectorAll('[data-poi]').forEach(pin=>{const poi=pois.find(x=>x.id===pin.dataset.poi),visible=(!query||`${poi.name} ${poi.description||''}`.toLowerCase().includes(query))&&(!type||poi.type===type);pin.hidden=!visible;if(visible)shown++;});host.querySelector('[data-visible]').textContent=`${shown} markers visible`;};
  host.querySelector('[data-search]').addEventListener('input',applyFilter);host.querySelector('[data-type]').addEventListener('change',applyFilter);
  sidebar.addEventListener('click',event=>{const jump=event.target.closest('[data-jump]');if(jump)host.querySelector(`[data-poi="${CSS.escape(jump.dataset.jump)}"]`)?.click();});
  ...
  host.querySelector('[data-action="fit"]').addEventListener('click', () => { host.querySelectorAll('[data-poi]').forEach(item => item.classList.remove('selected')); sidebar.innerHTML = detail(null, pois); const world=host.querySelector('.atlas-v2-world'),mini=host.querySelector('.atlas-v2-minimap');world.classList.remove('focused');mini.hidden=true; });
}
