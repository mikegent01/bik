...
(() => {
  ...
  function buildUi() {
    ...
    style.textContent = `.walu-ambient{position:fixed;right:14px;bottom:14px;z-index:450;display:flex;align-items:center;gap:4px;padding:4px;border:1px solid rgba(172,112,255,.35);border-radius:999px;background:rgba(18,10,30,.82);box-shadow:0 4px 16px rgba(0,0,0,.35);color:#eadbff;font:12px system-ui,sans-serif;backdrop-filter:blur(8px);opacity:.62;transition:opacity .15s ease}.walu-ambient:hover{opacity:1}.walu-ambient a{width:30px;height:30px;display:grid;place-items:center;border-radius:50%;color:#f4e7ff;text-decoration:none;border:1px solid rgba(234,219,255,.22)}.walu-ambient a:hover{background:rgba(172,112,255,.25)}.walu-ambient[hidden]{display:none}.walu-ambient button{border:1px solid rgba(234,219,255,.28);background:rgba(255,255,255,.08);color:#f4e7ff;border-radius:999px;min-width:30px;height:30px;cursor:pointer;pointer-events:auto}.walu-ambient button:hover{background:rgba(172,112,255,.25)}.walu-ambient-drag{cursor:grab!important;touch-action:none}.walu-ambient-drag:active{cursor:grabbing!important}.walu-ambient-title{max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;opacity:.88}.walu-ambient.is-playing{box-shadow:0 0 22px rgba(172,112,255,.32),0 8px 30px rgba(0,0,0,.45)}.walu-ambient.is-dragging{opacity:.9;box-shadow:0 16px 45px rgba(0,0,0,.5)}.walu-ambient.is-collapsed [data-walu-secondary],.walu-ambient.is-collapsed .walu-ambient-title{display:none}
/* The widget was eight glyph buttons in a row -- shuffle, next, library, phone
   mode, collapse, dismiss -- with no labels, which is why it read as useless.
   Play, the cog and the window controls stay out; everything else lives behind
...
.walu-ambient.tray-open [data-walu-settings]{transform:rotate(60deg);color:#ffd94d}.walu-music-panel{position:fixed;z-index:449;width:min(360px,calc(100vw - 20px));max-height:min(430px,calc(100vh - 90px));overflow:auto;border:1px solid rgba(172,112,255,.45);border-radius:18px;background:linear-gradient(180deg,rgba(34,18,48,.98),rgba(9,5,18,.98));box-shadow:0 18px 50px rgba(0,0,0,.55);color:#eadbff;padding:12px;display:none;font:12px system-ui,sans-serif}.walu-music-panel.open{display:block}.walu-music-panel-head{display:flex;justify-content:space-between;gap:8px;align-items:center}.walu-music-panel h3{margin:0 0 4px;font-size:14px}.walu-music-panel-close{border:1px solid rgba(234,219,255,.25);border-radius:50%;width:28px;height:28px;background:rgba(255,255,255,.08);color:#fff;cursor:pointer}.walu-music-panel p{margin:0 0 10px;opacity:.7;font-size:12px;line-height:1.35}.walu-music-list{display:grid;gap:7px}.walu-music-list button{width:100%;display:grid;grid-template-columns:24px 1fr auto;gap:8px;align-items:center;text-align:left;border:1px solid rgba(234,219,255,.16);border-radius:12px;background:rgba(255,255,255,.06);color:#f4e7ff;padding:8px;cursor:pointer}.walu-music-list button:hover,.walu-music-list button.active{border-color:#f8c14a;background:rgba(248,193,74,.13)}.walu-music-list small{opacity:.55}.walu-music-volume{width:100%;accent-color:#a855f7}@media(max-width:700px){.walu-ambient{right:10px;bottom:10px}.walu-ambient-title{max-width:105px}.walu-ambient [data-walu-music-shuffle]{display:none}}`;
    ...
    /* The control bar is now just play + a link to Settings.
       Everything else it used to carry -- shuffle, next, the library panel,
       mobile mode, volume, and the drag/collapse/dismiss controls invented to
  ...
  document.addEventListener('click', event => {
    const target = event.target?.closest?.('#musicBtn,[data-walu-settings],[data-walu-music-play],[data-walu-music-next],[data-walu-music-shuffle],[data-walu-music-panel-toggle],[data-walu-music-panel-close],[data-walu-music-track],[data-walu-music-collapse],[data-walu-music-dismiss],[data-walu-music-restore],[data-walu-mobile-mode]');
    if (!target) return;
    event.preventDefault();
    event.stopPropagation();
    if (target.matches('[data-walu-settings]')) {
      showWidget();
      const open = ui?.classList.toggle('tray-open');
      target.setAttribute('aria-expanded', open ? 'true' : 'false');
    }
    else if (target.matches('[data-walu-music-next]')) nextTrack();
