...
class BrosAttackWindow extends foundry.applications.api.ApplicationV2 {
  ...
  motion(attack,a,b,target) {
    const overlay=document.createElement("div");overlay.className="bros-clean-overlay";overlay.innerHTML=`<div class="bros-clean-drill"><h2>🤝 ${attack.name}</h2><div class="turn" data-turn></div><div class="bros-clean-pad" data-pad></div><div class="bros-clean-help" data-help></div><div data-progress></div><div class="bros-clean-buttons"><button class="bros-clean-cancel">Cancel</button></div></div>`;document.body.appendChild(overlay);
    const pad=overlay.querySelector("[data-pad]"),turn=overlay.querySelector("[data-turn]"),help=overlay.querySelector("[data-help]"),progress=overlay.querySelector("[data-progress]");let mode=null,index=0,last=null,distance=0,started=0,failures=0,finished=false;
    const cleanup=()=>overlay.remove(); const current=()=>attack.steps[index];
    const draw=()=>{const s=current();pad.replaceChildren();const icon=document.createElement("span");icon.className="bros-clean-icon";icon.textContent=s.icon;const arrow=document.createElement("span");arrow.className="bros-clean-arrow";arrow.textContent=s.gesture==="up"?"↑":s.gesture==="down"?"↓":s.gesture==="right"?"→":s.gesture==="aim"?"◎":"✦";pad.append(icon,arrow);turn.textContent=`${mode==="solo"?"One player":s.actor==="A"?a.name:b.name} — ${s.title}`;help.textContent=s.instruction;progress.textContent=`Step ${index+1} of ${attack.steps.length} · Mistakes remaining: ${2-failures}`;started=performance.now();distance=0;last=null;pad.className="bros-clean-pad";};
    const fail=()=>{if(finished)return;failures++;pad.className="bros-clean-pad fail";if(failures>=3){turn.textContent="Drill failed";help.textContent="Three mistakes broke the rhythm. No rolls were made and no energy was spent.";setTimeout(cleanup,1300);}else{turn.textContent="Motion missed";help.textContent="Try the same step again. Movement gestures do not need a mouse button.";setTimeout(draw,800);}};
    const success=()=>{if(finished)return;index++;if(index>=attack.steps.length){finished=true;pad.className="bros-clean-pad good";turn.textContent="Maneuver complete";help.textContent="The drill succeeded. Spending Bros Energy and resolving the normal rules…";setTimeout(async()=>{cleanup();if(await spendEnergy([a,b]))this.resolve(attack,a,b,target);},700);}else draw();};
