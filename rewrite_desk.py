...
new_func = r'''def events_generate(task: Task, client: Any, temperature: float) -> dict[str, Any]:
...
# Regex to match the existing events_generate function
pattern = re.compile(r'^def events_generate\(task: Task, client: Any, temperature: float\) -> dict\[str, Any\]:.*?(?=^def )', re.MULTILINE | re.DOTALL)
