import { WALLETS, CURRENCIES } from './Reputation-Matrix2/app/pages/commerce/currency.js';
...
async function loadWallets() {
  try {
    const walletUrl = new URL('./Reputation-Matrix2/data/commerce/wallets.json', import.meta.url);
    ...
    console.warn('[Shop Currency] Could not load Reputation-Matrix2/data/commerce/wallets.json; using currency.js wallets.', error);
  }
...
async function initShopAutoLinker() {
  ...
  try {
    ...
    await Promise.all(files.map(async f => {
      try {
        const res = await fetch(new URL(`./Reputation-Matrix2/data/${f}.json`, import.meta.url), { cache: 'no-cache' });
