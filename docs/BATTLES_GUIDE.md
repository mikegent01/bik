...
| `image` / `imageCaption` | – | Path relative to `Reputation-Matrix2/`. **`imageCaption` is required whenever `image` is set — `tools/check-battles.py` FAILS without it.** Both `battles.json` and `majorBattles.json` support these fields and both render through `battleThumb()`. Fill them with `tools/slice-battle-sheet.py` (3x2 sheets, `--gravity center`), prompt-sheeted first per [`IMAGE_GENERATION_GUIDE.md`](IMAGE_GENERATION_GUIDE.md); `Text in image: NONE` unless the prose names an inscription |
