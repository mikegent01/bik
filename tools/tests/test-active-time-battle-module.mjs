...
check('ATB API exposes state for NPC automation bridges', js.includes('isRunning,') && js.includes('activeId') && js.includes('isPrimaryGM') && js.includes('That combatant is not the active ATB turn'));
...
const combatAutomationPath = path.resolve('combat/scripts/automation.js');
if (fs.existsSync(combatAutomationPath)) {
  const nca = fs.readFileSync(combatAutomationPath, 'utf8');
  check('NPC combat automation bridge hands completed NPC turns back to ATB',
    nca.includes('_endActiveTimeBattleTurn') &&
    nca.includes('game.modules?.get("active-time-battle")?.api') &&
    nca.includes('api.endTurn(combat, combatant.id') &&
    nca.includes('api.isPrimaryGM'));
}

const combatMainPath = path.resolve('combat/scripts/main.js');
if (fs.existsSync(combatMainPath)) {
  const main = fs.readFileSync(combatMainPath, 'utf8');
  check('NPC combat automation defers to the ATB primary GM',
    main.includes('_activeTimeBattleApi') &&
    main.includes('ATB primary GM owns NPC automation handoff'));
}
