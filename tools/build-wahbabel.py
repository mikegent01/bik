...
# ------------------------------------------------------------- vocabulary
# One shared concept list, so every language covers the same ground and a
# sentence built from it can be written in any of them. A word is only listed
# here when it earns its place: the things a table actually says, plus the
# nouns this campaign keeps using.
#
# Adding a word: add the English term here, then give each language its form
# in WORDS below. Anything missing falls through to the cipher, so a partial
# entry is safe -- it just lowers that language's dictionary coverage.
CONCEPTS = [
    # people
    "friend", "enemy", "stranger", "guest", "host", "lord", "lady", "king",
    "guard", "servant", "thief", "witch", "child", "brother", "sister", "name",
    # body and state
    "blood", "bone", "hand", "eye", "heart", "dead", "alive", "hurt", "tired",
    "afraid", "angry", "mad",
    # the table's verbs
    "go", "run", "stop", "wait", "hide", "fight", "kill", "help", "take",
    "give", "open", "close", "speak", "listen", "look", "find", "lose",
    "remember", "forget", "swear", "lie", "trade", "pay", "owe",
    # things
    "door", "key", "book", "sword", "knife", "shield", "rope", "gold", "coin",
    "food", "water", "wine", "fire", "stone", "iron", "wood", "cloth", "ring",
    "map", "letter", "poison", "medicine",
    # place and time
    "house", "manor", "forest", "mountain", "river", "road", "city", "grave",
    "night", "day", "morning", "winter", "year", "today", "tomorrow", "never",
    # abstractions a session needs
    "danger", "safe", "truth", "lie_noun", "debt", "oath", "law", "war",
    "peace", "magic", "curse", "gift", "secret", "story", "song",
    # the small words that make a sentence
    "yes", "no", "hello", "goodbye", "please", "sorry", "thanks", "and", "or",
    "not", "this", "that", "here", "there", "who", "what", "why", "how",
    "many", "few", "big", "small", "good", "bad", "old", "new",
]
...
# ------------------------------------------------------------------ glyphs
# Words whose form is NOT what the cipher would produce. These are the
# irregulars -- the parts of a language that have to be learned rather than
# derived, which is what makes it feel like a language and not a filter.
# Every other concept is generated by running the concept through the
# language's own rules, so the dictionary is complete without being invented.
IRREGULARS = {
    "sylvan": {
        "friend": "aelaen", "danger": "morrath", "book": "vaelthum",
        "water": "sylae", "door": "thaelis", "name": "aesith",
        "blood": "vaerith", "manor": "thaelhaus", "song": "lirren",
        "gold": "aurael", "yes": "aeya", "no": "nael", "hello": "aelann",
        "run": "faellis", "forest": "aolothae", "magic": "sylaevar",
        "oath": "vaelorn", "debt": "oissael", "night": "nuathel",
        "secret": "hasselan", "truth": "aevarien", "guest": "aelvaen",
    },
    "goblin": {
        "gold": "shinies", "lord": "bigg", "food": "grub", "no": "nuh",
        "yes": "yaz", "fight": "krump", "run": "skarper", "friend": "notfood",
        "knife": "stikka", "trade": "trayd", "big": "bigg", "small": "grot",
        "hello": "oi", "danger": "badbadbad", "water": "wet",
        "book": "flatthing", "magic": "zappy", "dead": "stopped",
        "gift": "freestuff", "law": "bigg-sez", "secret": "shh",
        "enemy": "food", "king": "biggestbigg",
    },
    "draconic": {
        "fire": "irkhan", "mountain": "korthaxi", "gold": "vharaz",
        "name": "sirrakh", "dead": "morrkhaan", "king": "kaazirn",
        "oath": "vharrun", "law": "korrun", "blood": "sarrkh",
        "hello": "kharesh", "yes": "azh", "no": "nekh", "danger": "morrvax",
        "book": "khirith", "friend": "vhaxen", "magic": "zirrakh",
        "war": "vhagorn", "truth": "korrazh", "curse": "morrsirakh",
        "year": "kaazun", "never": "nekhaan",
    },
    "orcish": {
        "law": "grukk", "big": "brakh", "small": "grishnak", "war": "wagh",
        "lord": "urzog", "iron": "durak", "no": "nar", "yes": "zug",
        "fight": "wagh", "hello": "grakh", "friend": "bruddah",
        "blood": "gorr", "danger": "skragh", "gold": "shiny-rock",
        "dead": "krumped", "oath": "grukk-word", "enemy": "grishnak",
        "king": "urzog-brakh", "peace": "boring",
    },
    "feyward_court": {
        "guest": "aiveisse", "host": "vhairen", "debt": "oissun",
        "please": "aivh", "sorry": "nevhaii", "name": "aissen",
        "lady": "vhairenna", "yes": "aivhee", "no": "nevh",
        "hello": "aivhen", "danger": "morrivh", "oath": "vhaissun",
        "secret": "nevhaissen", "lie": "aivhessa", "truth": "vhairessa",
        "guard": "vhaissar", "manor": "vhairhaus",
    },
    "toadish": {
        "help": "taptap", "danger": "tapataptap", "safe": "tap",
        "friend": "opdop", "run": "tatatat", "yes": "tap-tap",
        "no": "taaap", "here": "dop", "hello": "opdap", "food": "mush",
        "hide": "tapdop", "wait": "taaa", "listen": "opop",
        "enemy": "dopdop", "dead": "notap",
    },
    "fungal": {
        "hello": "spuum", "yes": "mmuh", "no": "ssnn", "water": "wehtuu",
        "night": "ohsspuh", "danger": "sshnnuh", "friend": "mmuhspuum",
        "name": "spuhnn", "dead": "uunngrown", "food": "uhbsorb",
        "magic": "spuumuum", "secret": "sshspuh",
    },
}
...
def tidy(word: str) -> str:
    """Keep a ciphered word pronounceable.

    Applying every rule to a long word stacks expansions until it is noise:
    "remember" came out "regmegmbegr" and "mountain" as "mmaouintayeyn".
    Nobody can say those, so nobody will use them. Three passes:
      - collapse a letter tripled or more down to two;
      - break up a run of four or more consonants;
      - cap the length, cutting on a vowel boundary so the ending still sounds
        like the language rather than a truncation.
    """
    vowels = "aeiouy"
    out = []
    for ch in word:
...
def apply_cipher(word: str, rules: list) -> str:
    """Run one word through a language's ordered rules, then tidy it.

    Same rule logic as the browser's wbTranslateWord, kept deliberately in
    step: if these two ever disagree, the dictionary would not match what the
    page produces for the same input.
    """
    out = word.lower()
    for frm, to in rules:
...
def build_lexicon(lang: dict) -> dict:
    """Every concept gets an entry. Irregulars are listed by hand; the rest are
    derived by running the concept through the language's own cipher, so the
    dictionary is complete and internally consistent without inventing words
    nobody can predict."""
    irr = IRREGULARS.get(lang["id"], {})
    lex = {}
    for concept in CONCEPTS:
        # "lie_noun" disambiguates the noun from the verb in the concept list;
        # the dictionary shows the plain word.
        shown = concept.replace("_noun", "")
        if concept in irr:
            lex[shown] = irr[concept]
        else:
            lex[shown] = apply_cipher(shown, lang["rules"])
    return lex
    ...
    for L in LANGUAGES:
        entry = {k: L[k] for k in
                 ("id", "name", "family", "canon", "speakers", "script",
                  "colour", "flavour", "rules")}
        entry["lexicon"] = build_lexicon(L)
        entry["irregulars"] = sorted(IRREGULARS.get(L["id"], {}).keys())
        # Two concepts can legitimately share a word: Orcish has one word for
        # "fight" and "war", Goblin one for "lord" and "big". That is a fact
        # about the language, not a fault -- but reverse translation has to
        # pick one, so the ambiguity is published rather than hidden.
        _rev: dict = {}
        for _en, _fo in entry["lexicon"].items():
            _rev.setdefault(_fo, []).append(_en)
        entry["homonyms"] = {k: sorted(v) for k, v in sorted(_rev.items()) if len(v) > 1}
        entry["glyphs"] = build_glyphs(L)
        entry["lexiconSize"] = len(entry["lexicon"])
        langs.append(entry)
    return {
        "_README": {
...
def main() -> int:
    ...
    for L in langs:
        ...
        expected = {c.replace("_noun", "") for c in CONCEPTS}
        missing = expected - set(L["lexicon"])
        if missing:
            problems.append(f"{L['id']}: missing {len(missing)} concepts, e.g. {sorted(missing)[:3]}")
        # A cipher that leaves a word unchanged has not translated it.
        same = [k for k, v in L["lexicon"].items() if k == v]
        if len(same) > len(expected) * 0.15:
            problems.append(f"{L['id']}: {len(same)} words identical to English, e.g. {same[:4]}")
        # A word nobody can pronounce is a word nobody will use at the table.
        # Only CIPHERED words are checked: irregulars are hand-written and are
        # allowed to be strange on purpose -- Toadish "taaa" is a held drum
        # beat and Fungal "sshnnuh" is a hiss, not a typo.
        import re as _re
        irr_forms = set(IRREGULARS.get(L["id"], {}).values())
        harsh = [v for v in L["lexicon"].values()
                 if v not in irr_forms
                 and (len(v) > 13 or _re.search(r"[bcdfghjklmnpqrstvwxz]{4,}", v)
                      or _re.search(r"(.)\1\1", v))]
        if harsh:
            problems.append(f"{L['id']}: {len(harsh)} unpronounceable ciphered words, e.g. {harsh[:3]}")
    ...
    for L in langs:
        print(f"       {L['id']:<16} {L['script']:<11} words {L['lexiconSize']:>4} "
              f"({len(L['irregulars'])} irregular) · rules {len(L['rules']):>2}")
