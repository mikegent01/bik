#!/usr/bin/env python3
"""Slice a 3x3 contact sheet of field-sheet backgrounds into individual files.

Why this exists
---------------
Field sheets are the Reading Desk's unlockable site backgrounds. They are
generated three sheets at a time (one per rarity tier) because a 3x3 contact
sheet costs one image generation for nine backgrounds, and the nine come back
stylistically consistent *because they were drawn together*.

Grid size follows the finding recorded in tools/slice-banner-sheet.py and
docs/worklists/REMASTER_PLAN_2026_09.md: a 9x9 sheet at ~1024px gives each
cell ~113px and coherence collapses (motifs bleed into neighbours), so the
desk does not use 9x9 even though it was the first suggestion on the table.
3x3 yields ~340px cells, sliced out at 512px and served `cover` + `fixed`,
which scales cleanly to any viewport. That is the whole answer to
"background sheets that scale well".

The generator draws its own gutters (and on one sheet a decorative film
frame), so the sheet is first cropped with --crop fractions, then the real
cell boundaries are found by looking for the dark gutter rows/columns, then
each cell is trimmed inward to drop the residual gutter edge.

Usage
-----
  python3 tools/slice-field-sheet.py \\
      --sheet Reputation-Matrix2/assets/images/field-sheets/_sheets/sheet-vault.jpg \\
      --crop 0.035 0.04 0.957 0.958 \\
      --names oracle-marble wahwire-frame brass-deco \\
              starfall-teal oilslick filigree-char \\
              violet-rain obsidian-scale tracery-moon
...
def main():
    ...
    ap.add_argument("--dest", default="Reputation-Matrix2/assets/images/field-sheets")
    ap.add_argument("--crop", nargs=4, type=float, default=[0.0, 0.0, 1.0, 1.0],
                    metavar=("LEFT", "TOP", "RIGHT", "BOTTOM"),
                    help="fractions of the sheet to keep before looking for gutters")
    ...
    w, h = img.size
    cl, ct, cr, cb = args.crop
    if (cl, ct, cr, cb) != (0.0, 0.0, 1.0, 1.0):
        img = img.crop((round(w * cl), round(h * ct), round(w * cr), round(h * cb)))
    ...
    for r in range(n):
        for c in range(n):
            ...
            out = os.path.join(args.dest, f"{name}.jpg")
            cell.save(out, "JPEG", quality=args.quality, optimize=True)
            print(f"  wrote {out}  ({edge}px source)")
            written += 1

    print(f"{written} field sheet(s) written to {args.dest}")
